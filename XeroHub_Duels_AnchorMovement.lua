...
-- ==========================================
-- ANCHOR MOVEMENT | SPEED + FLY
--
-- No usa WalkSpeed ni BodyVelocity:
-- ancla HumanoidRootPart y desplaza Character:PivotTo() con deltaTime.
-- Speed sigue el suelo por raycast; Fly usa movimiento 3D relativo a cámara.
-- ==========================================
...
function runtime.AnchorMovementGetMoveVector()
    ...
    -- PlayerModule: sirve para joystick móvil y teclado/gamepad.
    ...
    -- Fallback PC si el ControlModule todavía no terminó de cargar.
...
function runtime.AnchorMovementWorldDirection(mode)
    ...
    if mode == "Fly" then
        -- Fly completo: forward sigue también el pitch de la cámara.
        ...
        -- Speed: siempre horizontal y relativo a la cámara.
...
function runtime.AnchorMovementRaycastGround(position, char)
...
function runtime.AnchorMovementEnable(mode)
    ...
    -- Ghost mueve el mismo HRP dos veces por frame; apagar su toggle evita
    -- que ambos sistemas intenten escribir el CFrame en el mismo frame.
    ...
    local initialHit = runtime.AnchorMovementRaycastGround(root.Position, char)
    state.GroundOffset =
        initialHit and (root.Position.Y - initialHit.Position.Y)
        or (hum.HipHeight + root.Size.Y * 0.5)
    ...
    state.RenderConnection = runtime.Track(
        RunService.RenderStepped:Connect(function(dt)
            ...
            -- El HRP debe permanecer anclado incluso si otro script intenta soltarlo.
            if not root.Anchored then
                root.Anchored = true
            end

            local direction = runtime.AnchorMovementWorldDirection(mode)
            ...
            if direction.Magnitude > 0.001 then
                local desiredPos =
                    ...
                    + direction
                    * state.Speed
                    * math.clamp(dt, 0, 0.05)

                if mode == "Speed" then
                    local hit = runtime.AnchorMovementRaycastGround(desiredPos, char)
                    if hit then
                        desiredPos = Vector3.new(
                            desiredPos.X,
                            hit.Position.Y + (state.GroundOffset or 3),
                            desiredPos.Z
                        )
                    else
                        -- Si no hay suelo debajo, no avanza hacia un vacío enorme.
                        desiredPos = Vector3.new(
                            desiredPos.X,
                            currentPos.Y,
                            desiredPos.Z
                ...
                pcall(function()
                    ...
                    if mode == "Fly" and cam then
                        -- Copia directamente la rotación de cámara. Evita la degeneración
                        -- de lookAt cuando la cámara apunta casi verticalmente.
                        nextCF =
                            CFrame.new(desiredPos)
                            * (cam.CFrame - cam.CFrame.Position)
                    else
                        local facing =
                            cam and cam.CFrame.LookVector
                            or currentPivot.LookVector
...
-- Botones ↑ / ↓ para móvil. También funcionan con mouse.
...
runtime.AnchorFlyUpButton = runtime.CreateAnchorFlyButton("↑", 0)
runtime.AnchorFlyDownButton = runtime.CreateAnchorFlyButton("↓", 68)

runtime.Track(runtime.AnchorFlyUpButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch
        or input.UserInputType == Enum.UserInputType.MouseButton1 then
        runtime.AnchorMovement.UpHeld = true
    end
end))
runtime.Track(runtime.AnchorFlyUpButton.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch
        or input.UserInputType == Enum.UserInputType.MouseButton1 then
...
runtime.Track(runtime.AnchorFlyDownButton.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch
        or input.UserInputType == Enum.UserInputType.MouseButton1 then
        runtime.AnchorMovement.DownHeld = true
    end
end))
runtime.Track(runtime.AnchorFlyDownButton.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.Touch
        or input.UserInputType == Enum.UserInputType.MouseButton1 then
...
Tabs.Mov:Section({Title = "Anchor Movement"})

UIElements.SliAnchorSpeed = Tabs.Mov:Slider({
    Title = "Velocidad Anchor",
    Desc = "Velocidad usada por Anchor Speed y Anchor Fly.",
    Step = 1,
    Value = {Min = 10, Max = 150, Default = 55},
    Callback = function(value)
        runtime.AnchorMovement.Speed = tonumber(value) or 55
    end
})

UIElements.TogAnchorSpeed = Tabs.Mov:Toggle({
    Title = "Anchor Speed",
    Desc = "Te mantiene anclado y mueve el personaje por PivotTo siguiendo el suelo.",
    Value = false,
    Callback = function(enabled)
        if enabled then
            if runtime.AnchorMovement.Mode == "Fly" and UIElements.TogAnchorFly then
...
UIElements.TogAnchorFly = Tabs.Mov:Toggle({
    Title = "Anchor Fly",
    Desc = "Fly anclado relativo a cámara. PC: Espacio/E sube, Ctrl/Q baja.",
    Value = false,
    Callback = function(enabled)
        if enabled then
            if runtime.AnchorMovement.Mode == "Speed" and UIElements.TogAnchorSpeed then
                runtime.AnchorMovement.Switching = true
                pcall(function()
                    UIElements.TogAnchorSpeed:Set(false)
                end)
...
runtime.AnchorMovementCleanup = function()
    ...
    if runtime.AnchorFlyGui and runtime.AnchorFlyGui.Parent then
        pcall(function()
