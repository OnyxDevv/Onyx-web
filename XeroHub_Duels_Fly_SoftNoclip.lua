...
function runtime.Cleanup()
    ...
    if runtime.KillAllGunCleanup then pcall(runtime.KillAllGunCleanup) end
    if runtime.AnchorMovementCleanup then pcall(runtime.AnchorMovementCleanup) end
    if runtime.SoftNoclipCleanup then pcall(runtime.SoftNoclipCleanup) end
...
-- Kill All Knife / Gun conservado como referencia interna, pero desactivado.
if false then
...
local function obtenerAttackCFKillAll(
    ...
    -- Posición exacta debajo del enemigo.
    local attackPos = (
        enemyHrp.CFrame
        * CFrame.new(
            0,
    ...
    -- Miramos desde abajo hacia la zona baja/central del enemigo.
    -- CFrame.lookAt hace que el LookVector REAL del personaje apunte hacia arriba,
    -- evitando el problema de +90/-90 donde quedaba acostado mirando al piso.
    local aimPos =
        enemyHrp.Position
        + Vector3.new(
            0,
    ...
    -- Como la mirada es casi vertical, usamos una referencia horizontal del enemigo
    -- para estabilizar el roll y evitar que el avatar se voltee de lado aleatoriamente.
    local upReference =
        enemyHrp.CFrame.LookVector

    if math.abs(
        direction:Dot(upReference)
    ) > 0.94 then
        upReference =
            enemyHrp.CFrame.RightVector
    end
...
UIElements.TogKillAll =
    ...
    Callback =
        ...
        if Value then
            ...
            -- Knife y Gun no pueden controlar el Character al mismo tiempo.
            if runtime.KillAllGunState
                and runtime.KillAllGunState.Enabled
                and UIElements.TogKillAllGun
            then
                runtime.KillAllGunState.SwitchingMode = true
                pcall(function()
                    UIElements.TogKillAllGun:Set(false)
                end)
                runtime.KillAllGunState.SwitchingMode = false
                task.wait()
            end
...
-- ==========================================
-- XEROHUB | KILL ALL GUN
--
-- Reutiliza:
--   * detección de enemigos / zonas seguras
--   * obtenerAttackCFKillAll() para ponerse debajo
--   * obtenerPistola() / esLaPistola()
--   * aimHookState para redirigir el disparo al objetivo exacto
--
-- Flujo:
--   enemigo -> equipar pistola -> TP debajo mirando arriba ->
--   seguirlo -> disparar -> confirmar muerte -> siguiente.
-- ==========================================

Tabs.KillAll:Section({
    Title = "Kill all Pistola"
})

runtime.KillAllGunState = runtime.KillAllGunState or {
    Enabled = false,
    BaseCFrame = nil,
    BaseCharacter = nil,
    WasSafe = false,
...
runtime.KillAllGunFireInterval = 0.13
runtime.KillAllGunTargetTimeout = 4.50

function runtime.KillAllGunReleaseCharacter(returnToBase)
    local state = runtime.KillAllGunState
    local char = player.Character
    ...
    if returnToBase
        and char
        and char.Parent
        and state.BaseCFrame
    then
        pcall(function()
            char:PivotTo(state.BaseCFrame)
        end)
    end
...
function runtime.KillAllGunGetTool(char, hum)
    if not char or not hum then
        return nil
    end
    ...
    for _, item in ipairs(char:GetChildren()) do
        if item:IsA("Tool") and esLaPistola(item) then
            return item
        end
    end
    ...
    if gun.Parent == char then
        -- Deja que RightGrip / scripts del Tool queden listos.
        KillRunService.Heartbeat:Wait()
        return gun
    end
...
function runtime.KillAllGunTargetPart(enemyChar)
    if not enemyChar then
        return nil
    end

    return enemyChar:FindFirstChild("Head")
        or enemyChar:FindFirstChild("UpperTorso")
        or enemyChar:FindFirstChild("Torso")
        or enemyChar:FindFirstChild("HumanoidRootPart")
end

function runtime.KillAllGunFollowTarget(myChar, myHum, myHrp, enemyHrp)
    if not myChar
        or not myChar.Parent
        or not myHum
        or not myHum.Parent
    ...
    local attackCF = obtenerAttackCFKillAll(enemyHrp)
    if not attackCF then
        return false
    end

    -- Mantiene la misma pose debajo del enemigo; LookVector apunta físicamente hacia arriba.
    pcall(function()
        myChar:PivotTo(attackCF)
...
function runtime.KillAllGunAttackTarget(targetPlayer, token)
    local state = runtime.KillAllGunState
    ...
    if not enemyChar
        ...
        or not enemyHrp
        or enemyHum.Health <= 0
        or estaEnZonaSeguraKillAll(enemyChar, targetPlayer)
    then
        return false
    end
    ...
    if not myChar or not myHum or not myHrp or myHum.Health <= 0 then
        return false
    end

    local gun = runtime.KillAllGunGetTool(myChar, myHum)
    if not gun then
        showBottomMessage("Kill All Gun: no se encontró una pistola.", {
            title = "XeroHub · Kill All",
            key = "killall:gun:nogun",
        })
        return false
    end
    ...
    -- El sonido personalizado de kill ya conoce la víctima antes de disparar.
    pcall(function()
        if runtime.ArmNativeDeathTarget then
            runtime.ArmNativeDeathTarget(enemyChar, gun)
        end
    end)
    ...
    while runtime.Alive
        ...
        and targetPlayer.Character == enemyChar
        and enemyChar.Parent
        and enemyHum.Parent
        and enemyHum.Health > 0
        and enemyHrp.Parent
        and (os.clock() - started) < runtime.KillAllGunTargetTimeout
    do
        if estaEnZonaSeguraKillAll(enemyChar, targetPlayer) then
            break
        end

        -- TP/tracking debajo del enemigo en CADA frame.
        if not runtime.KillAllGunFollowTarget(
            myChar,
            myHum,
            myHrp,
            enemyHrp
        ...
        -- Silent target explícito: el hook existente redirige Raycast/Mouse.Hit.
        local targetPart = runtime.KillAllGunTargetPart(enemyChar)
        if not targetPart then
            break
        end
        ...
        local now = os.clock()
        if now - lastFire >= runtime.KillAllGunFireInterval then
            lastFire = now
            pcall(function()
                gun:Deactivate()
                gun:Activate()
            ...
            -- No mantenemos Activate pegado indefinidamente.
            task.delay(0.025, function()
                if gun and gun.Parent == myChar then
                    pcall(function()
                        gun:Deactivate()
                    end)
                end
    ...
    local killed = enemyHum.Health <= 0
    state.Target = nil
    return killed
end

function runtime.KillAllGunRun(token)
    local state = runtime.KillAllGunState
    ...
    do
        ...
        -- Respawn: la posición actual se vuelve la nueva base.
        if state.BaseCharacter ~= myChar then
            state.BaseCharacter = myChar
        ...
        local localSafe =
            estaEnLobby()
            or estaEnZonaSeguraKillAll(myChar, player)

        if localSafe then
            runtime.KillAllGunReleaseCharacter(false)
        ...
        if state.WasSafe or not state.BaseCFrame then
            state.BaseCFrame = myHrp.CFrame
            state.BaseCharacter = myChar
            state.WasSafe = false
        end
        ...
        for _, targetPlayer in ipairs(listaJugadores) do
            if not runtime.Alive
                or not state.Enabled
                or state.LoopToken ~= token
            then
                break
            end

            if targetPlayer ~= player
                and isEnemy(targetPlayer)
                and targetPlayer.Character
            then
                local enemyChar = targetPlayer.Character
                ...
                if enemyHum
                    and enemyHrp
                    and enemyHum.Health > 0
                    and not estaEnZonaSeguraKillAll(enemyChar, targetPlayer)
                then
                    local base = state.BaseCFrame
                    local delta = base and (base.Position - enemyHrp.Position)
                    if not delta or delta:Dot(delta) <= KILL_RANGE_SQ then
                        foundTarget = true

                        if estaVulnerableKillAll(enemyChar, targetPlayer) then
                            attacked = true
                            runtime.KillAllGunAttackTarget(targetPlayer, token)

                            -- Al morir uno, el FOR continúa y busca el siguiente.
                            task.wait(0.035)
                        end
                    end
                end
        ...
        if not foundTarget then
            -- Misma idea del Knife: cuando ya limpió la ronda vuelve a la base.
            runtime.KillAllGunReleaseCharacter(true)
            task.wait(0.12)
        elseif not attacked then
            -- Hay objetivos, pero siguen protegidos/invulnerables.
            runtime.KillAllGunReleaseCharacter(true)
            task.wait(0.08)
        else
            task.wait(0.04)
...
runtime.KillAllGunCleanup = function()
    local state = runtime.KillAllGunState
    if not state then
        return
    end
    ...
    state.Enabled = false
    state.LoopToken = (state.LoopToken or 0) + 1
    runtime.KillAllGunReleaseCharacter(false)
end

UIElements.TogKillAllGun = Tabs.KillAll:Toggle({
    Title = "Activar Kill All Gun",
    Desc = "Se coloca debajo del enemigo y le dispara hasta eliminarlo.",
    Value = false,
    ...
    Callback = function(Value)
        ...
        if Value then
            -- Apaga el Kill All Knife antes de tomar control del Character.
            if killAllEnabled and UIElements.TogKillAll then
            ...
            if not char
                ...
                or hum.Health <= 0
                or estaEnLobby()
                or estaEnZonaSeguraKillAll(char, player)
            then
                state.Enabled = false

                showBottomMessage(
                    "Kill All Gun no se puede activar en zona segura.",
                    {title = "XeroHub · Kill All"}
                )

                task.defer(function()
                    pcall(function()
                        if UIElements.TogKillAllGun then
                            UIElements.TogKillAllGun:Set(false)
                        end
                    end)
                end)
            ...
            showBottomMessage(
                "Kill All Gun: ACTIVADO",
                {title = "XeroHub · Kill All", key = "killall:gun"}
            )

            task.spawn(function()
                runtime.KillAllGunRun(token)
            end)
        else
            state.Enabled = false
            state.LoopToken = (state.LoopToken or 0) + 1
            ...
            if not state.SwitchingMode then
                showBottomMessage(
                    "Kill All Gun: DESACTIVADO",
                    {title = "XeroHub · Kill All", key = "killall:gun"}
                )
            end
        end
...
-- ==========================================
-- SOFT ANCHOR FLY | CÁMARA + PERSISTENCIA ENTRE RONDAS
--
-- Sin Speed y sin botones ↑/↓.
-- La dirección COMPLETA de la cámara controla el vuelo:
-- mira arriba + avanza = sube, mira abajo + avanza = baja.
--
-- DesiredEnabled permanece true entre respawns/rondas. Si Roblox reemplaza
-- Character/HRP, el supervisor vuelve a montar automáticamente los constraints.
-- ==========================================

runtime.AnchorMovement = runtime.AnchorMovement or {
    DesiredEnabled = false,
    Mode = nil,
    Speed = 55,
    Character = nil,
    ...
    Rebinding = false,
    LastBindAttempt = 0,
    SupervisorAccumulator = 0,
}

function runtime.AnchorMovementGetMoveVector()
    local move = Vector3.zero

    -- PlayerModule cubre joystick móvil, teclado y gamepad.
...
function runtime.AnchorMovementWorldDirection()
    local move = runtime.AnchorMovementGetMoveVector()
    local cam = workspace.CurrentCamera

    if not cam or move.Magnitude < 0.001 then
        return Vector3.zero
    end

    -- El LookVector conserva su componente Y:
    -- mirar arriba/abajo controla la altura simplemente avanzando/retrocediendo.
    local world =
        cam.CFrame.RightVector * move.X
        + cam.CFrame.LookVector * (-move.Z)

    if world.Magnitude > 1 then
        world = world.Unit
    end
...
function runtime.AnchorMovementDestroyPhysics()
    local state = runtime.AnchorMovement

    if state.PhysicsConnection then
        pcall(function()
            state.PhysicsConnection:Disconnect()
        end)
        state.PhysicsConnection = nil
    end
...
function runtime.AnchorMovementRestore(keepDesired)
    local state = runtime.AnchorMovement

    runtime.AnchorMovementDestroyPhysics()
    ...
    if root and root.Parent then
        pcall(function()
            root.Anchored = state.OriginalAnchored == true
    ...
    if hum and hum.Parent then
        pcall(function()
            hum.AutoRotate = state.OriginalAutoRotate ~= false
            hum.PlatformStand = state.OriginalPlatformStand == true
        end)
    end
    ...
    if not keepDesired then
        state.DesiredEnabled = false
    end
end

function runtime.AnchorMovementCreatePhysics(root)
    local state = runtime.AnchorMovement

    local attachment = Instance.new("Attachment")
    attachment.Name = "XeroSoftFlyAttachment"
    attachment.Parent = root

    local velocity = Instance.new("LinearVelocity")
    velocity.Name = "XeroSoftFlyVelocity"
    ...
    orientation.Name = "XeroSoftFlyOrientation"
    orientation.Attachment0 = attachment
    orientation.Mode = Enum.OrientationAlignmentMode.OneAttachment
    orientation.RigidityEnabled = false
    orientation.Responsiveness = 120
    orientation.MaxTorque = math.huge
    orientation.CFrame = root.CFrame - root.Position
    orientation.Parent = root

    state.Attachment = attachment
    state.LinearVelocity = velocity
    state.AlignOrientation = orientation

    return velocity, orientation
end

function runtime.AnchorMovementBindCharacter()
    local state = runtime.AnchorMovement

    if not state.DesiredEnabled
        or state.Rebinding
    then
        return false
    end
    ...
    local now = os.clock()
    if now - (state.LastBindAttempt or 0) < 0.08 then
        return false
    end
    state.LastBindAttempt = now
    state.Rebinding = true

    -- Limpia el rig anterior SIN apagar la intención de mantener Fly activo.
    runtime.AnchorMovementRestore(true)
    ...
    -- Fantasma también escribe movimiento; evita que ambos motores se peleen.
    if UIElements.TogGhostMode then
        state.Switching = true

        pcall(function()
            UIElements.TogGhostMode:Set(false)
        end)
    ...
    -- Revalida porque el pequeño wait anterior puede coincidir con un respawn.
    ...
    state.Humanoid = hum
    state.Root = root
    state.OriginalAnchored = root.Anchored
    state.OriginalAutoRotate = hum.AutoRotate
    state.OriginalPlatformStand = hum.PlatformStand
    ...
    local velocity, orientation =
        runtime.AnchorMovementCreatePhysics(root)

    if not velocity or not orientation then
        runtime.AnchorMovementRestore(true)
        state.Rebinding = false
        return false
    end

    state.PhysicsConnection = runtime.Track(
        RunService.Heartbeat:Connect(function()
            -- Si se apagó manualmente, limpiar por completo.
            if not runtime.Alive
                or not state.DesiredEnabled
            then
                runtime.AnchorMovementRestore(false)
                return
            end

            -- Nueva ronda / respawn / reemplazo de HRP:
            -- soltamos el rig viejo pero CONSERVAMOS DesiredEnabled.
            if state.Character ~= player.Character
                or not char.Parent
                or not hum.Parent
                or hum.Health <= 0
                or not root.Parent
                or not velocity.Parent
                or not orientation.Parent
            then
                runtime.AnchorMovementRestore(true)
                return
            end
            ...
            -- Sin input queda clavado en el aire.
            velocity.VectorVelocity =
                runtime.AnchorMovementWorldDirection()
                * state.Speed

            local cam = workspace.CurrentCamera
            if cam then
                -- El avatar adopta la orientación completa de cámara.
                orientation.CFrame =
...
Tabs.Mov:Section({
    Title = "Soft Anchor Fly"
})

UIElements.SliAnchorSpeed = Tabs.Mov:Slider({
    Title = "Velocidad Fly",
    Desc = "Velocidad del vuelo físico.",
    Step = 1,
    Value = {
        Min = 10,
        Max = 150,
    ...
    Callback = function(value)
        runtime.AnchorMovement.Speed =
            tonumber(value) or 55
    end
})

UIElements.TogAnchorFly = Tabs.Mov:Toggle({
    Title = "Soft Anchor Fly",
    Desc = "Mira arriba/abajo y avanza para subir/bajar. Permanece activo entre rondas.",
    Value = false,
    ...
    Callback = function(enabled)
        ...
        if enabled then
            ...
            -- Si estamos entre rondas puede no existir todavía el nuevo HRP.
            -- Dejamos el toggle ON y el supervisor lo engancha apenas aparezca.
            runtime.AnchorMovementBindCharacter()
        else
            state.DesiredEnabled = false
            runtime.AnchorMovementRestore(false)
        end
...
-- no toca el toggle. Sólo vuelve a enlazar el Fly si una ronda reemplazó el Character.
runtime.AnchorMovementSupervisor =
    runtime.Track(
        RunService.Heartbeat:Connect(function(dt)
            ...
            state.SupervisorAccumulator =
                (state.SupervisorAccumulator or 0) + dt

            if state.SupervisorAccumulator < 0.18 then
                return
            end

            state.SupervisorAccumulator = 0
            ...
            if char
                ...
                and (
                    state.Character ~= char
                    or state.Root ~= root
                    or not state.LinearVelocity
                    or not state.LinearVelocity.Parent
                    or not state.AlignOrientation
                    or not state.AlignOrientation.Parent
                )
            then
                task.defer(runtime.AnchorMovementBindCharacter)
            end
        end)
    )

-- Acelera el reenganche al respawn, sin desactivar el toggle.
runtime.Track(
    player.CharacterAdded:Connect(function(char)
        if not runtime.AnchorMovement.DesiredEnabled then
            return
        end
        ...
        task.spawn(function()
            ...
            if runtime.AnchorMovement.DesiredEnabled
                and hum
                and root
                and hum.Health > 0
            then
...
runtime.AnchorMovementCleanup = function()
    runtime.AnchorMovement.DesiredEnabled = false
    runtime.AnchorMovementRestore(false)
end


-- ==========================================
-- SOFT NOCLIP | NoCollisionConstraint DINÁMICO
--
-- No toca CanCollide globalmente.
-- Sólo crea constraints temporales entre el Character y obstáculos del mapa
-- que están pegados / delante de la dirección en la que intentas moverte.
-- Al salir del obstáculo, los constraints se destruyen automáticamente.
-- ==========================================

runtime.SoftNoclip = runtime.SoftNoclip or {
    DesiredEnabled = false,
    Character = nil,
    Humanoid = nil,
    Root = nil,
    CharacterParts = {},
    ActiveParts = setmetatable({}, {__mode = "k"}),
    ScanAccumulator = 0,
    ScanInterval = 0.045,
    ReleaseAfter = 0.18,
    BoxSize = Vector3.new(5.2, 6.5, 5.2),
}

function runtime.SoftNoclipDestroyEntry(worldPart)
    local state = runtime.SoftNoclip
    local entry = state.ActiveParts[worldPart]
    if not entry then
        return
    end

    if entry.Constraints then
        for i = #entry.Constraints, 1, -1 do
            local constraint = entry.Constraints[i]
            if constraint then
                pcall(function()
                    constraint:Destroy()
                end)
            end
            entry.Constraints[i] = nil
        end
    end

    state.ActiveParts[worldPart] = nil
end

function runtime.SoftNoclipClearConstraints()
    local state = runtime.SoftNoclip

    local parts = {}
    for worldPart in pairs(state.ActiveParts) do
        parts[#parts + 1] = worldPart
    end

    for i = 1, #parts do
        runtime.SoftNoclipDestroyEntry(parts[i])
    end
...
function runtime.SoftNoclipRefreshCharacter()
    local state = runtime.SoftNoclip
    local char = player.Character
    ...
    runtime.SoftNoclipClearConstraints()
    ...
    for _, object in ipairs(char:GetDescendants()) do
        if object:IsA("BasePart") then
            state.CharacterParts[#state.CharacterParts + 1] =
                object
        end
    end
...
function runtime.SoftNoclipIsOtherCharacterPart(part)
    if not part then
        return false
    end
...
function runtime.SoftNoclipGetDirection()
    local state = runtime.SoftNoclip

    -- Si Soft Fly está activo, usamos exactamente la misma dirección física.
    if runtime.AnchorMovement
        and runtime.AnchorMovement.DesiredEnabled
        and type(runtime.AnchorMovementWorldDirection) == "function"
    then
        local ok, dir =
            pcall(runtime.AnchorMovementWorldDirection)
    ...
    -- Standalone: movimiento normal del Humanoid.
    local hum = state.Humanoid
    if hum
        and hum.MoveDirection.Magnitude > 0.01
    then
...
function runtime.SoftNoclipCreateEntry(worldPart)
    local state = runtime.SoftNoclip

    if not worldPart
        or not worldPart.Parent
        or not worldPart:IsA("BasePart")
        or runtime.SoftNoclipIsOtherCharacterPart(worldPart)
    then
        return nil
    end

    local existing =
        state.ActiveParts[worldPart]
    ...
    for i = 1, #state.CharacterParts do
        local bodyPart = state.CharacterParts[i]

        if bodyPart
            and bodyPart.Parent
            and bodyPart:IsA("BasePart")
            and bodyPart ~= worldPart
        then
            local noCollision =
                Instance.new("NoCollisionConstraint")

            noCollision.Name =
                "XeroSoftNoclipConstraint"

            noCollision.Part0 = bodyPart
            noCollision.Part1 = worldPart
            noCollision.Parent = bodyPart
    ...
    state.ActiveParts[worldPart] = entry
    return entry
end

function runtime.SoftNoclipScan()
    local state = runtime.SoftNoclip

    if not runtime.SoftNoclipRefreshCharacter() then
        return
    end
    ...
    local direction =
        runtime.SoftNoclipGetDirection()

    local now = os.clock()

    -- No input: no abrimos nuevas colisiones.
    -- Las ya activas se mantienen unos milisegundos para terminar de salir de la pared.
    if direction.Magnitude > 0.01 then
        local overlap =
            OverlapParams.new()
        ...
        -- Caja ligeramente adelantada hacia donde te estás moviendo.
        local boxCF =
            root.CFrame
            + direction * 1.15
        ...
        for i = 1, #nearby do
            ...
            if part
                ...
                and part.CanCollide
                and not runtime.SoftNoclipIsOtherCharacterPart(part)
            then
                local offset =
                ...
                if offset.Magnitude < 0.05 then
                    ...
                    local dot =
                        offset.Unit:Dot(direction)

                    -- Adelante o ya prácticamente encima del cuerpo.
                    shouldPhase =
                        dot > -0.20
                        or state.ActiveParts[part] ~= nil
                end

                if shouldPhase then
                    local entry =
                        runtime.SoftNoclipCreateEntry(part)
        ...
        -- Raycast frontal adicional: arma la pared ANTES de que el cuerpo choque.
        local rayParams =
            RaycastParams.new()
        ...
        if hit
            ...
            and not runtime.SoftNoclipIsOtherCharacterPart(hit.Instance)
        then
            local entry =
                runtime.SoftNoclipCreateEntry(hit.Instance)
    ...
    -- Liberar obstáculos que ya quedaron atrás/fuera del cuerpo.
    local expired = {}

    for worldPart, entry in pairs(state.ActiveParts) do
        if not worldPart
            or not worldPart.Parent
            or not entry
            or now - (entry.LastSeen or 0) > state.ReleaseAfter
        then
            expired[#expired + 1] =
                worldPart
        end
    ...
    for i = 1, #expired do
        runtime.SoftNoclipDestroyEntry(expired[i])
    end
end
...
UIElements.TogSoftNoclip = Tabs.Mov:Toggle({
    Title = "Soft Noclip",
    Desc = "Abre colisión sólo contra paredes/obstáculos cercanos mientras los atraviesas.",
    Value = false,

    Callback = function(enabled)
        local state = runtime.SoftNoclip
        state.DesiredEnabled = enabled == true

        if enabled then
            runtime.SoftNoclipRefreshCharacter()
        else
            runtime.SoftNoclipClearConstraints()
            state.Character = nil
            state.Humanoid = nil
...
runtime.SoftNoclipConnection =
    runtime.Track(
        RunService.Heartbeat:Connect(function(dt)
            local state = runtime.SoftNoclip

            if not state.DesiredEnabled then
                return
            end

            state.ScanAccumulator =
                (state.ScanAccumulator or 0) + dt

            if state.ScanAccumulator
                < state.ScanInterval
            then
                return
            end

            state.ScanAccumulator = 0
            runtime.SoftNoclipScan()
        end)
    )

runtime.Track(
    player.CharacterAdded:Connect(function()
        if not runtime.SoftNoclip.DesiredEnabled then
            return
        end

        runtime.SoftNoclipClearConstraints()
        runtime.SoftNoclip.Character = nil
        runtime.SoftNoclip.Humanoid = nil
        runtime.SoftNoclip.Root = nil
        runtime.SoftNoclip.CharacterParts = {}

        task.delay(0.20, function()
            if runtime.Alive
                and runtime.SoftNoclip.DesiredEnabled
            then
                runtime.SoftNoclipRefreshCharacter()
            end
        end)
...
runtime.SoftNoclipCleanup = function()
    runtime.SoftNoclip.DesiredEnabled = false
    runtime.SoftNoclipClearConstraints()
