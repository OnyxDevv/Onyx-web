...
-- ==========================================
-- PHASE FLY | FLY + NOCLIP EN UN SOLO MOTOR
--
-- No usa suelo/raycast de piso.
-- No hay toggle de Noclip separado.
-- Mientras Phase Fly esté ON:
--   * LinearVelocity controla todo el movimiento.
--   * La cámara controla subida/bajada.
--   * NoCollisionConstraint se mantiene activo alrededor del cuerpo SIEMPRE,
--     incluso sin input o mientras ya estás metido dentro de una pared.
--   * El Humanoid se mantiene en Freefall localmente para que el controlador
--     no dependa del estado de suelo.
-- ==========================================
...
runtime.AnchorMovement = runtime.AnchorMovement or {
    ...
    CharacterParts = {},
    PhaseParts = setmetatable({}, {__mode = "k"}),
    PhaseScanAccumulator = 0,
    PhaseScanInterval = 0.035,
    PhaseReleaseAfter = 0.30,
    PhaseBoxSize = Vector3.new(5.6, 7.4, 5.6),
    LastFreefallAt = 0,

    -- Auto-reparación del noclip.
    LastCharacterPartsRefresh = 0,
    CharacterPartsRefreshInterval = 0.55,
    LastConstraintAudit = 0,
    ConstraintAuditInterval = 0.10,
}
...
function runtime.AnchorMovementWorldDirection()
    ...
    -- Pitch completo de cámara:
    -- mirar arriba + avanzar = subir
    -- mirar abajo + avanzar = bajar
    local world =
...
function runtime.PhaseFlyIsOtherCharacterPart(part)
    if not part then
        return false
    end
...
function runtime.PhaseFlyDestroyPart(worldPart)
    local state = runtime.AnchorMovement
    local entry = state.PhaseParts[worldPart]
    ...
    state.PhaseParts[worldPart] = nil
end

function runtime.PhaseFlyClearConstraints()
    local state = runtime.AnchorMovement
    local pending = {}

    for worldPart in pairs(state.PhaseParts) do
        pending[#pending + 1] = worldPart
    end

    for i = 1, #pending do
        runtime.PhaseFlyDestroyPart(pending[i])
    end
...
function runtime.PhaseFlyRefreshCharacterParts()
    local state = runtime.AnchorMovement
    ...
    state.LastCharacterPartsRefresh = os.clock()
end

function runtime.PhaseFlyValidCharacterPartCount()
    local state = runtime.AnchorMovement
    local count = 0
...
function runtime.PhaseFlyEntryHealthy(worldPart, entry)
    local state = runtime.AnchorMovement

    if not worldPart
        or not worldPart.Parent
        or not entry
        or type(entry.Constraints) ~= "table"
        or type(entry.ByBodyPart) ~= "table"
    then
        return false
    end

    local validBodyParts = 0
    local validConstraints = 0
    ...
    for i = 1, #state.CharacterParts do
        ...
        then
            validBodyParts = validBodyParts + 1

            local constraint =
                entry.ByBodyPart[bodyPart]

            if constraint
                and constraint.Parent
                and constraint:IsA("NoCollisionConstraint")
                and constraint.Part0 == bodyPart
                and constraint.Part1 == worldPart
            then
                validConstraints = validConstraints + 1
            else
                -- Una sola pieza faltante basta para considerar la entrada rota.
                return false
            end
        end
    end
    ...
    return validBodyParts > 0
        and validConstraints == validBodyParts
end

function runtime.PhaseFlyCreatePart(worldPart)
    local state = runtime.AnchorMovement

    if not worldPart
        or not worldPart.Parent
        or not worldPart:IsA("BasePart")
        or runtime.PhaseFlyIsOtherCharacterPart(worldPart)
    then
        return nil
    end

    local entry =
        state.PhaseParts[worldPart]

    if entry then
        if runtime.PhaseFlyEntryHealthy(
            worldPart,
            entry
        ) then
        ...
        -- La tabla decía que estaba activo, pero Roblox destruyó/invalidó
        -- uno o más constraints. Limpiamos y reconstruimos AHORA MISMO.
        runtime.PhaseFlyDestroyPart(worldPart)
    end
    ...
    for i = 1, #state.CharacterParts do
        ...
        then
            ...
            constraint.Name =
                "XeroPhaseFlyConstraint"

            constraint.Part0 = bodyPart
            constraint.Part1 = worldPart
    ...
    state.PhaseParts[worldPart] = entry
    return entry
end

function runtime.PhaseFlyAuditConstraints()
    local state = runtime.AnchorMovement
    local now = os.clock()

    if now - (state.LastConstraintAudit or 0)
        < state.ConstraintAuditInterval
    then
        return
    end

    state.LastConstraintAudit = now

    -- Si el juego añadió/quitó partes del rig sin reemplazar Character,
    -- refrescamos la lista y después reconstruimos las paredes activas.
    if now - (state.LastCharacterPartsRefresh or 0)
        >= state.CharacterPartsRefreshInterval
    then
        runtime.PhaseFlyRefreshCharacterParts()
    end

    local broken = {}

    for worldPart, entry in pairs(state.PhaseParts) do
        if worldPart
            and worldPart.Parent
            and entry
            and not runtime.PhaseFlyEntryHealthy(
                worldPart,
                entry
            )
        then
    ...
    for i = 1, #broken do
        local worldPart = broken[i]

        runtime.PhaseFlyDestroyPart(
            worldPart
        )

        -- Si sigue cerca, el mismo scan o este rebuild la reactiva
        -- sin necesidad de tocar el toggle.
        if worldPart
            and worldPart.Parent
            and worldPart:IsA("BasePart")
            and worldPart.CanCollide
        then
            runtime.PhaseFlyCreatePart(
                worldPart
            )
        end
    end
...
function runtime.PhaseFlyScan(direction)
    local state = runtime.AnchorMovement
    local char = state.Character
    ...
    -- Watchdog del noclip: repara constraints destruidos/huérfanos
    -- aunque el toggle nunca haya cambiado.
    runtime.PhaseFlyAuditConstraints()
    ...
    -- IMPORTANTE:
    -- se refrescan piezas cercanas AUN SIN INPUT.
    -- Esto corrige el bug donde el toggle seguía ON pero los constraints expiraban
    -- dentro de una pared al dejar de mover el joystick/teclas.
    for i = 1, #nearby do
        local part = nearby[i]
        ...
        if part
            ...
            and part.CanCollide
            and not runtime.PhaseFlyIsOtherCharacterPart(part)
        then
            local entry =
                runtime.PhaseFlyCreatePart(part)
    ...
    -- Pre-armar lo que viene enfrente para atravesarlo sin el golpe inicial.
    if direction
    ...
    then
        ...
        if hit
            ...
            and not runtime.PhaseFlyIsOtherCharacterPart(hit.Instance)
        then
            local entry =
                runtime.PhaseFlyCreatePart(
                    hit.Instance
    ...
    for worldPart, entry in pairs(state.PhaseParts) do
        if not worldPart
            or not worldPart.Parent
            or not entry
            or now - (entry.LastSeen or 0)
                > state.PhaseReleaseAfter
        then
            expired[#expired + 1] =
                worldPart
        end
...
function runtime.AnchorMovementRestore(keepDesired)
    ...
    runtime.PhaseFlyClearConstraints()
    ...
    if hum and hum.Parent then
        pcall(function()
            hum.AutoRotate =
                state.OriginalAutoRotate ~= false

            hum.PlatformStand =
                state.OriginalPlatformStand == true
        end)
    end
...
function runtime.AnchorMovementBindCharacter()
    ...
    if now - (state.LastBindAttempt or 0)
        < 0.08
    then
        return false
    end
    ...
    state.OriginalAnchored =
        root.Anchored

    state.OriginalAutoRotate =
        hum.AutoRotate

    state.OriginalPlatformStand =
        hum.PlatformStand
    ...
    state.PhysicsConnection =
        runtime.Track(
            RunService.Heartbeat:Connect(function(dt)
                ...
                -- Ronda nueva / Character nuevo:
                -- conserva DesiredEnabled; supervisor reengancha.
                ...
                -- No dependemos del suelo. El Humanoid se mantiene localmente
                -- en estado aéreo mientras el constraint sostiene la posición.
                local clockNow = os.clock()

                if clockNow - (state.LastFreefallAt or 0)
                    >= 0.075
                then
                    state.LastFreefallAt =
                        clockNow
                ...
                state.PhaseScanAccumulator =
                    (state.PhaseScanAccumulator or 0)
                    + dt

                if state.PhaseScanAccumulator
                    >= state.PhaseScanInterval
                then
                    state.PhaseScanAccumulator = 0

                    runtime.PhaseFlyScan(
                        direction
                    )
                end
            end)
...
UIElements.TogAnchorFly = Tabs.Mov:Toggle({
    Title = "Phase Fly",
    Desc = "Fly + noclip dinámico en un solo modo. La cámara controla la altura.",
    Value = false,

    Callback = function(enabled)
        local state =
            runtime.AnchorMovement
    ...
    runtime.Track(
        RunService.Heartbeat:Connect(function(dt)
            ...
            state.SupervisorAccumulator =
                (state.SupervisorAccumulator or 0)
                + dt

            if state.SupervisorAccumulator
                < 0.18
            then
                return
            end
