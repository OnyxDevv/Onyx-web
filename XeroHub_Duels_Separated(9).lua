...
local function bindGunshot(sound)
    if not soundState.Enabled or soundState.Originals[sound] ~= nil or not isTargetGunshot(sound) then
        return false
    end
    ...
    table.insert(connections, sound:GetPropertyChangedSignal("Volume"):Connect(function()
        if soundState.Enabled and sound.Parent and not soundState.Muting[sound] and sound.Volume ~= 0 then
            soundState.Muting[sound] = true
            pcall(function() sound.Volume = 0 end)
            soundState.Muting[sound] = nil
        end
...
local function watchCandidateSound(sound)
    ...
    local function retry()
        if not runtime.Alive or not soundState.Enabled or not sound.Parent then return end
        bindGunshot(sound)
    end
...
runtime.Track(game.DescendantAdded:Connect(function(object)
    if not soundState.Enabled or not object:IsA("Sound") then return end
    watchCandidateSound(object)
end))
