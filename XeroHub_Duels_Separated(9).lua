...
-- ==========================================
local ZONAS_SEGURAS = {
    {Centro = Vector3.new(-320.50, 280.82, 16.00), Radio = 500}, -- Lobby principal
    {Centro = Vector3.new(1564.14, -155.45, 40.04), Radio = 300}  -- Zona de votación de mapa
}
...
local function activateEntry(entry, changedSelection)
    if not entry then
        soundState.DesiredEnabled = false
...
local function isOwnedByLocalPlayer(sound)
...
local function classifyKillSound(sound)
    ...
    -- Confirmado en DUELS: Workspace.<tu usuario>.DefaultGun.Handle.GunKill
    if (digits == KILL_GUN_SOUND_ID or name == "gunkill") and isOwnedByLocalPlayer(sound) then
        return "gunkill"
    end

    -- Fallback de Roblox/DUELS para cuchillo u otras muertes.
...
runtime.ArmNativeDeathTarget = function(character, tool)
    -- Hook para sistemas que YA conocen la víctima exacta (p. ej. Kill All).
    return armNativeDeathTarget(character, tool, true)
end
...
local function playKillReplacement(originSound, sourceType)
    if not killSoundState.Enabled or sourceType ~= "death" then return false end

    -- Si este Died ya fue parcheado ANTES de la muerte, dejamos que Roblox lo
    -- reproduzca de forma nativa. No lo silenciamos ni lanzamos otro Sound.
    if originSound and killSoundState.NativeDeathOriginalIds[originSound] ~= nil then
        return false
    end
    ...
    local now = os.clock()
    if now - (killSoundState.RecentLocalAttack or 0) > KILL_FALLBACK_ATTACK_WINDOW then
        return false
    end

    -- Si el último ataque salió de una Tool con GunKill, NO usamos Died como
    -- fallback: dejamos que el juego reproduzca el GunKill ya parcheado nativamente.
    if toolHasNativeGunKill(killSoundState.RecentAttackTool) then
        return false
    end
    ...
    local lastForSound = originSound and (killSoundState.LastTrigger[originSound] or 0) or 0
    if originSound and now - lastForSound < 0.04 then return true end
    if originSound then killSoundState.LastTrigger[originSound] = now end

    killSoundState.LastKillReplacementAt = now
    pcall(function()
        replacement:Stop()
        replacement.TimePosition = 0
        replacement:Play()
...
local function bindKillSound(sound)
    ...
    if killSoundState.Bindings[sound] then
        if kind == "gunkill" then applyNativeGunKill(sound) end
        return true
    end
    ...
    if kind == "gunkill" then
        ...
        -- Algunos juegos vuelven a escribir SoundId al reequipar. Lo reponemos antes
        -- de la siguiente kill, sin reproducir nada manualmente.
        table.insert(connections, sound:GetPropertyChangedSignal("SoundId"):Connect(function()
            if killSoundState.Enabled and sound.Parent and not killSoundState.NativeUpdating[sound] then
                applyNativeGunKill(sound)
            end
        end))

        table.insert(connections, sound.Played:Connect(function()
            killSoundState.LastConfirmedKillTrigger = os.clock()
        end))
    else
        table.insert(connections, sound.Played:Connect(function()
            playKillReplacement(sound, kind)
        end))
        table.insert(connections, sound:GetPropertyChangedSignal("Playing"):Connect(function()
            if sound.Playing then playKillReplacement(sound, kind) end
        end))
    end
    ...
    -- Si Died ya estaba sonando al engancharlo, aún puede servir como fallback.
    if kind == "death" and (sound.IsPlaying or sound.Playing) then
        playKillReplacement(sound, kind)
    end
    return true
end
...
local function startLocalAttackMonitoring()
    ...
    bindAttackRoot(player.Character)
    bindAttackRoot(player:FindFirstChildOfClass("Backpack"))
    local connection = player.CharacterAdded:Connect(function(character)
        if not killSoundState.Enabled then return end
        task.defer(function()
            if killSoundState.Enabled then
                bindAttackRoot(character)
                bindAttackRoot(player:FindFirstChildOfClass("Backpack"))
                scanKillSounds()
            end
        end)
...
local function activateKillEntry(entry, changedSelection)
    if not entry then
        killSoundState.DesiredEnabled = false
    ...
    task.spawn(function()
        ...
        -- Si había una víctima armada mientras cambiaste de dropdown, actualizamos
        -- ese mismo Died al asset nuevo sin esperar a la siguiente kill.
        for sound in pairs(killSoundState.NativeDeathOriginalIds) do
            applyNativeDeathSound(sound)
        end

        startLocalAttackMonitoring()
        local linked = scanKillSounds()
        if previous and previous ~= replacement then
            pcall(function() previous:Destroy() end)
        end
        ...
        showBottomMessage(
            (changedSelection and "Sonido de muerte cambiado a " or "Sonido de muerte activado: ")
                .. entry.label .. " · " .. tostring(linked) .. " objetivo(s) enlazado(s).",
...
runtime.Track(game.DescendantAdded:Connect(function(object)
...
function estaEnLobby()
    ...
    if hrp then
        ...
        for i = 1, #ZONAS_SEGURAS do
            if (myPos - ZONAS_SEGURAS[i].Centro).Magnitude <= ZONAS_SEGURAS[i].Radio then 
                return true
            end
        end
    end
...
function runtime.AvatarCloneBuildMotorSync(char, overlay)
    ...
    local function syncPose()
        ...
        -- state.MotorPairs ya está en orden raíz -> extremidades.
                ...
                pcall(function()
                    if pair.R6Fallback then
                        local currentRelative =
                            realParent.CFrame:ToObjectSpace(realChild.CFrame)

                        local delta =
                            pair.InitialRealRelativeInverse * currentRelative

                        cloneChild.CFrame =
                            cloneParent.CFrame
                            * pair.InitialCloneRelative
                            * delta
                    else
                        local poseTransform

                        if pair.RealC0 and pair.RealC1 then
                            -- Si el Character conserva RigAttachments, ésta es
                            -- la lectura más fiel de la pose visible actual.
                            local parentJointWorld =
                                realParent.CFrame * pair.RealC0
                            local childJointWorld =
                                realChild.CFrame * pair.RealC1

                            poseTransform =
                                parentJointWorld:ToObjectSpace(childJointWorld)
                        else
                            -- El juego quitó sus attachments: usamos pivotes
                            -- estándar del avatar local + CFrames visibles.
                            poseTransform =
                                pair.DriverC0Inverse
                                * realParent.CFrame:Inverse()
                                * realChild.CFrame
                                * pair.DriverC1
                        end

                        -- Ecuación de joint aplicada a LOS PIVOTES DEL CLON:
...
function runtime.BeginAvatarCloneRespawnMask(char, generation)
    ...
    task.spawn(function()
        ...
        state.RespawnMaskConnection = RunService.RenderStepped:Connect(function()
            ...
            -- Hot path: sin GetDescendants(), sin FindFirstChild() por joint y sin
            -- construir tablas. Sólo leemos la pose cacheada y actualizamos el clon.
            for i = 1, #posePairs do
                local pair = posePairs[i]
...
local function enemigoEnLobby(enemyChar, enemyHrp)
    ...
    if eHrp then
        for _, zona in ipairs(ZONAS_SEGURAS) do
            ...
            if delta:Dot(delta) <= (zona.Radio * zona.Radio) then return true end
        end
    end
    return false
end
                                ...
                                if enemyChar and isEnemy(p) then
                                    ...
                                    if enemyHum and enemyHum.Health > 0 and enemyDelta and enemyDelta:Dot(enemyDelta) <= 640000 then
                                            ...
                                            for j = 1, #mState.scAS do
                                                ...
                                                local record = table.remove(mState.poolAS)
                                                if not record then record = {} end
                                                record.Part = part
                                                record.DistSq = partDelta:Dot(partDelta)
                                                record.Char = enemyChar
                                                table_insert(mState.objAS, record)
                                            end
                                    end
                                end 
                            end 
                            
                            table_sort(mState.objAS, function(a, b) return a.DistSq < b.DistSq end)
                            local closestTargetPart = nil
                            mState.igAS[1] = char
                            
                            for i = 1, #mState.objAS do
                                local obj = mState.objAS[i]
                                mState.igAS[2] = obj.Char
                                mState.pAS.FilterDescendantsInstances = mState.igAS
                                if not ws_Raycast(workspace, headPos, obj.Part.Position - headPos, mState.pAS) then
                                    closestTargetPart = obj.Part
                                    break 
                                end
                            end 

                            -- Recicla los registros de candidatos: después del primer uso ya no
                            -- crea una tabla nueva por parte del cuerpo en cada actualización.
                            for i = #mState.objAS, 1, -1 do
                                local record = mState.objAS[i]
                                record.Part = nil
                                record.Char = nil
                                record.DistSq = 0
                                table_insert(mState.poolAS, record)
                                mState.objAS[i] = nil
                            end
...
local function atacarUnaVezKillAll(
    ...
    if (
        referenceCF.Position
        - enemyHrp.Position
    ).Magnitude > KILL_RANGE
    then
        return false
    end
    ...
    task.spawn(function()
        ...
        for _, module in ipairs(ReplicatedStorage:GetDescendants()) do
            if module:IsA("ModuleScript") then
                ...
                local function scanTable(t, depth)
                    ...
                    for k, v in pairs(t) do
                        if type(v) == "table" then
                            ...
                            if type(k) == "string" and table.find(knownRarities, k) then
                ...
                if scannedModules % 25 == 0 then task.wait() end
            end
        end
