...
-- ============================================================
-- XEROHUB UI ADAPTER
-- Misma interfaz que XeroHub DUELS; conserva el estado y la lógica del script.
-- ============================================================
local dt = {}
dt.__state = {}
dt.__callbacks = {}
...
function dt.GetState(id)
    return dt.__state[id]
end

function dt.SetState(id, value, fireWatchers)
    dt.__state[id] = value
    if fireWatchers and dt.__callbacks[id] then
        for _, callback in ipairs(dt.__callbacks[id]) do
            pcall(callback, value)
        end
    end
end

function dt.OnChange(id, callback)
    dt.__callbacks[id] = dt.__callbacks[id] or {}
    table.insert(dt.__callbacks[id], callback)
end

function r.getState(id, fallback)
    local value = dt.GetState(id)
    if value ~= nil then return value end
    return fallback
end

function r.isOn(id)
    return dt.GetState(id) == true
end

function r.optionValue(id, fallback)
    local value = dt.GetState(id)
    if value == nil then return fallback end
    return value
end

function r.multiSelected(id)
    local value = dt.GetState(id)
    local selected = {}
...
function r.multiHasAny(id)
    return next(r.multiSelected(id)) ~= nil
end

function r.selectionAllows(id, value)
    if not r.multiHasAny(id) then return true end
    return r.multiSelected(id)[value] == true
end

function r.matchesMutationFilter(id, record)
    if not r.multiHasAny(id) then return true end
    local selected = r.multiSelected(id)
    for _, mutation in ipairs(r.recordMutations(record)) do
        if selected[mutation] then return true end
    end
    return false
end

function r.matchesEggFilters(record, zoneFilter, rarityFilter, mutationFilter)
    if zoneFilter then
        local areaId = record.AreaId
        if typeof(areaId) ~= "string" or not r.selectionAllows(zoneFilter, areaId) then return false end
    end
    local rarity = r.resolveRarity(record.AssetCategory)
    if typeof(rarity) ~= "string" or not r.selectionAllows(rarityFilter, rarity) then return false end
    return r.matchesMutationFilter(mutationFilter, record)
end

local XeroUI
local XeroWindow
local xeroEnv = (getgenv and getgenv()) or _G
local XERO_UI_URL = xeroEnv.NOX_UI_URL or "https://raw.githubusercontent.com/OnyxDevv/Onyx-web/refs/heads/main/main%20(3).lua"
...
local uiOk, uiResult = pcall(function()
    ...
    if isfile and readfile and isfile("XeroHub_UI.lua") then
        ...
        source = game:HttpGet(XERO_UI_URL)
    end
    local chunk, compileError = loadstring(source)
    if not chunk then error("No se pudo compilar XeroHub UI: " .. tostring(compileError)) end
    return chunk()
end)

if not uiOk or not uiResult then
    warn("[XeroHub] No se pudo cargar la interfaz: " .. tostring(uiResult))
    s = false
    if dh then pcall(function() dh:Destroy() end) end
    for _, connection in ipairs(br) do
        pcall(function()
...
XeroUI = uiResult
XeroWindow = XeroUI:CreateWindow({
    Title = "Xero | STEAL AN EGG",
    ...
    Resizable = true,
    OpenButton = {Title = "XEROHUB", Enabled = true},
})

dt.__window = XeroWindow

local mainNav = XeroWindow:Section({Title = "PRINCIPAL", Opened = true})
local personalNav = XeroWindow:Section({Title = "PERSONAL", Opened = true})
...
local function tabFromContainer(container, ensureSection)
    if type(container) == "table" then
        local tab = container.XeroTab or container.Tab or container
        if ensureSection and container.__xeroSectionPending then
            tab:Section({Title = container.Title or "Sección"})
            container.__xeroSectionPending = false
        end
        return tab
    end
    return container
...
function dt.AddTab(options)
    local id = tostring(options.Id or options.Title or ("tab" .. tostring(#dt.__tabs + 1)))
    local title = tabNames[id] or options.Title or id
    local owner = id == "system" and personalNav or mainNav
    local tab = owner:Tab({Title = title, Desc = tabDescriptions[id] or options.Description})
    local wrapper = {Id = id, XeroTab = tab}
    dt.__tabs[id] = wrapper
    return wrapper
end

function dt.AddSection(tabContainer, options)
    local tab = tabFromContainer(tabContainer, false)
    return {
        XeroTab = tab,
        Title = options.Title,
        Description = options.Description,
        __xeroSectionPending = true,
    }
end
...
local function runControlCallback(options, value)
    if options.Id ~= nil then dt.SetState(options.Id, value, true) end
    if options.Callback then pcall(options.Callback, value) end
end

function dt.AddToggle(container, options)
    ...
    local control = tab:Toggle({
        ...
        Callback = function(value)
            runControlCallback(options, value)
        end,
    })
...
function dt.AddSlider(container, options)
    local tab = tabFromContainer(container, true)
    local minimum = tonumber(options.Min) or 0
    local maximum = tonumber(options.Max) or 100
    local default = tonumber(options.Default)
    if default == nil then default = minimum end
    prepareState(options.Id, default)
    local desc = options.Description or options.Desc
    if not desc and options.Suffix and options.Suffix ~= "" then
        desc = "Unidad: " .. tostring(options.Suffix):gsub("^%s+", "")
    end
    local control = tab:Slider({
        Title = options.Title or "Slider",
        Desc = desc,
        Step = tonumber(options.Step) or 1,
        Value = {Min = minimum, Max = maximum, Default = default},
        Callback = function(value)
            runControlCallback(options, value)
        end,
    })
...
function dt.AddButton(container, options)
    local tab = tabFromContainer(container, true)
    return tab:Button({
        Title = options.Title or options.Text or "Acción",
        Desc = options.Description or options.Desc,
        Callback = function()
...
function dt.AddParagraph(container, options)
    local tab = tabFromContainer(container, true)
    return tab:Paragraph({
        Title = options.Title or "",
        Desc = options.Content or options.Description or options.Desc or "",
    })
end

function dt.AddDivider(container, options)
    local tab = tabFromContainer(container, true)
    return tab:Section({Title = (options and options.Title) or ""})
end

function dt.AddStatus(container, options)
    local tab = tabFromContainer(container, true)
    local control = tab:Paragraph({
        Title = options.Title or "Estado",
        Desc = tostring(options.Value or ""),
    })
    local wrapper = {Control = control, Status = "Neutral"}
    function wrapper:SetValue(value)
        if self.Control and not self.Control.Destroyed then
            self.Control:SetDesc(tostring(value or ""))
        end
        return self
    end
    function wrapper:SetStatus(status)
        self.Status = tostring(status or "Neutral")
        return self
    end
    return wrapper
end

function dt.AddInput(container, options)
    local tab = tabFromContainer(container, true)
    ...
    local control = tab:Input({
        ...
        Value = default,
        Callback = function(value)
            runControlCallback(options, tostring(value or ""))
        end,
    })
    if options.Id then dt.__controls[options.Id] = control end
    return control
...
function r.notify(title, content, noticeType, duration)
    if XeroUI and XeroUI.Notify then
        return XeroUI:Notify({
            Title = tostring(title or "XeroHub"),
...
pcall(function()
    XeroWindow:OnDestroy(function()
        if s and type(r.unload) == "function" then
            task.defer(function()
                if s and type(r.unload) == "function" then pcall(r.unload) end
            end)
        end
    end)
end)
...
function r.unload()
    ...
    pcall(function()
        if XeroWindow and not XeroWindow.Destroyed then XeroWindow:Destroy() end
    end)
    a.__APEX_HUB_RUNNING = nil
...
r.notify("XeroHub", "Listo · usa minimizar para mostrar el botón flotante.", "Success", 5)
