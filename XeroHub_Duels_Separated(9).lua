...
runtime.AppearanceCategories = {
    ...
    "Dominus", "Sparkle Time", "Espalda", "Hombro", "Exclusivos"
}
...
function runtime.RebuildAppearanceBodyGuards(char)
    ...
    if runtime.Appearance.Enabled.Korblox then
        ...
        if upper and upper:IsA("MeshPart") then
            -- MeshId/TextureID se reafirman después de cada cambio de apariencia,
            -- pero no se vigilan por string porque Roblox puede normalizar URLs de assets.
            ...
            if realParent and realParent.Parent
                ...
                pcall(function()
                    if pair.R6Fallback then
                        ...
                        local delta =
                            pair.InitialRealRelative:ToObjectSpace(currentRelative)
...
function runtime.BindAppearanceAccessoryGuard(char, generation)
    ...
    local function queueEnsure()
        if runtime.Appearance.AvatarClone and runtime.Appearance.AvatarClone.Applying then return end
...
function runtime.FastRestoreAppearanceOnRespawn(char, generation)
    ...
    -- No esperamos un tiempo fijo. Sólo esperamos las piezas indispensables y
    -- continuamos en cuanto existen; normalmente esto tarda uno o pocos frames.
    local humanoid = char:FindFirstChildOfClass("Humanoid")
