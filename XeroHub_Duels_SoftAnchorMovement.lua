...
-- ==========================================
-- SOFT ANCHOR MOVEMENT | SPEED + FLY
--
-- IMPORTANTE:
-- NO usamos Root.Anchored=true. Un HRP anclado + PivotTo puede quedarse sólo
-- visual/local y el servidor conserva la posición anterior.
--
-- Este sistema usa LinearVelocity + AlignOrientation sobre el HRP SIN ANCLAR:
-- se siente "anclado" cuando no hay input, pero el desplazamiento ocurre por
-- física del Character y puede replicarse mediante el network ownership normal.
-- ==========================================

runtime.AnchorMovement = runtime.AnchorMovement or {
    Mode = nil, -- "Speed" | "Fly" | nil
    Speed = 55,
    Character = nil,
    Humanoid = nil,
    Root = nil,
...
function runtime.AnchorMovementGetMoveVector()
    ...
    -- PlayerModule: joystick móvil / teclado / gamepad.
...
function runtime.AnchorMovementWorldDirection(mode)
    ...
    if mode == "Fly" then
        ...
        world = world + Vector3.new(0, vertical, 0)
    ...
    -- Speed: movimiento horizontal relativo a la cámara.
...
function runtime.AnchorMovementDestroyPhysics()
    ...
    if state.LinearVelocity then
        pcall(function() state.LinearVelocity:Destroy() end)
        state.LinearVelocity = nil
    end

    if state.AlignOrientation then
        pcall(function() state.AlignOrientation:Destroy() end)
...
function runtime.AnchorMovementRestore()
    local state = runtime.AnchorMovement
    ...
    if runtime.AnchorFlyGui then
        runtime.AnchorFlyGui.Visible = false
    end
end
...
function runtime.AnchorMovementCreatePhysics(root)
    ...
    attachment.Name = "XeroSoftAnchorAttachment"
    attachment.Parent = root

    local velocity = Instance.new("LinearVelocity")
    velocity.Name = "XeroSoftAnchorVelocity"
    velocity.Attachment0 = attachment
    velocity.RelativeTo = Enum.ActuatorRelativeTo.World
    velocity.VelocityConstraintMode = Enum.VelocityConstraintMode.Vector
    velocity.VectorVelocity = Vector3.zero

    -- Compatibilidad entre builds/API de Roblox.
    pcall(function()
    ...
    orientation.Name = "XeroSoftAnchorOrientation"
...
function runtime.AnchorMovementEnable(mode)
    local state = runtime.AnchorMovement

    runtime.AnchorMovementRestore()
    ...
    -- Evita dos motores escribiendo movimiento a la vez.
    ...
    -- CLAVE: debe estar desanclado para que la física pueda replicar.
    pcall(function()
        root.Anchored = false
        root.AssemblyLinearVelocity = Vector3.zero
        root.AssemblyAngularVelocity = Vector3.zero
    ...
    if runtime.AnchorFlyGui then
        runtime.AnchorFlyGui.Visible = mode == "Fly"
    end
    ...
    state.PhysicsConnection = runtime.Track(
        RunService.Heartbeat:Connect(function()
            ...
            -- Si algo vuelve a anclar el Root, lo soltamos; de lo contrario
            -- volvemos al problema del movimiento puramente local.
            if root.Anchored then
                root.Anchored = false
            end

            local direction =
                runtime.AnchorMovementWorldDirection(mode)

            if mode == "Fly" then
                -- Sin input = VectorVelocity 0: sensación de estar "clavado" en el aire.
                velocity.VectorVelocity =
                    direction * state.Speed
            else
                -- Speed conserva la componente Y que está resolviendo la física:
                -- gravedad, suelo, escalones y pequeñas caídas siguen funcionando.
                local currentY =
            ...
            if cam then
                if mode == "Fly" then
                    -- Fly orientado como la cámara.
                    orientation.CFrame =
                        cam.CFrame - cam.CFrame.Position
                else
                    -- Speed sólo rota en yaw; nunca inclina el avatar.
                    local look = cam.CFrame.LookVector
            ...
            -- El constraint controla traslación; limpiamos giro residual.
            root.AssemblyAngularVelocity = Vector3.zero
        end)
    )
...
-- Botones ↑ / ↓ para móvil. También aceptan mouse.
runtime.AnchorFlyGui = Instance.new("Frame")
runtime.AnchorFlyGui.Name = "XeroAnchorFlyControls"
runtime.AnchorFlyGui.AnchorPoint = Vector2.new(1, 1)
runtime.AnchorFlyGui.Position = UDim2.new(1, -22, 1, -120)
runtime.AnchorFlyGui.Size = UDim2.fromOffset(58, 126)
runtime.AnchorFlyGui.BackgroundTransparency = 1
runtime.AnchorFlyGui.Visible = false
runtime.AnchorFlyGui.ZIndex = 90
runtime.AnchorFlyGui.Parent = screenGui

function runtime.CreateAnchorFlyButton(textValue, y)
    local button = Instance.new("TextButton")
    ...
    button.AutoButtonColor = false
    button.ZIndex = 91
    button.Parent = runtime.AnchorFlyGui
...
runtime.AnchorFlyUpButton =
    runtime.CreateAnchorFlyButton("↑", 0)

runtime.AnchorFlyDownButton =
    runtime.CreateAnchorFlyButton("↓", 68)

runtime.Track(
    runtime.AnchorFlyUpButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch
            or input.UserInputType == Enum.UserInputType.MouseButton1 then
            runtime.AnchorMovement.UpHeld = true
        end
    end)
)

runtime.Track(
    runtime.AnchorFlyUpButton.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch
            or input.UserInputType == Enum.UserInputType.MouseButton1 then
            runtime.AnchorMovement.UpHeld = false
        end
    end)
)

runtime.Track(
    runtime.AnchorFlyDownButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch
            or input.UserInputType == Enum.UserInputType.MouseButton1 then
            runtime.AnchorMovement.DownHeld = true
        end
    end)
)

runtime.Track(
    runtime.AnchorFlyDownButton.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch
            or input.UserInputType == Enum.UserInputType.MouseButton1 then
            runtime.AnchorMovement.DownHeld = false
        end
    end)
)

Tabs.Mov:Section({
    Title = "Soft Anchor Movement"
})

UIElements.SliAnchorSpeed = Tabs.Mov:Slider({
    Title = "Velocidad",
    Desc = "Velocidad física usada por Soft Anchor Speed y Fly.",
    Step = 1,
    Value = {
        Min = 10,
        Max = 150,
...
UIElements.TogAnchorSpeed = Tabs.Mov:Toggle({
    Title = "Soft Anchor Speed",
    Desc = "Movimiento físico horizontal sin modificar WalkSpeed.",
    Value = false,

    Callback = function(enabled)
        if enabled then
            if runtime.AnchorMovement.Mode == "Fly"
                and UIElements.TogAnchorFly then
                runtime.AnchorMovement.Switching = true
                pcall(function()
                    UIElements.TogAnchorFly:Set(false)
                end)
                runtime.AnchorMovement.Switching = false
                task.wait()
            end

            if not runtime.AnchorMovementEnable("Speed") then
                task.defer(function()
                    pcall(function()
                        UIElements.TogAnchorSpeed:Set(false)
                    end)
                end)
            end

        elseif runtime.AnchorMovement.Mode == "Speed" then
            runtime.AnchorMovementRestore()
        end
    end
})
...
UIElements.TogAnchorFly = Tabs.Mov:Toggle({
    ...
    Desc = "Fly físico. PC: Espacio/E sube, Ctrl/Q baja.",
    Value = false,

    Callback = function(enabled)
        if enabled then
            if runtime.AnchorMovement.Mode == "Speed"
                and UIElements.TogAnchorSpeed then
                runtime.AnchorMovement.Switching = true
                pcall(function()
                    UIElements.TogAnchorSpeed:Set(false)
                end)
            ...
            if not runtime.AnchorMovementEnable("Fly") then
                task.defer(function()
                    pcall(function()
                        UIElements.TogAnchorFly:Set(false)
                    end)
        ...
        elseif runtime.AnchorMovement.Mode == "Fly" then
            runtime.AnchorMovementRestore()
        end
    end
})
...
runtime.AnchorMovementCleanup = function()
    ...
    if runtime.AnchorFlyGui
        and runtime.AnchorFlyGui.Parent then
        pcall(function()
            runtime.AnchorFlyGui:Destroy()
        end)
    end

    runtime.AnchorFlyGui = nil
    runtime.AnchorFlyUpButton = nil
    runtime.AnchorFlyDownButton = nil
end

runtime.Track(
    player.CharacterAdded:Connect(function()
        if runtime.AnchorMovement.Mode then
            runtime.AnchorMovementRestore()
            ...
            task.defer(function()
                ...
                pcall(function()
                    if UIElements.TogAnchorSpeed then
                        UIElements.TogAnchorSpeed:Set(false)
                    end

                    if UIElements.TogAnchorFly then
                        UIElements.TogAnchorFly:Set(false)
                    end
                end)
