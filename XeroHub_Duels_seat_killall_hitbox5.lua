...
ATTACK_Y_OFFSET = -8.65
-- TP real del Character: bastante más abajo de los pies. Con R15 esto deja
-- la cabeza por debajo de las plantas en vez de pegada a ellas.
ATTACK_CHARACTER_Y_OFFSET = -7.10
-- Nos colocamos un poco detrás del eje del enemigo para que el frente del
-- avatar/cuchillo tenga espacio para apuntar hacia arriba y entrar a las piernas.
ATTACK_FORWARD_OFFSET = 1.10
-- En Roblox, rotación X POSITIVA levanta el LookVector. Antes estaba en -18 y
-- por eso el avatar quedaba casi recto/apuntando hacia abajo.
ATTACK_PITCH_DEGREES = 58
...
-- Ventana activa de ataque. Durante este tiempo seguimos al objetivo, hacemos
-- pequeños lunges desde abajo hacia sus pies y reactivamos el cuchillo varias
-- veces. La base sigue quedando profunda; sólo el lunge sube temporalmente.
KILL_SWING_TRACK_TIME = 0.72
KILL_ATTACK_SPAM_INTERVAL = 0.085
KILL_LUNGE_PERIOD = 0.18
KILL_LUNGE_UP = 4.10
KILL_LUNGE_TOWARD = 0.65
KILL_LUNGE_EXTRA_PITCH = 12
...
-- ==========================================
-- CFRAME DEL SEAT EN ATAQUE
-- ==========================================

local function obtenerAttackCFKillAll(enemyHrp, lungeAlpha)
    if not enemyHrp or not enemyHrp.Parent then
        return nil
    end

    lungeAlpha = math.clamp(tonumber(lungeAlpha) or 0, 0, 1)

    local _, yaw, _ = enemyHrp.CFrame:ToOrientation()
    local seatY = ATTACK_Y_OFFSET + (KILL_LUNGE_UP * 0.82 * lungeAlpha)
    local seatZ = ATTACK_FORWARD_OFFSET - (KILL_LUNGE_TOWARD * lungeAlpha)
    local pitch = ATTACK_PITCH_DEGREES + (KILL_LUNGE_EXTRA_PITCH * lungeAlpha)

    -- La posición base queda profunda. Durante el lunge el Seat sube un poco
    -- y avanza hacia el objetivo para que brazo/Handle barran pies y pantorrillas.
    return CFrame.new(enemyHrp.Position)
        * CFrame.Angles(0, yaw, 0)
        * CFrame.new(0, seatY, seatZ)
        * CFrame.Angles(math.rad(pitch), 0, 0)
end

local function obtenerLungeAlphaKillAll(startedAt)
    local period = math.max(0.05, tonumber(KILL_LUNGE_PERIOD) or 0.18)
    local phase = ((os.clock() - (startedAt or os.clock())) / period) % 1
    -- Onda triangular: abajo -> arriba -> abajo, sin saltos bruscos.
    if phase < 0.5 then
        return phase * 2
    end
    return (1 - phase) * 2
...
local function seguirObjetivoKillAll(myChar, myHrp, enemyHrp, lungeAlpha)
    ...
    local attackCF = obtenerAttackCFKillAll(enemyHrp, lungeAlpha)
    if not attackCF then
        return false
    end

    local _, yaw, _ = enemyHrp.CFrame:ToOrientation()
    local characterY = ATTACK_CHARACTER_Y_OFFSET + (KILL_LUNGE_UP * lungeAlpha)
    local characterZ = ATTACK_FORWARD_OFFSET - (KILL_LUNGE_TOWARD * lungeAlpha)
    local pitch = ATTACK_PITCH_DEGREES + (KILL_LUNGE_EXTRA_PITCH * lungeAlpha)
    local characterCF = CFrame.new(enemyHrp.Position)
        * CFrame.Angles(0, yaw, 0)
        * CFrame.new(0, characterY, characterZ)
        * CFrame.Angles(math.rad(pitch), 0, 0)
    ...
    -- TP REAL como el Kill All anterior. El Seat por sí solo no transporta el
    -- Character de forma fiable cuando está Anchored, así que movemos ambos.
    -- Con una inclinación fuerte hacia arriba, el avatar sigue sentado y la mano/cuchillo suben hacia las piernas.
    pcall(function()
        seat.CFrame = attackCF
        myChar:PivotTo(characterCF)
        freezeKillRoot(myHrp)

        -- PivotTo puede hacer que Roblox rehaga el SeatWeld; reafirmarlo deja
        -- al Humanoid sentado sin depender del movimiento físico de la silla.
        seat:Sit(myHum)
        myHum.Sit = true
        if myHum:GetState() ~= Enum.HumanoidStateType.Seated then
            myHum:ChangeState(Enum.HumanoidStateType.Seated)
...
local function atacarUnaVezKillAll(
    ...
    -- ======================================
    -- SPAM CONTROLADO + LUNGE DESDE DEBAJO
    --
    -- La posición base permanece profunda. Durante cada ciclo subimos y
    -- avanzamos momentáneamente para que el Handle atraviese pies/piernas.
    -- Cada intervalo hacemos Deactivate -> Activate para retrigger real del Tool.
    -- ======================================
    ...
    do
        ...
        local now = os.clock()
        local lungeAlpha = obtenerLungeAlphaKillAll(swingStarted)

        -- El rig hace un pequeño "estirón" hacia arriba/adelante y vuelve abajo.
        seguirObjetivoKillAll(myChar, myHrp, enemyHrp, lungeAlpha)
