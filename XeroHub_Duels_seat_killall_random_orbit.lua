...
function runtime.Cleanup()
    ...
    if runtime.KillAllCleanup then pcall(runtime.KillAllCleanup) end
...
runtime.StartupCheckpoint = "Preparando controles"
runtime.StartupWatchdogToken = (runtime.StartupWatchdogToken or 0) + 1
runtime.StartupWatchdogExpectedToken = runtime.StartupWatchdogToken
...
-- Failsafe del splash: si cualquier inicialización tarda demasiado o un hilo principal
-- queda esperando, la pantalla de carga no puede quedarse bloqueada para siempre.
task.delay(8, function()
    if not runtime.Alive or runtime.StartupWatchdogToken ~= runtime.StartupWatchdogExpectedToken then return end
    local startupGui = startupSplashState.Gui
    if startupGui and startupGui.Parent then
        warn("[XeroHub] Startup watchdog: se excedieron 8s en '"
            .. tostring(runtime.StartupCheckpoint or "etapa desconocida")
            .. "'. Cerrando splash de seguridad.")
...
-- ==========================================
runtime.StartupCheckpoint = "Sonidos"
...
-- ==========================================
runtime.StartupCheckpoint = "Texturas"
...
-- ==========================================
runtime.StartupCheckpoint = "Aimbot y combate"
-- PESTAÑA AIMBOT
-- ==========================================
...
-- =========================
runtime.StartupCheckpoint = "Apariencia"
-- UI DE APARIENCIA
-- =========================
...
--              ↓
-- ESPERAR SENTADO EN SILLA INVISIBLE
--              ↓
-- ENEMIGO VULNERABLE
--              ↓
-- EQUIPAR CUCHILLO
--              ↓
-- SEAT DEBAJO + TRACKING DEL ENEMIGO
--              ↓
...
-- BASE UNA SOLA VEZ
--              ↓
-- SENTADO/DERECHO SIN MICRO MOVIMIENTOS
-- ==========================================
...
runtime.KillAllSeat = nil
runtime.KillAllSeatCharacter = nil

-- Kill All reutiliza el Hitbox Expander normal del hub. Guardamos el estado
-- previo para que al apagarlo no pise la configuración manual del usuario.
runtime.KillAllHitboxBackup = nil

function runtime.SyncKillAllHitboxUI()
    local previousSuppress = runtime.SuppressNotifications
    runtime.SuppressNotifications = true

    pcall(function()
        if UIElements.TogHitbox then UIElements.TogHitbox:Set(hitboxEnabled) end
    end)
    pcall(function()
        if UIElements.TogHbInv then UIElements.TogHbInv:Set(hitboxInvisible) end
    end)
    pcall(function()
        if UIElements.SliHitbox then UIElements.SliHitbox:Set(hitboxSize) end
    end)

    runtime.SuppressNotifications = previousSuppress
end

function runtime.EnableKillAllHitbox()
    if not runtime.KillAllHitboxBackup then
        runtime.KillAllHitboxBackup = {
            Enabled = hitboxEnabled == true,
            Invisible = hitboxInvisible == true,
            Size = tonumber(hitboxSize) or 10,
        }
    end
    ...
    hitboxSize = 5
    runtime.SyncKillAllHitboxUI()
end

function runtime.EnsureKillAllHitbox()
    if not killAllEnabled then return end
    if hitboxEnabled ~= true or hitboxInvisible ~= true or tonumber(hitboxSize) ~= 5 then
        hitboxEnabled = true
        hitboxInvisible = true
        hitboxSize = 5
        runtime.SyncKillAllHitboxUI()
...
function runtime.RestoreKillAllHitbox()
    local backup = runtime.KillAllHitboxBackup
    if not backup then return end

    hitboxEnabled = backup.Enabled == true
    hitboxInvisible = backup.Invisible == true
    hitboxSize = tonumber(backup.Size) or 10
    runtime.KillAllHitboxBackup = nil
    runtime.SyncKillAllHitboxUI()
end
...
-- Ataque desde debajo de los pies. La silla queda bastante más abajo porque
-- Seat:Sit() vuelve a acomodar el HRP por encima del centro del asiento.
ATTACK_Y_OFFSET = -6.40
-- Un poco más alto que la versión anterior para quedar más pegado a pies/pantorrillas.
ATTACK_CHARACTER_Y_OFFSET = -4.65
-- Se conserva como radio base/fallback; durante ataque usamos una órbita corta aleatoria.
ATTACK_FORWARD_OFFSET = 1.85
-- Fallback si por alguna razón no puede calcularse lookAt.
ATTACK_PITCH_DEGREES = 48
KILL_SEAT_BASE_OFFSET = -1.55

-- ÓRBITA EVASIVA: movimientos cortos alrededor del enemigo, sin alejarnos de la hitbox.
KILL_ORBIT_RADIUS_MIN = 1.45
KILL_ORBIT_RADIUS_MAX = 2.05
KILL_ORBIT_CHANGE_MIN = 0.11
KILL_ORBIT_CHANGE_MAX = 0.23
KILL_ORBIT_STEP_MIN_DEG = 28
KILL_ORBIT_STEP_MAX_DEG = 72
KILL_ORBIT_VERTICAL_JITTER = 0.16
KILL_ORBIT_AIM_Y = -2.45

runtime.KillAllOrbit = {
    Target = nil,
    Angle = 0,
    TargetAngle = 0,
    Radius = ATTACK_FORWARD_OFFSET,
    TargetRadius = ATTACK_FORWARD_OFFSET,
    Vertical = 0,
    TargetVertical = 0,
    NextChange = 0,
    LastAt = os.clock(),
...
local HANDLE_READY_DISTANCE = 6.2
...
-- Ventana activa de ataque. El cuerpo permanece sentado en una pose fija y
-- pegado al objetivo; lo único que se repite es el cuchillazo.
KILL_SWING_TRACK_TIME = 0.72
KILL_ATTACK_SPAM_INTERVAL = 0.085
-- Se conservan por compatibilidad con el resto del bloque, pero la pose de ataque
-- ya no usa lunge vertical/horizontal.
KILL_LUNGE_PERIOD = 0.18
KILL_LUNGE_UP = 0
KILL_LUNGE_TOWARD = 0
KILL_LUNGE_EXTRA_PITCH = 0

-- Máximo esperando la muerte después de la ventana de spam/lunge.
local KILL_CONFIRM_TIME = 0.85

-- Reintentar después si el golpe no mató.
local FAILED_RETRY_DELAY = 0.00
...
-- ==========================================
-- BLOQUEO DE MOVIMIENTO KILL ALL
--
-- Mientras Kill All está activo deshabilitamos los Controls del PlayerModule
-- y guardamos/restauramos las propiedades de movimiento del Humanoid.
-- Esto evita que WASD/thumbstick/salto saquen al jugador del Seat.
-- ==========================================

function bloquearMovimientoKillAll(humanoid)
    if not humanoid or not humanoid.Parent then return end

    if runtime.KillAllMoveHumanoid ~= humanoid then
        runtime.KillAllMoveHumanoid = humanoid
        runtime.KillAllMoveState = {
    ...
    if not runtime.KillAllControls then
        pcall(function()
            local playerScripts = player:FindFirstChild("PlayerScripts")
            local playerModule = playerScripts and playerScripts:FindFirstChild("PlayerModule")
            if playerModule then
                runtime.KillAllControls = require(playerModule):GetControls()
            end
        end)
    end

    if runtime.KillAllControls then
        pcall(function() runtime.KillAllControls:Disable() end)
    end
...
function restaurarMovimientoKillAll(humanoid)
    if runtime.KillAllControls then
        pcall(function() runtime.KillAllControls:Enable() end)
    end

    local targetHumanoid = humanoid or runtime.KillAllMoveHumanoid
    local state = runtime.KillAllMoveState
    if targetHumanoid and targetHumanoid.Parent and state then
        pcall(function()
            targetHumanoid.WalkSpeed = state.WalkSpeed
            targetHumanoid.UseJumpPower = state.UseJumpPower
    ...
    runtime.KillAllMoveHumanoid = nil
    runtime.KillAllMoveState = nil
end

-- ==========================================
-- SILLA INVISIBLE KILL ALL
--
-- El Seat sustituye PlatformStand + rotación de 90°. El personaje permanece
-- en Seated y derecho; para seguir a un objetivo movemos el Seat, no el rig.
-- ==========================================

function destruirSillaKillAll(humanoid)
    local seat = runtime.KillAllSeat

    if not humanoid and seat and seat.Parent then
    ...
    if humanoid and humanoid.Parent then
        pcall(function()
            ...
            humanoid.AutoRotate = killAllEnabled and false or humanoid.AutoRotate
...
function colocarSillaKillAll(myChar, myHum, seatCF)
    if not killAllEnabled
        or not myChar or not myChar.Parent
        or not myHum or not myHum.Parent
        or not seatCF
    then
        return nil
    end
    ...
    if seat and (not seat.Parent or runtime.KillAllSeatCharacter ~= myChar) then
        destruirSillaKillAll()
        seat = nil
    end

    if not seat then
        seat = Instance.new("Seat")
        seat.Name = "XeroHub_KillAllSeat"
        seat.Size = Vector3.new(2, 0.55, 2)
        seat.Transparency = 1
        seat.Anchored = true
        ...
        runtime.KillAllSeat = seat
        runtime.KillAllSeatCharacter = myChar
    end
    ...
    pcall(function()
        ...
        -- PivotTo/teleports continuos pueden sacar visualmente al Humanoid de
        -- la pose sentada aunque el Seat siga existiendo. Sólo reentramos a
        -- Seated si Roblox realmente nos sacó del estado, para no resetear
        -- innecesariamente la animación del cuchillo en cada frame.
        if myHum:GetState() ~= Enum.HumanoidStateType.Seated then
            myHum:ChangeState(Enum.HumanoidStateType.Seated)
        end
    end)
...
runtime.KillAllCleanup = function()
    local char = player.Character

    if runtime.KillAllSeat and char and char.Parent and killAllBaseCFrame then
        pcall(function() char:PivotTo(killAllBaseCFrame) end)
    end

    destruirSillaKillAll()
    restaurarMovimientoKillAll()
end
...
local function obtenerParteObjetivoKillAll(
    ...
    -- Para el modo debajo-del-piso validamos contra piernas/pies, no contra
    -- UpperTorso. Así el cuchillo puede estar realmente abajo y seguir contando
    -- como contacto válido.
    local targetPart =
...
-- ==========================================
-- ÓRBITA / CFRAME DE ATAQUE
--
-- Se mueve en una órbita corta y pseudoaleatoria alrededor del enemigo,
-- pero mantiene radio pequeño y SIEMPRE mira hacia sus pies.
-- ==========================================

function runtime.UpdateKillAllOrbit(enemyHrp)
    local state = runtime.KillAllOrbit
    if not state or not enemyHrp or not enemyHrp.Parent then
        return 0, ATTACK_FORWARD_OFFSET, 0
    end

    local now = os.clock()

    if state.Target ~= enemyHrp then
        state.Target = enemyHrp
        state.Angle = math.rad(math.random(-25, 25))
        state.TargetAngle = state.Angle
        state.Radius = ATTACK_FORWARD_OFFSET
        state.TargetRadius = ATTACK_FORWARD_OFFSET
        state.Vertical = 0
        state.TargetVertical = 0
        state.NextChange = now
        state.LastAt = now
    end

    local dt = math.clamp(now - (state.LastAt or now), 0, 0.06)
    state.LastAt = now

    if now >= (state.NextChange or 0) then
        local direction = math.random(0, 1) == 0 and -1 or 1
        local stepDeg = math.random(KILL_ORBIT_STEP_MIN_DEG, KILL_ORBIT_STEP_MAX_DEG)

        state.TargetAngle = state.Angle + math.rad(stepDeg * direction)
        state.TargetRadius =
            KILL_ORBIT_RADIUS_MIN
            + math.random() * (KILL_ORBIT_RADIUS_MAX - KILL_ORBIT_RADIUS_MIN)

        state.TargetVertical =
            (math.random() * 2 - 1) * KILL_ORBIT_VERTICAL_JITTER

        state.NextChange =
            now
            + KILL_ORBIT_CHANGE_MIN
            + math.random() * (KILL_ORBIT_CHANGE_MAX - KILL_ORBIT_CHANGE_MIN)
    end

    -- Suavizado corto: evasivo pero sin saltos gigantes que separen el Handle.
    local alpha = math.clamp(dt * 11, 0, 1)
    state.Angle = state.Angle + (state.TargetAngle - state.Angle) * alpha
    state.Radius = state.Radius + (state.TargetRadius - state.Radius) * alpha
    state.Vertical = state.Vertical + (state.TargetVertical - state.Vertical) * alpha

    return state.Angle, state.Radius, state.Vertical
end

function runtime.GetKillAllOrbitCFrames(enemyHrp)
    if not enemyHrp or not enemyHrp.Parent then
        return nil, nil
    end

    local angle, radius, vertical = runtime.UpdateKillAllOrbit(enemyHrp)

    -- Offset local alrededor del enemigo; al multiplicarlo por su CFrame la órbita
    -- acompaña su orientación/movimiento sin perder cercanía.
    local orbitLocal = Vector3.new(
        math.sin(angle) * radius,
        0,
        math.cos(angle) * radius
    )

    local horizontalWorld =
        enemyHrp.CFrame:VectorToWorldSpace(orbitLocal)

    local characterPos =
        enemyHrp.Position
        + horizontalWorld
        + Vector3.new(0, ATTACK_CHARACTER_Y_OFFSET + vertical, 0)

    local seatPos =
        enemyHrp.Position
        + horizontalWorld
        + Vector3.new(0, ATTACK_Y_OFFSET + vertical, 0)

    -- Apuntamos a pies/pantorrillas. Desde debajo, lookAt inclina naturalmente
    -- el rig hacia arriba y mantiene al enemigo enfrente aunque cambiemos de lado.
    local aimPoint =
        enemyHrp.Position
        + Vector3.new(0, KILL_ORBIT_AIM_Y, 0)

    local characterCF
    local seatCF

    pcall(function()
        characterCF = CFrame.lookAt(characterPos, aimPoint)
        seatCF = CFrame.lookAt(seatPos, aimPoint)
    end)

    if not characterCF or not seatCF then
        local _, yaw, _ = enemyHrp.CFrame:ToOrientation()
        characterCF =
            CFrame.new(enemyHrp.Position)
            * CFrame.Angles(0, yaw, 0)
            * CFrame.new(0, ATTACK_CHARACTER_Y_OFFSET, ATTACK_FORWARD_OFFSET)
            * CFrame.Angles(math.rad(ATTACK_PITCH_DEGREES), 0, 0)

        seatCF =
            CFrame.new(enemyHrp.Position)
            * CFrame.Angles(0, yaw, 0)
            * CFrame.new(0, ATTACK_Y_OFFSET, ATTACK_FORWARD_OFFSET)
            * CFrame.Angles(math.rad(ATTACK_PITCH_DEGREES), 0, 0)
    end

    return seatCF, characterCF
end

local function obtenerAttackCFKillAll(enemyHrp, _lungeAlpha)
    local seatCF = runtime.GetKillAllOrbitCFrames(enemyHrp)
    return seatCF
end

local function obtenerLungeAlphaKillAll(_startedAt)
    return 0
end
...
-- SALIR DE LA SILLA
--
-- Se usa en zona segura o cuando queremos devolver control normal.
-- ==========================================

local function liberarReposoKillAll(hum, hrp)
    destruirSillaKillAll(hum)
...
--
-- Kill All sigue ENCENDIDO, pero si ya no quedan enemigos soltamos el Seat
-- para que el usuario pueda caminar y saltar normalmente.
-- ==========================================

local function liberarMovimientoKillAll(myHum, myHrp)
    destruirSillaKillAll(myHum)

    if myHrp and myHrp.Parent then
        freezeKillRoot(myHrp)
    end

    killAllResting = false
    killAllRestCharacter = nil
    killAllIdleFree = true
end


-- ==========================================
-- REPOSO EN BASE SENTADO
--
-- En vez de acostar el rig con PlatformStand, colocamos una silla invisible
-- debajo de la posición base y mantenemos al Humanoid en Seated.
-- ==========================================

local function reposarEnBaseKillAll(myChar, myHum, myHrp)
    if runtime.KillAllOrbit then
        runtime.KillAllOrbit.Target = nil
    end
    ...
    if killAllRestCharacter ~= myChar then
        destruirSillaKillAll()
        killAllResting = false
    end

    local _, yaw, _ = killAllBaseCFrame:ToOrientation()
    local seatCF = CFrame.new(
        killAllBaseCFrame.Position + Vector3.new(0, KILL_SEAT_BASE_OFFSET, 0)
    ) * CFrame.Angles(0, yaw, 0)

    -- IMPORTANTE: mover sólo un Seat anclado no arrastra el rig de forma
    -- consistente en todos los ejecutores. Regresamos el Character de verdad
    -- y luego reafirmamos la silla para conservar la pose sentada/derecha.
    pcall(function()
        myChar:PivotTo(killAllBaseCFrame)
    end)

    freezeKillRoot(myHrp)

    local seat = colocarSillaKillAll(myChar, myHum, seatCF)
...
-- ==========================================
-- SEGUIR AL ENEMIGO CON EL SEAT
--
-- El rig ya no se teletransporta/rota directamente. El Seat se mantiene
-- bajo el objetivo y el Humanoid permanece sentado y vertical.
-- ==========================================

local function seguirObjetivoKillAll(myChar, myHrp, enemyHrp, _lungeAlpha)
    if not myChar or not myChar.Parent
        or not myHrp or not myHrp.Parent
        or not enemyHrp or not enemyHrp.Parent
    then
        return false
    end
    ...
    bloquearMovimientoKillAll(myHum)

    local attackCF, characterCF = runtime.GetKillAllOrbitCFrames(enemyHrp)
    if not attackCF or not characterCF then
        return false
    end

    local seat = colocarSillaKillAll(myChar, myHum, attackCF)
    if not seat then
        return false
    end

    -- Mover Character + Seat juntos. CFrame.lookAt hace que el avatar apunte
    -- siempre al enemigo mientras la órbita cambia alrededor de él.
    pcall(function()
        seat.CFrame = attackCF
        myChar:PivotTo(characterCF)
        freezeKillRoot(myHrp)
...
local function esperarContactoKillAll(
    ...
    do
        ...
        -- IMPORTANTE: mientras adquirimos contacto también hacemos un lunge
        -- suave. Así el Handle puede alcanzar pies/pantorrillas aun si la pose
        -- sentada base queda bastante profunda.
        seguirObjetivoKillAll(
            myChar,
            myHrp,
            enemyHrp,
            obtenerLungeAlphaKillAll(deadline - ARRIVAL_TIMEOUT)
        )
...
local function atacarUnaVezKillAll(
    ...
    -- ======================================
    -- PREPARAR SILLA PARA ATACAR
    --
    -- No nos levantamos: seguimos en Seated y el Seat será el que se mueva.
    -- ======================================
    ...
    --
    -- Primero nos colocamos debajo del objetivo ya sentados. El único Activate()
    -- ocurre después de confirmar contacto para evitar cuchillazos al aire.
    -- ======================================
    ...
    if not contacto then
        ...
        if enemyHrp and enemyHrp.Parent then
            seguirObjetivoKillAll(myChar, myHrp, enemyHrp, 0)
        end
    ...
    -- ======================================
    -- SPAM CONTROLADO SENTADO DESDE DEBAJO
    --
    -- El cuerpo permanece sentado y pegado, pero describe una órbita corta
    -- pseudoaleatoria alrededor del enemigo. CFrame.lookAt lo mantiene siempre
    -- mirando hacia sus pies mientras el Tool hace Deactivate -> Activate.
    -- ======================================

    seguirObjetivoKillAll(myChar, myHrp, enemyHrp, 0)
    KillRunService.Heartbeat:Wait()

    local swingStarted = os.clock()
    local nextAttackAt = 0
    local swingHadContact = false
    ...
    while runtime.Alive
        ...
        and enemyHrp
        and enemyHrp.Parent
        and (os.clock() - swingStarted) < KILL_SWING_TRACK_TIME
    do
        if estaEnZonaSeguraKillAll(enemyChar, targetPlayer) then
            break
        end

        local now = os.clock()

        -- Pose fija durante todo el spam: seguimos pegados al enemigo sin levantar
        -- el cuerpo. El movimiento repetido lo hace únicamente el Tool.
        seguirObjetivoKillAll(myChar, myHrp, enemyHrp, 0)

        if now >= nextAttackAt then
            nextAttackAt = now + KILL_ATTACK_SPAM_INTERVAL
            pcall(function()
        ...
        if contactoRealKillAll(myHrp, arma, enemyChar) then
            swingHadContact = true
        end
    end
    ...
    -- Aunque el chequeo geométrico no haya visto un frame perfecto, dejamos
    -- que la confirmación de muerte decida: el hitbox del juego puede registrar
    -- contacto entre Heartbeats.


    -- ======================================
    -- ESPERAR KILL CONFIRMADA
    --
    -- Después del spam seguimos debajo mientras llega la confirmación.
    -- ======================================
    ...
    -- ======================================
    -- MANTENER CONTACTO / BASE
    --
    -- Si el objetivo sigue vivo NO nos despegamos: permanecemos debajo de él
    -- y el siguiente intento comienza desde la misma posición. Sólo volvemos
    -- a la base cuando la muerte ya fue confirmada.
    -- ======================================
    ...
    elseif enemyHrp and enemyHrp.Parent and not estaEnZonaSeguraKillAll(enemyChar, targetPlayer) then
        seguirObjetivoKillAll(myChar, myHrp, enemyHrp, 0)
    end
...
UIElements.TogKillAll =
    ...
    Callback =
        ...
        if Value then
            ...
            -- Kill All usa temporalmente hitbox 5 e invisible para que el
            -- barrido del cuchillo tenga un volumen de contacto más consistente.
            runtime.EnableKillAllHitbox()

            bloquearMovimientoKillAll(hum)
            ...
            task.spawn(function()
                ...
                do
                    ...
                    if not encontroObjetivo then
                        ...
                        -- Kill All sigue ON: permanecemos sentados/bloqueados
                        -- en la base hasta que aparezca otro objetivo o se apague.
                    ...
                    elseif not intentoAtaque then
                        ...
                        -- Hay objetivo: no regresamos a base entre reintentos.
                        -- Conservamos Seat + controles bloqueados donde ya estamos.
                        bloquearMovimientoKillAll(myHum)
                        freezeKillRoot(myHrp)
        ...
        -- ==================================
            ...
            -- Restaurar exactamente el Hitbox Expander que había antes de
            -- encender Kill All (estado, invisibilidad y tamaño).
            runtime.RestoreKillAllHitbox()
            ...
            -- Si se apaga mientras todavía estamos debajo de un enemigo,
            -- volver a la base antes de soltar el Seat evita quedar bajo el mapa.
            if runtime.KillAllSeat and char and char.Parent and killAllBaseCFrame then
                pcall(function()
                    char:PivotTo(killAllBaseCFrame)
                end)
            end

            destruirSillaKillAll(hum)
            restaurarMovimientoKillAll(hum)
...
-- ==========================================
runtime.StartupCheckpoint = "Visuales"
...
-- ==========================================
runtime.StartupCheckpoint = "Movimiento"
-- PESTAÑA MOVIMIENTO
-- ==========================================
...
-- ==========================================
runtime.StartupCheckpoint = "Gráficos"
...
runtime.StartupCheckpoint = "AutoFarm y cajas"
...
-- Lazy resolve: nunca bloquea el arranque esperando Packages/Networking.
-- Si el juego todavía no creó el remote, se reintenta de forma barata mientras
-- Auto Farm esté activo y se cachea en cuanto aparece.
local farmNetwork = { Networking = nil, Remote = nil }

local function resolveRemoteFarm()
    if farmNetwork.Remote and farmNetwork.Remote.Parent then return farmNetwork.Remote end

    local packages = ReplicatedStorage:FindFirstChild("Packages")
    if not packages then
        farmNetwork.Networking = nil
        farmNetwork.Remote = nil
        return nil
    end

    if not farmNetwork.Networking or not farmNetwork.Networking.Parent then
        farmNetwork.Networking = packages:FindFirstChild("Networking")
    end
    if not farmNetwork.Networking then
        farmNetwork.Remote = nil
        return nil
    end

    farmNetwork.Remote = farmNetwork.Networking:FindFirstChild("RE/Events/CollectEventSpawnable")
    return farmNetwork.Remote
end
        ...
        task.spawn(function()
            ...
            while runtime.Alive and getgenv().AutoEventFarm do
                ...
                if hrp then
                    ...
                    pcall(function()
                        local remote = resolveRemoteFarm()
                        if remote then
                            remote:FireServer()
                        end
                    end)
...
-- ==========================================
runtime.StartupCheckpoint = "Animaciones"
...
-- ==========================================
runtime.StartupCheckpoint = "Configuración"
...
runtime.StartupCheckpoint = "Listo"
runtime.StartupWatchdogToken = (runtime.StartupWatchdogToken or 0) + 1
startupSplashState.Finish()
runtime.NotificationsReady = true
...
-- XERO_KILLALL_SEAT_LOCK_STICKY_TARGET_2026_09_20
