...
ATTACK_Y_OFFSET = -7.25
-- El Character queda un poco más alto que en la versión anterior, pero sigue
-- claramente debajo de los pies. La silla compensa la altura del estado Seated.
ATTACK_CHARACTER_Y_OFFSET = -5.50
-- Un poco más atrás del enemigo para que el brazo/cuchillo tenga recorrido hacia
-- sus pies sin meter nuestro torso dentro de su HRP.
ATTACK_FORWARD_OFFSET = 1.70
-- Inclinación moderada: sentado y reclinado hacia arriba, no casi acostado.
-- El cuchillo queda orientado hacia pies/pantorrillas.
ATTACK_PITCH_DEGREES = 38
...
local function obtenerAttackCFKillAll(enemyHrp, _lungeAlpha)
    ...
    -- Pose FIJA de ataque: la silla no sube/baja durante el spam. Permanecemos
    -- sentados, un poco detrás del enemigo y apuntando hacia sus pies.
...
local function seguirObjetivoKillAll(myChar, myHrp, enemyHrp, lungeAlpha)
    ...
    local attackCF = obtenerAttackCFKillAll(enemyHrp, 0)
...
local function atacarUnaVezKillAll(
    ...
    --
    -- El cuerpo permanece sentado en una pose fija debajo/detrás del enemigo.
    -- Cada intervalo hacemos Deactivate -> Activate para retrigger real del Tool
    -- mientras el tracking mantiene el cuchillo orientado hacia los pies.
    -- ======================================
