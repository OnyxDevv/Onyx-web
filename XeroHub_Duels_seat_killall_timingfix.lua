...
-- Ventana real del swing. El cuchillo se activa una sola vez y durante este
-- tiempo seguimos al objetivo para que el Handle atraviese piernas/pies cuando
-- la animación llegue a su frame de impacto.
KILL_SWING_TRACK_TIME = 0.26
...
local function obtenerAttackCFKillAll(enemyHrp)
    ...
    -- Debajo de los pies + un poco desplazado en el eje local del enemigo.
    -- La inclinación del Seat hace que el rig permanezca sentado pero con el
    -- torso/mano ligeramente orientados hacia las piernas.
...
local function seguirObjetivoKillAll(myChar, myHrp, enemyHrp)
...
local function atacarUnaVezKillAll(
    ...
    -- ======================================
    -- ÚNICO CUCHILLAZO + VENTANA REAL DEL SWING
    --
    -- Antes esperábamos a terminar TODO y recién entonces Activate(). En este
    -- cuchillo eso deja el frame de impacto demasiado tarde. Ahora activamos
    -- una sola vez estando ya debajo y seguimos pegados durante el wind-up.
    -- ======================================

    -- Un frame limpio para que SeatWeld, pose y Handle adopten el CFrame final.
    seguirObjetivoKillAll(myChar, myHrp, enemyHrp)
    ...
    do
        ...
        -- Mantener exactamente la pose debajo de pies durante TODA la animación.
        seguirObjetivoKillAll(myChar, myHrp, enemyHrp)
        KillRunService.Heartbeat:Wait()
    ...
    -- Si la animación nunca acercó realmente el Handle, no fingimos éxito.
    if not swingHadContact and enemyHum and enemyHum.Health > 0 then
        pcall(function()
            if arma and arma.Parent == myChar then arma:Deactivate() end
        end)
        reposarEnBaseKillAll(myChar, myHum, myHrp)
        failedTargets[targetPlayer] = os.clock() + FAILED_RETRY_DELAY
        return false
    end
