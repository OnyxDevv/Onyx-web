...
-- Centro del Seat debajo del objetivo. Al estar sentado el HRP queda
-- aproximadamente en la misma zona de contacto que antes, pero sin acostarnos.
ATTACK_Y_OFFSET = -4.05
-- Posición REAL del HumanoidRootPart durante el ataque. Conserva la altura
-- efectiva del Kill All anterior, pero sin la rotación de 90 grados.
ATTACK_CHARACTER_Y_OFFSET = -2.55
...
local function obtenerAttackCFKillAll(enemyHrp)
    ...
    return CFrame.new(
        enemyHrp.Position + Vector3.new(0, ATTACK_Y_OFFSET, 0)
...
local function seguirObjetivoKillAll(myChar, myHrp, enemyHrp)
    ...
    local _, yaw, _ = enemyHrp.CFrame:ToOrientation()
    local characterCF = CFrame.new(
        enemyHrp.Position + Vector3.new(0, ATTACK_CHARACTER_Y_OFFSET, 0)
    ) * CFrame.Angles(0, yaw, 0)
    ...
    -- Sin la rotación X de 90° el avatar permanece vertical/sentado.
    pcall(function()
