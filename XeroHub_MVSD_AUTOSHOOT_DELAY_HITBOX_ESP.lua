...
local function ensureHitboxAdornment(part)
    ...
    box.Transparency = 0.68
    box.Size = part.Size
    box.Visible = state.HitboxVisible
    box.Parent = part

    runtime.HitboxAdornments[part] = box
    return box
end
...
local function restoreAllExpandedHitboxes()
    local list = {}
...
local function refreshExpandedHitboxes()
    if not state.HitboxExpander or state.InLobby then
        restoreAllExpandedHitboxes()
        return
    end
...
local function ensureSkeletonLines(plr, needed)
    ...
    while #lines < needed do
        lines[#lines + 1] = newESPLine(1.25, 0.9)
    end
...
local function autoShootOneShot()
    ...
    if ok then
        ...
        state.AutoShootNextAllowedAt =
            now + math.clamp(
                tonumber(state.AutoShootDelay) or 0.45,
                0,
                3
            )
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    if state.HitboxAccumulator >= 0.05 then
        ...
        if state.HitboxExpander and not state.InLobby then
            refreshExpandedHitboxes()
        elseif next(runtime.HitboxOriginal) ~= nil then
            restoreAllExpandedHitboxes()
        end
    end
...
UIElements.SliAutoShootDelay = Tabs.Aim:Slider({
    ...
    Desc = "Espera antes de disparar al siguiente enemigo.",
    Step = 0.05,
    Value = {
        Min = 0,
        Max = 3,
    ...
    Callback = function(value)
        state.AutoShootDelay =
            math.clamp(tonumber(value) or 0.45, 0, 3)
    end,
})
...
Tabs.Vis:Section({Title = "Hitbox Expander"})

UIElements.TogHitboxExpander = Tabs.Vis:Toggle({
...
UIElements.SliHitboxSize = Tabs.Vis:Slider({
...
UIElements.TogHitboxVisible = Tabs.Vis:Toggle({
