-- XeroHub | MVSD | Kev
-- UI DUELS + Silent Aim + Auto Shoot + Rapid Fire + Round Start Spam + ESP + Modo Fantasma.
...
local state = {
    ...
    RoundStartSpamDuration = 4,
    RoundStartSpamAccumulator = 0,
    RoundStartSpamActiveUntil = 0,
    RoundStartSpamLastMatchId = nil,
...
local function chooseAutoShootTarget()
    ...
    for i = 1, #runtime.PlayerList do
        ...
        if plr ~= player and isEnemy(plr) then
            ...
            then
                for j = 1, #runtime.AutoShootPartNames do
                    local body = char:FindFirstChild(runtime.AutoShootPartNames[j])
                    local hitbox = resolveActualHitbox(body)

                    if hitbox and hitbox.Parent then
                        -- "Visible" aquí significa:
                        -- 1) la parte está realmente dentro de la pantalla,
                        -- 2) no hay una pared/objeto bloqueándola.
...
local function autoShootOneShot()
    ...
    if ok then
        -- MVSD es one-shot; damos tiempo a que replique Humanoid.Died
        -- antes de volver a elegir al mismo jugador.
...
local function beginRoundStartSpam()
    if not state.RoundStartSpam or state.InLobby then
        return
    end

    local matchId = state.MatchId
    if matchId == nil then
        return
    end

    state.RoundStartSpamLastMatchId = matchId
    state.RoundStartSpamAccumulator = 0
    state.RoundStartSpamActiveUntil =
        os.clock() + math.clamp(
            tonumber(state.RoundStartSpamDuration) or 4,
            1,
            8
        )
end
...
runtime.BeginRoundStartSpam = beginRoundStartSpam
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    if state.LobbyTimer >= 0.35 then
        ...
        if state.InLobby and not wasLobby then
            ...
            state.RoundStartSpamAccumulator = 0
            state.RoundStartSpamActiveUntil = 0
            state.RoundStartSpamLastMatchId = nil
    ...
    -- Super spam al comenzar la ronda. Funciona sin Tool equipado.
    if state.RoundStartSpam
        and not state.InLobby
        and state.RoundStartSpamActiveUntil > os.clock()
    then
        ...
        if state.RoundStartSpamActiveUntil > 0
            and state.RoundStartSpamActiveUntil <= os.clock()
        then
            state.RoundStartSpamActiveUntil = 0
        end
...
UIElements.TogAutoShoot = Tabs.Aim:Toggle({
    Title = "Auto Shoot",
    Desc = "Dispara solo cuando alguna parte visible del enemigo aparece. Requiere la gun equipada.",
    Value = false,
    Callback = function(value)
...
Tabs.Aim:Section({Title = "Round Start Spam"})

UIElements.TogRoundStartSpam = Tabs.Aim:Toggle({
    Title = "Super spam al iniciar ronda",
    Desc = "Usa ShootGun aunque la gun todavía no se pueda equipar.",
    Value = false,
    Callback = function(value)
        state.RoundStartSpam = value
        state.RoundStartSpamAccumulator = 0

        if not value then
            state.RoundStartSpamActiveUntil = 0
            state.RoundStartSpamLastMatchId = nil
        elseif not state.InLobby then
            beginRoundStartSpam()
        end
    end,
})

UIElements.SliRoundStartSpamRate = Tabs.Aim:Slider({
    Title = "Potencia inicial",
    Desc = "ShootGun por segundo durante el inicio.",
    Step = 1,
    Value = {
        Min = 5,
        Max = 60,
...
UIElements.SliRoundStartSpamDuration = Tabs.Aim:Slider({
    Title = "Duración del super spam",
    Desc = "Segundos que insiste al comenzar la ronda.",
    Step = 0.5,
    Value = {
        Min = 1,
        Max = 8,
        Default = state.RoundStartSpamDuration,
    },
    Callback = function(value)
        state.RoundStartSpamDuration =
            math.clamp(tonumber(value) or 4, 1, 8)
    end,
})
...
local function serializeConfig(name)
    return {
        ...
        Sliders = {
            ...
            RoundStartSpamDuration = state.RoundStartSpamDuration,
        },
        Colors = {
            ESP = {
                R = state.ESPColor.R,
...
local function applyConfig(decoded)
    ...
    if toggles.RoundStartSpam ~= nil then
        state.RoundStartSpam = toggles.RoundStartSpam == true
        state.RoundStartSpamAccumulator = 0
        state.RoundStartSpamActiveUntil = 0
        state.RoundStartSpamLastMatchId = nil
        setElement(UIElements.TogRoundStartSpam, state.RoundStartSpam)
    end
    ...
    if sliders.RoundStartSpamDuration ~= nil then
        state.RoundStartSpamDuration = math.clamp(
            tonumber(sliders.RoundStartSpamDuration) or state.RoundStartSpamDuration,
            1,
            8
        )
        setElement(
            UIElements.SliRoundStartSpamDuration,
            state.RoundStartSpamDuration
        )
    end
