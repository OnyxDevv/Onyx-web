...
local function isValidHitboxTarget(plr)
    ...
    if data.Character:FindFirstChildOfClass("ForceField") then
        return false
    end

    return true, data.Character, data.Humanoid, data.Root
end
...
local function ensureHitbox(plr)
    ...
    if fakeHitbox.Transparency ~= 1 then
        fakeHitbox.Transparency = 1
    end
    ...
    if box then
        ...
        local wantedTransparency = state.HitboxInvisible
            and 1
            or math.clamp(tonumber(state.HitboxTransparency) or 0.6, 0, 1)
...
print("[XeroHub] MVSD cargado | Hitbox TEST | Skyboxes | Sonidos | Silent/ESP | Noclip | Mobile Fly")
