-- XeroHub | MVSD | Kev
-- UI DUELS + Silent Aim + Rapid Fire AutoEquip Ready + ESP + Modo Fantasma.
...
local state = {
    ...
    RapidFire = false,
    RapidFireRate = 16,
    RapidFireVisuals = true,
    RapidFireAccumulator = 0,

    -- Fase pre-equip del mismo Rapid Fire.
    RapidFirePreAccumulator = 0,
    RapidFirePreRate = 45,

    -- Autoequip: espera a que el Tool parezca realmente desbloqueado.
    RapidFireRoundBeganAt = 0,
    RapidFireEquipRetryAt = 0,
    RapidFireCandidateTool = nil,
    RapidFireCandidateSeenAt = 0,
    RapidFireSawLockedTool = false,
...
local function findRapidFireBackpackTool()
    local backpack = player:FindFirstChildOfClass("Backpack")
    if not backpack then
        return nil
    end

    local preferred = backpack:FindFirstChild("Default")
    ...
    for _, obj in ipairs(backpack:GetChildren()) do
        if obj:IsA("Tool") then
            local fire = obj:FindFirstChild("Fire", true)
            if fire and fire:IsA("Sound") then
                return obj
            end
        end
...
local function resetRapidFireEquipState()
    state.RapidFireEquipRetryAt = 0
    state.RapidFireCandidateTool = nil
    state.RapidFireCandidateSeenAt = 0
    state.RapidFireSawLockedTool = false
end

local function tryEquipRapidFireWhenReady()
    if not state.RapidFire or state.InLobby then
        return false
    end

    -- Ya está equipada de verdad.
    local equipped = getRapidFireTool()
    if equipped then
        return true
    end
    ...
    local tool = findRapidFireBackpackTool()
    if not tool then
        state.RapidFireCandidateTool = nil
        state.RapidFireCandidateSeenAt = 0
        return false
    ...
    if state.RapidFireCandidateTool ~= tool then
        state.RapidFireCandidateTool = tool
        state.RapidFireCandidateSeenAt = now
        state.RapidFireSawLockedTool = tool.Enabled == false
    elseif tool.Enabled == false then
        -- Si MVSD usa Tool.Enabled para el lock inicial, esperamos el cambio
        -- a true en vez de intentar equiparla antes de tiempo.
        state.RapidFireSawLockedTool = true
        return false
    end
    ...
    -- Dos formas de considerarla lista:
    -- 1) vimos Enabled=false y ahora ya está true;
    -- 2) nunca vimos el lock, pero el Tool lleva un rato estable en Backpack
    --    y ya pasó el arranque inicial de la ronda.
    local unlockedTransition = state.RapidFireSawLockedTool
    local stableInBackpack =
        now - (state.RapidFireCandidateSeenAt or now) >= 0.35
        and now - (state.RapidFireRoundBeganAt or now) >= 1.25

    if not unlockedTransition and not stableInBackpack then
        return false
    end

    if now < (state.RapidFireEquipRetryAt or 0) then
        return false
    end

    -- Si falla, no lo spameamos: dejamos que MVSD avance su estado
    -- y probamos otra vez más adelante.
    state.RapidFireEquipRetryAt = now + 0.45
...
local function rapidFireOneShot()
    ...
    if state.RapidFireVisuals then
...
local function rapidFirePreEquipOneShot()
    if not state.RapidFire or state.InLobby then
        return false
    end
...
runtime.RapidFirePreEquipOneShot = rapidFirePreEquipOneShot
runtime.TryEquipRapidFireWhenReady = tryEquipRapidFireWhenReady
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    if state.LobbyTimer >= 0.35 then
        ...
        if state.InLobby and not wasLobby then
            ...
            state.RapidFirePreAccumulator = 0
            state.RapidFireRoundBeganAt = 0
            resetRapidFireEquipState()
        elseif not state.InLobby
            and (wasLobby or oldMatchId ~= state.MatchId)
        then
            state.RapidFireAccumulator = 0
            state.RapidFirePreAccumulator = 0
            state.RapidFireRoundBeganAt = os.clock()
            resetRapidFireEquipState()
        end
    end
    ...
    -- Rapid Fire UNIFICADO, pero preservando las dos rutas que ya funcionan.
    if state.RapidFire and not state.InLobby then
        local equippedTool = getRapidFireTool()

        if equippedTool then
            -- FASE 2: arma realmente equipada.
            -- Usa rapidFireOneShot() ORIGINAL, sin tocar su FireServer.
            state.RapidFirePreAccumulator = 0
            state.RapidFireAccumulator =
                state.RapidFireAccumulator + dt

            local rate = math.clamp(
                tonumber(state.RapidFireRate) or 16,
                1,
                30
            )
            ...
            if state.RapidFireAccumulator >= interval then
                state.RapidFireAccumulator =
                    state.RapidFireAccumulator - interval

                rapidFireOneShot()
            end
        else
            -- FASE 1: MVSD todavía no deja usar/equipar la gun.
            -- No forzamos el Tool antes de tiempo. Conservamos el spam
            -- pre-equip original mientras esperamos que quede listo.
            state.RapidFireAccumulator = 0

            tryEquipRapidFireWhenReady()

            state.RapidFirePreAccumulator =
                state.RapidFirePreAccumulator + dt

            local preRate = math.clamp(
                tonumber(state.RapidFirePreRate) or 45,
                5,
                60
            )
            local preInterval = 1 / preRate
            local shotsThisFrame = 0

            while state.RapidFirePreAccumulator >= preInterval
                and shotsThisFrame < 3
            do
                state.RapidFirePreAccumulator =
                    state.RapidFirePreAccumulator - preInterval

                rapidFirePreEquipOneShot()
                shotsThisFrame = shotsThisFrame + 1
            end

            if state.RapidFirePreAccumulator > preInterval * 3 then
                state.RapidFirePreAccumulator = preInterval * 3
            end
        end
    else
        state.RapidFireAccumulator = 0
...
UIElements.TogRapidFire = Tabs.Aim:Toggle({
    Title = "Rapid Fire",
    Desc = "Spam al inicio, autoequipa cuando MVSD lo permite y sigue disparando.",
    Value = false,
    Callback = function(value)
        ...
        if value and not state.InLobby then
            state.RapidFireRoundBeganAt = os.clock()
...
UIElements.SliRapidFireRate = Tabs.Aim:Slider({
    ...
    Desc = "Cadencia cuando la gun ya está equipada.",
    Step = 1,
    Value = {
        Min = 1,
        Max = 30,
    ...
    Callback = function(value)
        state.RapidFireRate =
            math.clamp(tonumber(value) or 16, 1, 30)
        state.RapidFireAccumulator = 0
    end,
})

UIElements.TogRapidFireVisuals = Tabs.Aim:Toggle({
    Title = "Efectos de Rapid Fire",
    Desc = "Reproduce sonido, BulletBeam y animación Shoot.",
    Value = true,
    Callback = function(value)
        state.RapidFireVisuals = value
    end,
})
...
local function serializeConfig(name)
    return {
        ...
        Toggles = {
            ...
            RapidFireVisuals = state.RapidFireVisuals,
            ESP = state.ESP,
...
local function applyConfig(decoded)
    ...
    if toggles.RapidFire ~= nil then
        state.RapidFire = toggles.RapidFire == true
        state.RapidFireAccumulator = 0
        state.RapidFirePreAccumulator = 0
        resetRapidFireEquipState()
        setElement(UIElements.TogRapidFire, state.RapidFire)
    end
    if toggles.RapidFireVisuals ~= nil then
        state.RapidFireVisuals = toggles.RapidFireVisuals == true
        setElement(UIElements.TogRapidFireVisuals, state.RapidFireVisuals)
    end
...
print("[XeroHub] MVSD cargado | Rapid Fire Auto | Skyboxes | Sonidos | Silent/ESP | Noclip | Mobile Fly")
