-- XeroHub | MVSD | Kev
-- UI DUELS + Silent Aim + Rapid Fire AutoEquip + ESP + Modo Fantasma.
...
local function rapidFireOneShot()
    ...
    if state.RapidFireVisuals then
        ...
        -- La animación se reproduce cuando el arma ya logró equiparse.
        if tool then
            rapidPlayAnimation()
        end
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    -- Rapid Fire unificado:
    -- 1) intenta autoequipar el arma continuamente,
    -- 2) NO espera a que el equip funcione para mandar ShootGun,
    -- 3) en cuanto el Tool entra al Character añade los efectos/animación normales.
    if state.RapidFire and not state.InLobby then
        state.RapidFireEquipAccumulator =
            state.RapidFireEquipAccumulator + dt

        if state.RapidFireEquipAccumulator >= 0.10 then
            state.RapidFireEquipAccumulator = 0
        ...
        local rate = math.clamp(
            tonumber(state.RapidFireRate) or 30,
            1,
            60
        )
        local interval = 1 / rate

        -- Recupera un poco de cadencia si un frame fue lento,
        -- pero evita soltar un burst enorme tras un freeze.
        if state.RapidFireAccumulator > interval * 3 then
            state.RapidFireAccumulator = interval * 3
        end

        local shotsThisFrame = 0
        while state.RapidFireAccumulator >= interval
...
UIElements.SliRapidFireRate = Tabs.Aim:Slider({
    Title = "Potencia",
    Desc = "Disparos por segundo del Rapid Fire.",
    Step = 1,
    Value = {
        Min = 1,
        Max = 60,
    ...
    Callback = function(value)
        state.RapidFireRate =
            math.clamp(tonumber(value) or 30, 1, 60)
        state.RapidFireAccumulator = 0
    end,
})
...
local function applyConfig(decoded)
    ...
    if sliders.RapidFireRate ~= nil then
        state.RapidFireRate = math.clamp(tonumber(sliders.RapidFireRate) or state.RapidFireRate, 1, 60)
        state.RapidFireAccumulator = 0
