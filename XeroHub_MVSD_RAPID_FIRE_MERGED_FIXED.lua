-- XeroHub | MVSD | Kev
-- UI DUELS + Silent Aim + Rapid Fire Unificado + AutoEquip + ESP + Modo Fantasma.
...
local function mergedRapidFireOneShot()
    if not state.RapidFire or state.InLobby then
        return false
    end
...
runtime.RapidFireOneShot = mergedRapidFireOneShot
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    if state.LobbyTimer >= 0.35 then
        ...
        elseif not state.InLobby and wasLobby then
            -- Empieza a intentar equipar apenas arranca la ronda.
    ...
    -- Usa exactamente la ruta ShootGun que sí cuenta en tu build bueno.
    -- El autoequip es independiente: aunque el juego todavía no deje equipar,
    -- la rutina de disparo sigue usando el fallback que ya funcionaba.
    if state.RapidFire and not state.InLobby then
        ...
        -- Misma idea del build que sí funcionaba: conserva un pequeño
        -- margen acumulado, pero sólo manda un tiro por Heartbeat.
...
UIElements.TogRapidFire = Tabs.Aim:Toggle({
    Title = "Rapid Fire",
    Desc = "Autoequipa la gun y dispara solo durante toda la ronda.",
    Value = false,
    Callback = function(value)
...
print("[XeroHub] MVSD cargado | Rapid Fire FIXED + AutoEquip | Skyboxes | Sonidos | Silent/ESP | Noclip | Mobile Fly")
