-- XeroHub | MVSD | Kev
-- UI DUELS + Silent Aim + Rapid Fire StableNet AutoEquip + ESP + Modo Fantasma.
...
local state = {
    ...
    RapidFireEquipAccumulator = 0,
    RapidFireLastSoundAt = 0,
    RapidFireLastTracerAt = 0,
    RapidFireLastAnimAt = 0,
...
local function tryAutoEquipRapidFire()
    ...
    if not hum or hum.Health <= 0 or not tool then
        return false
    end
...
local function rapidFireOneShot()
    ...
    if state.RapidFireVisuals then
        ...
        -- Sonido: suficientemente rápido para conservar el efecto de ráfaga,
        -- pero no crea/reinicia audio 30-60 veces por segundo.
        if now - state.RapidFireLastSoundAt >= 0.065 then
            state.RapidFireLastSoundAt = now
            rapidPlayFireSound(fireSound or getRapidFireFallbackSound())
        end

        -- Animación: reiniciarla cada Remote es costoso e innecesario.
        if tool and now - state.RapidFireLastAnimAt >= 0.10 then
            state.RapidFireLastAnimAt = now
            rapidPlayAnimation()
        end

        -- Tracer: cada Beam crea varios Instances; lo limitamos a ~10/s.
        if now - state.RapidFireLastTracerAt >= 0.10 then
            state.RapidFireLastTracerAt = now
...
runtime.TryAutoEquipRapidFire = tryAutoEquipRapidFire
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    if state.LobbyTimer >= 0.35 then
        ...
        if state.InLobby and not wasLobby then
            ...
            state.RapidFireEquipAccumulator = 0
            state.RapidFireLastSoundAt = 0
            state.RapidFireLastTracerAt = 0
            state.RapidFireLastAnimAt = 0
        elseif not state.InLobby and wasLobby then
            -- Al entrar a ronda, empieza a intentar equipar inmediatamente.
            state.RapidFireAccumulator = 0
            state.RapidFireEquipAccumulator = 999
        end
    end
    ...
    -- Rapid Fire unificado y estable para red:
    -- nunca hace catch-up ni manda múltiples ShootGun en el mismo Heartbeat.
    if state.RapidFire and not state.InLobby then
        state.RapidFireEquipAccumulator =
            state.RapidFireEquipAccumulator + dt

        -- EquipTool repetido demasiado rápido también puede generar churn de replicación.
        if state.RapidFireEquipAccumulator >= 0.25 then
            state.RapidFireEquipAccumulator = 0
            tryAutoEquipRapidFire()
        end
        ...
        if state.RapidFireAccumulator >= interval then
            -- Sin acumulación atrasada: si hubo lag, se pierde ese tick en vez
            -- de mandar 2-3 Remotes juntos y empeorar la conexión.
            state.RapidFireAccumulator = 0
            rapidFireOneShot()
        end
    else
...
UIElements.TogRapidFire = Tabs.Aim:Toggle({
    Title = "Rapid Fire",
    Desc = "Autoequipa la gun y dispara automáticamente durante toda la ronda.",
    Value = false,
    Callback = function(value)
...
UIElements.SliRapidFireRate = Tabs.Aim:Slider({
    Title = "Potencia",
    Desc = "Disparos por segundo. 16 es el valor recomendado para estabilidad.",
    Step = 1,
    Value = {
        Min = 1,
        Max = 20,
    ...
    Callback = function(value)
        state.RapidFireRate =
            math.clamp(tonumber(value) or 16, 1, 20)
        state.RapidFireAccumulator = 0
    end,
})
...
UIElements.TogRapidFireVisuals = Tabs.Aim:Toggle({
    ...
    Desc = "Sonido, BulletBeam y animación cuando el arma está equipada.",
    Value = true,
    Callback = function(value)
...
local function applyConfig(decoded)
    ...
    if sliders.RapidFireRate ~= nil then
        state.RapidFireRate = math.clamp(tonumber(sliders.RapidFireRate) or state.RapidFireRate, 1, 20)
        state.RapidFireAccumulator = 0
