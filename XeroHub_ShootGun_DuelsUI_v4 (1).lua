...
local function estaEnLobby()
    ...
    -- ForceField suele usarse durante lobby/spawn protegido.
    if char:FindFirstChildOfClass("ForceField") then
        return true, "ForceField"
    end

    -- Flags explícitos de lobby tienen prioridad.
    for i = 1, #LOBBY_TRUE_ATTRS do
        local attr = LOBBY_TRUE_ATTRS[i]
        if boolAttribute(player, attr) == true or boolAttribute(char, attr) == true then
            return true, "Atributo " .. attr
        end
    end

    -- Si existe un flag explícito de partida y está false, cuenta como lobby.
    for i = 1, #ROUND_TRUE_ATTRS do
        local attr = ROUND_TRUE_ATTRS[i]
        local pValue = boolAttribute(player, attr)
        local cValue = boolAttribute(char, attr)

        if pValue ~= nil then
            return not pValue, "Atributo " .. attr
        end
        if cValue ~= nil then
            return not cValue, "Atributo " .. attr
        end
    end

    -- Team con nombre típico de lobby/espectador.
    if player.Team and textContainsLobbyWord(player.Team.Name) then
        return true, "Team " .. player.Team.Name
    end

    -- Neutral suele ser lobby en muchos round-games; sólo lo usamos cuando
    -- además no hay herramienta equipada para evitar falsos positivos en partida.
    local tool = char:FindFirstChildOfClass("Tool")
    if player.Neutral == true and not tool then
        local team = player.Team
        if not team then
            -- Sin señal concluyente: no bloqueamos por defecto.
        elseif textContainsLobbyWord(team.Name) then
            return true, "Neutral/Lobby"
        end
    end
            ...
            if self == workspace then
                if method == "Raycast" then
                    ...
                    then
                        ...
                        if not camera
                            or (origin - camera.CFrame.Position).Magnitude > 1
                        then
                ...
                then
                    ...
                    if typeof(ray) == "Ray" and ray.Direction.Magnitude > 5 then
                        ...
                        if not camera
                            or (ray.Origin - camera.CFrame.Position).Magnitude > 1
                        then
...
UIElements.TogTeamCheck = Tabs.Vis:Toggle({
    Title = "Team Check",
    Desc = "Ignora jugadores de tu mismo equipo.",
    Value = state.TeamCheck,
    Callback = function(value)
...
local function applyConfig(decoded)
    ...
    if toggles.TeamCheck ~= nil then
        state.TeamCheck = toggles.TeamCheck == true
        setElement(UIElements.TogTeamCheck, state.TeamCheck)
    end
...
print("[XeroHub] MVSD cargado | UI DUELS | Selector 2D | Configs | Silent Aim optimizado | Auto Shoot independiente | ESP 10 Hz")
