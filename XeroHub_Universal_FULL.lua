...
-- ==========================================
-- SILENT AIM HOOK · UNIVERSAL CONSERVADOR
-- ==========================================

-- A generic game can use Workspace:Raycast for movement, camera, footsteps,
-- interaction, etc. Therefore we only touch a ray during a very short, real
-- weapon-input window and only when its origin is near our camera/weapon.
...
local function recentUniversalShot()
    return os.clock() - (runtime.LastManualShotInputAt or 0) <= 0.28
end

local function getUniversalToolOrigin()
...
local function looksLikeWeaponOrigin(origin)
    if typeof(origin) ~= "Vector3" or not recentUniversalShot() then
        return false
    end

    local camera = workspace.CurrentCamera

    if camera and (origin - camera.CFrame.Position).Magnitude <= 2.5 then
        return true
    end

    local toolOrigin = getUniversalToolOrigin()

    if toolOrigin and (origin - toolOrigin).Magnitude <= 8 then
        return true
    end
...
local aimHookState = runtimeEnv.__XERO_UNIVERSAL_AIM_STATE
...
if not aimHookState then
    ...
    runtimeEnv.__XERO_UNIVERSAL_AIM_STATE = aimHookState

    local oldNamecall
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        local hookState = runtimeEnv.__XERO_UNIVERSAL_AIM_STATE
        local target = hookState and hookState.Target
        local owner = hookState and hookState.Runtime
        ...
        then
            ...
            if self == workspace and method == "Raycast" then
                ...
                if typeof(origin) == "Vector3"
                    ...
                    and looksLikeWeaponOrigin(origin)
                then
                    ...
                    then
                        -- Preserve the weapon's original ray length. Universal builds
                        -- should not fabricate a 5000-stud ray for every game.
                        return oldNamecall(
                            self,
                            origin,
            ...
            then
                ...
                if typeof(ray) == "Ray"
                    ...
                    and looksLikeWeaponOrigin(ray.Origin)
                then
                    local delta = target.Position - ray.Origin
                    local originalLength = ray.Direction.Magnitude
        ...
        elseif hookState and target and not target.Parent then
            hookState.Target = nil
        end
...
print("[XeroHub] UNIVERSAL cargado | UI Xero | Silent/ESP/Hitbox | Movimiento | by Kev")
