...
local state = {
    ...
    UniversalContextKey = nil,
    UniversalContextValue = nil,
    UniversalEnemyKind = "FFA",
    UniversalEnemyKey = nil,
    UniversalEnemyLabel = "FFA (auto)",
    UniversalSchemeTimer = 1,
...
-- ==========================================
-- UNIVERSAL ROUND / TEAM DETECTION
-- ==========================================

local UNIVERSAL_CONTEXT_KEYS = {
    "Match", "MatchId", "match", "matchId",
    "Round", "RoundId", "round", "roundId",
    "Arena", "ArenaId", "arena", "arenaId",
    "Duel", "DuelId", "duel", "duelId",
}

local UNIVERSAL_TEAM_ATTR_KEYS = {
...
local function localCharacterActive()
...
local function detectUniversalContext()
    state.UniversalContextKey = nil
    state.UniversalContextValue = nil

    for i = 1, #UNIVERSAL_CONTEXT_KEYS do
        local key = UNIVERSAL_CONTEXT_KEYS[i]
        ...
        if mine ~= nil then
            ...
            for j = 1, #runtime.PlayerList do
                local plr = runtime.PlayerList[j]
                if plr then
                    local value = plr:GetAttribute(key)
                    if value ~= nil then
            ...
            -- Only isolate by Match/Round/etc when the server actually exposes
            -- more than one simultaneous context. This avoids false isolation
            -- in games where "Round" is just a shared round number.
            if distinct >= 2 then
                state.UniversalContextKey = key
                state.UniversalContextValue = mine
                return
            end
        end
    end
...
local function sameUniversalContext(plr)
    local key = state.UniversalContextKey
    if not key then return true end

    local mine = state.UniversalContextValue
    local theirs = plr and plr:GetAttribute(key)

    return theirs ~= nil and theirs == mine
end

local function detectUniversalEnemyScheme()
    detectUniversalContext()

    -- 1) Native Roblox Teams.
    if player.Team ~= nil then
        ...
        for i = 1, #runtime.PlayerList do
            ...
            if plr and sameUniversalContext(plr) and plr.Team ~= nil then
                if not distinct[plr.Team] then
                    distinct[plr.Team] = true
                    count += 1
                end
            end
        end
        ...
        if count >= 2 then
            state.UniversalEnemyKind = "Team"
            state.UniversalEnemyKey = nil
            state.UniversalEnemyLabel = "Player.Team"
            return
        end
    end

    -- 2) TeamColor, but only when Roblox considers us non-neutral.
    if player.TeamColor ~= nil and player.Neutral == false then
        local distinct = {}
        local count = 0
        ...
        for i = 1, #runtime.PlayerList do
            ...
            if plr and sameUniversalContext(plr) and plr.TeamColor ~= nil then
                local token = tostring(plr.TeamColor.Number)

                if not distinct[token] then
                    distinct[token] = true
                    count += 1
                end
            end
        ...
        if count >= 2 then
            state.UniversalEnemyKind = "TeamColor"
            state.UniversalEnemyKey = nil
            state.UniversalEnemyLabel = "Player.TeamColor"
            return
        end
    end

    -- 3) Common custom team/faction/squad attributes.
    for i = 1, #UNIVERSAL_TEAM_ATTR_KEYS do
        local key = UNIVERSAL_TEAM_ATTR_KEYS[i]
        local mine = player:GetAttribute(key)
        ...
        if mine ~= nil then
            ...
            for j = 1, #runtime.PlayerList do
                ...
                if plr and sameUniversalContext(plr) then
                    local value = plr:GetAttribute(key)
            ...
            if count >= 2 then
                state.UniversalEnemyKind = "Attribute"
                state.UniversalEnemyKey = key
                state.UniversalEnemyLabel = "Attribute:" .. key
                return
            end
        end
    end

    -- 4) No team structure found: assume FFA.
    state.UniversalEnemyKind = "FFA"
    state.UniversalEnemyKey = nil
    state.UniversalEnemyLabel = "FFA (auto)"
end

local function estaEnLobby()
    local active, reason = localCharacterActive()

    if not active then
        return true, reason
    end

    return false, state.UniversalEnemyLabel
end

local function refreshLobbyState()
    detectUniversalEnemyScheme()
...
local function isEnemy(plr)
    ...
    if not sameUniversalContext(plr) then
        return false
    end

    local kind = state.UniversalEnemyKind
    ...
    if kind == "Attribute" then
        local key = state.UniversalEnemyKey
        local mine = key and player:GetAttribute(key)
        local theirs = key and plr:GetAttribute(key)
...
-- ==========================================
-- SILENT AIM HOOK · UNIVERSAL V2 / CRASH FIX
-- ==========================================

-- Neutraliza hooks universales viejos que pueden seguir instalados en la sesión.
do
    local oldUniversal = runtimeEnv.__XERO_UNIVERSAL_AIM_STATE
    if type(oldUniversal) == "table" then
        oldUniversal.Target = nil
        oldUniversal.Mouse = nil
        oldUniversal.Runtime = {Alive = false}
    end

    local oldLab = runtimeEnv.__XERO_UNIVERSAL_SILENT_HOOK
    if type(oldLab) == "table" then
        oldLab.Target = nil
        oldLab.Mouse = nil
        oldLab.Runtime = {Alive = false}
        oldLab.State = {Enabled = false}
    end
end

runtime.LastManualShotInputAt = 0
runtime.UniversalToolBindings = setmetatable({}, {__mode = "k"})

local aimHookState = runtimeEnv.__XERO_UNIVERSAL_AIM_STATE_V2
...
if type(aimHookState) ~= "table" then
    ...
    runtimeEnv.__XERO_UNIVERSAL_AIM_STATE_V2 = aimHookState
else
    aimHookState.Target = nil
    aimHookState.TargetInstance = nil
    aimHookState.TargetPosition = nil
    aimHookState.TargetCFrame = nil
    aimHookState.Mouse = mouse
    aimHookState.Runtime = runtime
    aimHookState.ShotUntil = 0
    aimHookState.CameraPosition = nil
    aimHookState.ToolPosition = nil
    aimHookState.ToolActive = false
end

local function setAimHookTarget(part)
    aimHookState.Target = part
    aimHookState.TargetInstance = part

    if part and part.Parent then
        aimHookState.TargetPosition = part.Position
        aimHookState.TargetCFrame = part.CFrame
    else
        aimHookState.TargetPosition = nil
        aimHookState.TargetCFrame = nil
    end
...
local function getToolOriginOutsideHook(tool)
...
local function markUniversalShot()
    ...
    aimHookState.ShotUntil = runtime.LastManualShotInputAt + 0.30
    aimHookState.ToolActive = true
    aimHookState.ToolPosition = getToolOriginOutsideHook(tool)
end

runtime.Track(RunService.RenderStepped:Connect(function()
    local camera = workspace.CurrentCamera
    aimHookState.CameraPosition = camera and camera.CFrame.Position or nil
end))
...
local function bindUniversalTool(tool)
    if not tool
        or not tool:IsA("Tool")
        or runtime.UniversalToolBindings[tool]
    then
        return
    end
    ...
    runtime.UniversalToolBindings[tool] = connection
    runtime.Track(connection)
end

local function scanUniversalTools(container)
    if not container then return end

    for _, child in ipairs(container:GetChildren()) do
        if child:IsA("Tool") then
            bindUniversalTool(child)
        end
    end
end
...
local function bindUniversalCharacterTools(char)
    if not char then return end

    state.EquippedTool = nil
    aimHookState.ToolActive = false
    aimHookState.ToolPosition = nil
    scanUniversalTools(char)
...
if player.Character then
    bindUniversalCharacterTools(player.Character)
end

runtime.Track(player.CharacterAdded:Connect(bindUniversalCharacterTools))

if not aimHookState.Hooked then
    aimHookState.Hooked = true

    local oldNamecall
    oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        local hookState = runtimeEnv.__XERO_UNIVERSAL_AIM_STATE_V2

        -- IMPORTANTE:
        -- Dentro del metamethod NO llamamos FindFirstChild/GetDescendants/getEquippedTool
        -- ni consultamos propiedades del target. Todo llega cacheado desde fuera.
        if hookState
            and hookState.Runtime
            and hookState.Runtime.Alive
            and hookState.ToolActive
            and hookState.TargetPosition
            and os.clock() <= (hookState.ShotUntil or 0)
            and not checkcaller()
        then
            ...
            if self == workspace and method == "Raycast" then
                ...
                then
                    local cameraPos = hookState.CameraPosition
                    local toolPos = hookState.ToolPosition

                    local nearCamera = cameraPos
                        and (origin - cameraPos).Magnitude <= 2.5

                    local nearTool = toolPos
                        and (origin - toolPos).Magnitude <= 8

                    if nearCamera or nearTool then
                        local targetPosition = hookState.TargetPosition
                        local delta = targetPosition - origin
                        local originalLength = direction.Magnitude

                        if delta.Magnitude > 0.01
                            and delta.Magnitude <= originalLength + 2
                        then
                            return oldNamecall(
                                self,
                                origin,
                                delta.Unit * originalLength,
                                params
                            )
                        end
                    end
    ...
    oldIndex = hookmetamethod(game, "__index", function(object, key)
        ...
        -- object == Mouse se comprueba ANTES de cualquier otra lógica.
        -- Así una lectura de Part.Parent/Position jamás entra en recursión.
        if hookState
            and object == hookState.Mouse
            and hookState.Runtime
            and hookState.Runtime.Alive
            and hookState.ToolActive
            and os.clock() <= (hookState.ShotUntil or 0)
        ...
        then
            if key == "Hit" or key == "hit" then
                local cf = hookState.TargetCFrame
                if cf then
                    return cf
                end
            elseif key == "Target" or key == "target" then
                local instance = hookState.TargetInstance
                if instance then
                    return instance
                end
            end
...
-- ==========================================
-- HITBOX EXPANDER · PORT LIGERO DE DUELS
-- ==========================================

runtime.HitboxByCharacter = setmetatable({}, {__mode = "k"})
runtime.HitboxAdornment = setmetatable({}, {__mode = "k"})

local function destroyHitboxForCharacter(char)
    if not char then return end

    local fakeHitbox = runtime.HitboxByCharacter[char]
    runtime.HitboxByCharacter[char] = nil

    if fakeHitbox then
        runtime.HitboxAdornment[fakeHitbox] = nil

        pcall(function()
            fakeHitbox.CanQuery = false
            fakeHitbox.CanTouch = false
            fakeHitbox.CanCollide = false
        end)

        safeDestroy(fakeHitbox)
    end

    -- Limpia una hitbox legacy si la tabla se perdió por respawn/reload.
    if char.Parent then
        for _, child in ipairs(char:GetChildren()) do
            if child:GetAttribute("XeroMVSDHitbox") == true then
                safeDestroy(child)
            end
        end
    end
...
local function clearHitboxes()
    for char, fakeHitbox in pairs(runtime.HitboxByCharacter) do
        runtime.HitboxByCharacter[char] = nil
    ...
    -- Barrido corto sólo al apagar/entrar lobby/cleanup.
    for i = 1, #runtime.PlayerList do
        local plr = runtime.PlayerList[i]
...
local function isValidHitboxTarget(plr)
    if not state.Hitbox
        or state.InLobby
        or not plr
        or plr == player
...
local function ensureHitbox(plr)
    local valid, char, hum, hrp = isValidHitboxTarget(plr)

    if not valid then
        if plr and plr.Character then
            destroyHitboxForCharacter(plr.Character)
        end
        return nil
    end
    ...
    if not fakeHitbox or fakeHitbox.Parent ~= char then
        -- Recupera uno existente si quedó de una pasada previa.
        for _, child in ipairs(char:GetChildren()) do
            if child:GetAttribute("XeroMVSDHitbox") == true then
                fakeHitbox = child
                break
        ...
        if not fakeHitbox then
            ...
            fakeHitbox:SetAttribute("XeroMVSDHitbox", true)
            fakeHitbox.Shape = Enum.PartType.Block
            fakeHitbox.Size = Vector3.new(
                state.HitboxSize,
                state.HitboxSize,
                state.HitboxSize
            )
            ...
            fakeHitbox.CanCollide = false
            fakeHitbox.CanTouch = false
            fakeHitbox.CanQuery = true
            fakeHitbox.CastShadow = false
            fakeHitbox.Color = state.ESPColor
            fakeHitbox.Material = Enum.Material.ForceField
            fakeHitbox.Transparency = state.HitboxInvisible
                and 1
                or math.clamp(
                    tonumber(state.HitboxTransparency) or 0.6,
                    0,
                    1
                )
            ...
            weld.Name = "XeroHitboxWeld"
            weld.Part0 = hrp
            weld.Part1 = fakeHitbox
            weld.Parent = fakeHitbox

            local box = Instance.new("BoxHandleAdornment")
            box.Name = "XeroHitboxBox"
            box.Adornee = fakeHitbox
            box.AlwaysOnTop = true
            box.ZIndex = 5
            box.Size = fakeHitbox.Size
            box.Color3 = state.ESPColor
            box.Transparency = state.HitboxInvisible
                and 1
                or state.HitboxTransparency
            box.Visible = not state.HitboxInvisible
            box.Parent = fakeHitbox

            runtime.HitboxAdornment[fakeHitbox] = box
        end

        runtime.HitboxByCharacter[char] = fakeHitbox
    end

    local size = math.clamp(tonumber(state.HitboxSize) or 10, 2, 100)
    local targetSize = Vector3.new(size, size, size)

    if fakeHitbox.Size ~= targetSize then
        fakeHitbox.Size = targetSize
    end

    if fakeHitbox.CanQuery ~= true then
        fakeHitbox.CanQuery = true
    end

    if fakeHitbox.CanCollide ~= false then
        fakeHitbox.CanCollide = false
    end

    if fakeHitbox.CanTouch ~= false then
        fakeHitbox.CanTouch = false
    end

    local wantedTransparency = state.HitboxInvisible
        and 1
        or math.clamp(
            tonumber(state.HitboxTransparency) or 0.6,
            0,
    ...
    if fakeHitbox.Color ~= state.ESPColor then
        fakeHitbox.Color = state.ESPColor
    end

    if fakeHitbox.Transparency ~= wantedTransparency then
        fakeHitbox.Transparency = wantedTransparency
    end

    local box = runtime.HitboxAdornment[fakeHitbox]
    if not box or box.Parent ~= fakeHitbox then
        box = fakeHitbox:FindFirstChild("XeroHitboxBox")
        runtime.HitboxAdornment[fakeHitbox] = box
    end
    ...
    if box then
        ...
        box.Color3 = state.ESPColor
        box.Transparency = wantedTransparency
        box.Visible = not state.HitboxInvisible
    end
...
runtime.HitboxCleanup = function()
    state.Hitbox = false
    state.HitboxAccumulator = 0
    state.HitboxHadActive = false
    clearHitboxes()
end
...
runtime.Track(Players.PlayerRemoving:Connect(function(plr)
    local char = plr.Character
    if char then
        destroyHitboxForCharacter(char)
    end
    runtime.CharacterCache[plr] = nil
    removeESP(plr)
end))
...
runtime.Track(RunService.Heartbeat:Connect(function(dt)
    ...
    -- Hitbox a ~6.6 Hz, igual que el intervalo pesado de DUELS.
    if state.Hitbox and not state.InLobby then
        state.HitboxHadActive = true
        state.HitboxAccumulator = state.HitboxAccumulator + dt

        if state.HitboxAccumulator >= 0.15 then
            state.HitboxAccumulator = 0
            ...
            -- Limpia Character viejos después de respawn/cambio de ronda.
            for char, fakeHitbox in pairs(runtime.HitboxByCharacter) do
                local owner = Players:GetPlayerFromCharacter(char)
                if not owner
                    or owner.Character ~= char
                    or not isValidHitboxTarget(owner)
                then
                    destroyHitboxForCharacter(char)
                elseif not fakeHitbox or fakeHitbox.Parent ~= char then
                    runtime.HitboxByCharacter[char] = nil
                end
            end
        end
...
local lobbyParagraph = Tabs.Inicio:Paragraph({
    Title = "Detección universal",
    Desc = "Analizando equipos y partida...",
})
...
UIElements.TogHitbox = Tabs.Aim:Toggle({
    ...
    Desc = "Expande la caja de impacto de los enemigos.",
    Value = false,
    Callback = function(value)
        state.Hitbox = value == true

        if not state.Hitbox then
            clearHitboxes()
            state.HitboxHadActive = false
        else
            state.HitboxAccumulator = 1

            task.delay(0.40, function()
                if not runtime.Alive or not state.Hitbox then
                    return
                end

                local count = 0

                for char, hitbox in pairs(runtime.HitboxByCharacter) do
                    if char
                        and char.Parent
                        and hitbox
                        and hitbox.Parent == char
                    then
                        count += 1
                    end
                end
                ...
                runtime.Notify(
                    "Hitbox activa en " .. tostring(count) .. " enemigo(s).",
                    {Title = "XeroHub · Hitbox"}
                )
            end)
        end
    end,
...
UIElements.TogHitboxInvisible = Tabs.Aim:Toggle({
    Title = "Hitbox Invisible",
    Desc = "Oculta las cajas de los enemigos.",
    Value = state.HitboxInvisible,
    Callback = function(value)
        state.HitboxInvisible = value == true
        state.HitboxAccumulator = 1
    end,
})
...
UIElements.SliHitbox = Tabs.Aim:Slider({
    ...
    Callback = function(value)
        state.HitboxSize = math.clamp(tonumber(value) or 10, 2, 50)
        state.HitboxAccumulator = 1
    end,
})
...
Tabs.Aim:Input({
    ...
    Callback = function(textValue)
        ...
        if number then
            state.HitboxSize = math.clamp(number, 2, 100)
            state.HitboxAccumulator = 1

            pcall(function()
                if number >= 2 and number <= 50 and UIElements.SliHitbox then
                    UIElements.SliHitbox:Set(number)
                end
            end)

            runtime.Notify(
                "Hitbox fijada en: " .. tostring(state.HitboxSize),
                {Title = "XeroHub · Hitbox"}
            )
        end
    end,
...
UIElements.SliHitboxTransparency = Tabs.Aim:Slider({
    Title = "Transparencia del Hitbox",
    Desc = "0 = sólido | 1 = invisible.",
    Step = 0.05,
    Value = {
        Min = 0,
        Max = 1,
    ...
    Callback = function(value)
        state.HitboxTransparency = math.clamp(
            tonumber(value) or 0.6,
            0,
            1
        )
        state.HitboxAccumulator = 1
        ...
        if lobbyParagraph and lobbyParagraph.SetDesc then
            if state.InLobby then
                lobbyParagraph:SetDesc(
                    "Inactivo · " .. tostring(state.LobbyReason)
                )
            else
                local target = state.SilentTargetPlayer
                local context = state.UniversalContextKey
                    and (" · " .. tostring(state.UniversalContextKey)
                        .. "=" .. tostring(state.UniversalContextValue))
                    or ""

                lobbyParagraph:SetDesc(
                    tostring(state.UniversalEnemyLabel)
                    .. context
                    .. (
                        target
...
runtimeEnv.__XERO_MVSD_HUB_CLEANUP = function()
    ...
    if aimHookState and aimHookState.Runtime == runtime then
        setAimHookTarget(nil)
        aimHookState.ToolActive = false
        aimHookState.ToolPosition = nil
        aimHookState.ShotUntil = 0
    ...
    if runtime.HitboxCleanup then
        pcall(runtime.HitboxCleanup)
    end
...
print("[XeroHub] UNIVERSAL V2 cargado | CrashFix Silent | UI Xero | by Kev")
