...
local SoundCatalogCore = {
    ...
    CATALOG_URL = "https://api.github.com/repos/OnyxDevv/Onyx-web/contents/sounds?ref=main",
    CACHE_FOLDER = "XeroHub/Sounds",
}
...
function runtime.Cleanup()
    ...
    if runtime.VisualSpoofCleanup then pcall(runtime.VisualSpoofCleanup) end
    if runtime.SoundCleanup then pcall(runtime.SoundCleanup) end
    if runtime.DeathEffectCleanup then pcall(runtime.DeathEffectCleanup) end
    if runtime.TextureCleanup then pcall(runtime.TextureCleanup) end
    ...
    -- Restaura los metamethods del aim si esta ejecución los instaló.
    -- Esto evita dejar hooks vivos al cerrar/re-ejecutar XeroHub.
    if runtime.UninstallAimHooks then
        pcall(runtime.UninstallAimHooks, true)
    end

    if runtime.AimState and runtime.AimState.Owner == runtime then
        runtime.AimState.Target = nil
        runtime.AimState.RoundActive = false
        runtime.AimState.Wanted = false
    ...
    if runtime.AppearanceSyncCleanup then
        pcall(runtime.AppearanceSyncCleanup)
    end
    if runtime.DestroyOutfitBrowser then
        pcall(runtime.DestroyOutfitBrowser)
    end
...
local weaponVolumeSlider
local killVolumeSlider
...
local function fallbackCatalog()
    return SoundCatalogCore.parseCatalog({
        {
            name = "anime-magic-sound-effect (1).mp3",
            type = "file",
            sha = "057d16da9c27ae50223aca105eccc7fc0cf7d362",
            download_url = "https://raw.githubusercontent.com/OnyxDevv/Onyx-web/main/sounds/anime-magic-sound-effect%20(1).mp3",
        },
    })
end
...
local function downloadSound(entry)
    ...
    if not hasFolders then
        localPath = "XeroHub_Sound_" .. localPath:match("([^/]+)$")
    end
...
local function activateEntry(entry, changedSelection, silent)
    ...
    task.spawn(function()
        ...
        pcall(function() replacement.Volume = soundState.Volume end)
...
local function previewEntry(entry, volume)
    if not entry then
    ...
    task.spawn(function()
        ...
        pcall(function() preview.Volume = math.clamp(tonumber(volume) or 1, 0, 1) end)
        local endedConnection
...
local function applyNativeDeathSound(sound)
    ...
    if killSoundState.NativeDeathOriginalIds[sound] == nil then
        ...
        local suppressedVolume = killSoundState.SuppressedDeathOriginalVolumes[sound]
        killSoundState.NativeDeathOriginalVolumes[sound] =
            suppressedVolume ~= nil and suppressedVolume or sound.Volume
    end
...
local function restoreNativeDeathSound(sound)
    ...
    if originalId == nil then return end
    local originalVolume = killSoundState.NativeDeathOriginalVolumes[sound]

    killSoundState.NativeDeathOriginalIds[sound] = nil
    killSoundState.NativeDeathAppliedIds[sound] = nil
    killSoundState.NativeDeathOriginalVolumes[sound] = nil
    ...
    if sound and sound.Parent then
        ...
        pcall(function()
            ...
            if originalVolume ~= nil then sound.Volume = originalVolume end
        end)
        killSoundState.Muting[sound] = nil
        killSoundState.NativeDeathUpdating[sound] = nil
...
local function armNativeDeathTarget(character, tool, forceExactTarget)
    ...
    local function watchDeathSound(sound)
        ...
        releaseSuppressedDeathSound(sound)
        killSoundState.Muting[sound] = true
        pcall(function() sound.Volume = killSoundState.Volume end)
        killSoundState.Muting[sound] = nil
...
local function playKillReplacement(humanoid)
    ...
    oneShot.Volume = killSoundState.Volume
...
local function setWeaponVolumeSliderSilently(percent)
    if not weaponVolumeSlider then return end
    soundState.SyncingVolumeSlider = true
    pcall(function() weaponVolumeSlider:Set(percent) end)
    soundState.SyncingVolumeSlider = false
end

local function setKillVolumeSliderSilently(percent)
    if not killVolumeSlider then return end
    killSoundState.SyncingVolumeSlider = true
    pcall(function() killVolumeSlider:Set(percent) end)
    killSoundState.SyncingVolumeSlider = false
end
...
local function activateKillEntry(entry, changedSelection, silent)
    ...
    task.spawn(function()
        ...
        pcall(function() replacement.Volume = killSoundState.Volume end)
...
function runtime.SerializeSoundConfig()
    ...
    return {
        Arma = {
            ...
            Volumen = math.floor((soundState.Volume or 1) * 100 + 0.5),
        },
        Muerte = {
            ...
            Volumen = math.floor((killSoundState.Volume or 1) * 100 + 0.5),
        },
    }
end
...
local function applySavedSoundConfig(data)
    ...
    local savedWeaponVolume = math.clamp(tonumber(weaponConfig.Volumen) or 100, 0, 100)
    local savedKillVolume = math.clamp(tonumber(killConfig.Volumen) or 100, 0, 100)
    soundState.Volume = savedWeaponVolume / 100
    killSoundState.Volume = savedKillVolume / 100
    setWeaponVolumeSliderSilently(savedWeaponVolume)
    setKillVolumeSliderSilently(savedKillVolume)
...
Tabs.Sonidos:Button({
    ...
    Callback = function() previewEntry(getSelectedEntry(), soundState.Volume) end,
})

weaponVolumeSlider = Tabs.Sonidos:Slider({
    Title = "Volumen de disparo",
    Desc = "Ajusta el volumen del sonido personalizado.",
    Step = 1,
    Value = {Min = 0, Max = 100, Default = 100},
    Callback = function(value)
        if soundState.SyncingVolumeSlider then return end
        local percent = math.clamp(tonumber(value) or 100, 0, 100)
        soundState.Volume = percent / 100
        if soundState.ReplacementSound and soundState.ReplacementSound.Parent then
            pcall(function() soundState.ReplacementSound.Volume = soundState.Volume end)
        end
    end,
})
UIElements.SliWeaponSoundVolume = weaponVolumeSlider
...
Tabs.Sonidos:Button({
    ...
    Desc = "Escucha el sonido seleccionado.",
    Callback = function() previewEntry(getSelectedKillEntry(), killSoundState.Volume) end,
})

killVolumeSlider = Tabs.Sonidos:Slider({
    Title = "Volumen de muerte",
    Desc = "Ajusta el volumen del sonido personalizado de muerte.",
    Step = 1,
    Value = {Min = 0, Max = 100, Default = 100},
    Callback = function(value)
        if killSoundState.SyncingVolumeSlider then return end
        local percent = math.clamp(tonumber(value) or 100, 0, 100)
        killSoundState.Volume = percent / 100

        if killSoundState.ReplacementSound and killSoundState.ReplacementSound.Parent then
            pcall(function() killSoundState.ReplacementSound.Volume = killSoundState.Volume end)
        end
        for oneShot in pairs(killSoundState.ActiveOneShots) do
            if oneShot and oneShot.Parent then
                pcall(function() oneShot.Volume = killSoundState.Volume end)
            end
        end
        for sound in pairs(killSoundState.NativeDeathOriginalIds) do
            if sound and sound.Parent then
...
UIElements.SliKillSoundVolume = killVolumeSlider
...
-- ==========================================
-- DEATH EFFECTS · SOLO KILLS DEL JUGADOR LOCAL
-- Integrado desde el laboratorio NativeDummy/NativeEnemies. ClanFlag eliminado.
-- ==========================================
runtime.InitDeathEffects = function()
    local Players = game:GetService("Players")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local HttpService = game:GetService("HttpService")
    local UserInputService = game:GetService("UserInputService")
        ...
        if requestFn then
            local ok, response = pcall(function()
                return requestFn({
                    ...
                    Method = "GET",
                    Headers = {
                        ["User-Agent"] = "XeroHub-DeathEffects-Integrated/1.0",
    ...
    -- Repo / cache
    -- ============================================================
    local CACHE_FOLDER = "XeroHub/DeathEffectsIntegratedR19"
    ...
    -- Integrated R19: no blocking all-effects preload at startup.
    ...
    -- ============================================================
    -- Integrated XeroHub controller
    -- Only confirmed LocalPlayer eliminations are allowed to consume a victim.
    -- ============================================================
    local effectState = {
        Enabled = false,
        Selected = nil,
        ...
        PendingDeaths = {},
        PendingByHumanoid = setmetatable({}, {__mode = "k"}),
        KillValues = setmetatable({}, {__mode = "k"}),
        KillSounds = setmetatable({}, {__mode = "k"}),
        RecentCredits = {},
        LastSignalAt = 0,
        LastSignalSource = nil,
        LastAppliedAt = 0,
        WarmToken = 0,
    }
    runtime.DeathEffects = effectState

    local PENDING_LIFETIME = 1.75
    local CREDIT_LIFETIME = 1.10
    local SIGNAL_DEDUPE_WINDOW = 0.30

    local function trackEffectConnection(connection)
        if connection then
            effectState.Connections[#effectState.Connections + 1] = connection
            runtime.Track(connection)
        end
        return connection
    end
    ...
    local function selectedEffect()
        return effectState.Selected
    end

    local function isEnemyPlayer(targetPlayer, character)
        if not targetPlayer or targetPlayer == LP or not character or not character.Parent then
            return false
        end

        -- Reuse XeroHub's own round/lobby/safe-zone filter. This keeps deaths from
        -- simultaneous arenas, lobby, voting and spectators out of the effect system.
        if type(isRoundDeathEligible) == "function" and not isRoundDeathEligible(targetPlayer, character) then
            return false
        end

        if LP.Neutral == true or targetPlayer.Neutral == true then
            return false
        end
        ...
        return LP.TeamColor ~= targetPlayer.TeamColor
    end

    local function refreshHumanoidMode(targetPlayer, humanoid)
        local info = effectState.TrackedHumanoids[humanoid]
        if not info or not humanoid.Parent then return end

        local character = humanoid.Parent
        local prepare = effectState.Enabled and isEnemyPlayer(targetPlayer, character)
        pcall(function()
            humanoid.BreakJointsOnDeath = prepare and false or info.OriginalBreakJoints
        end)
    end
    ...
    local function valueMatchesLocal(value)
        if typeof(value) == "Instance" then return value == LP end
        if type(value) == "number" then return value == LP.UserId end
        if type(value) == "string" then
            local lowered = string.lower(value)
            return lowered == string.lower(LP.Name) or lowered == tostring(LP.UserId)
        end
        return false
    end

    local function objectValueMatchesLocal(object)
        if not object then return false end
        if object:IsA("ObjectValue") then
            local ok, value = pcall(function() return object.Value end)
            return ok and value == LP
        elseif object:IsA("IntValue") or object:IsA("NumberValue") then
            local ok, value = pcall(function() return object.Value end)
            return ok and tonumber(value) == LP.UserId
        elseif object:IsA("StringValue") then
            local ok, value = pcall(function() return object.Value end)
            return ok and valueMatchesLocal(value)
        end
        return false
    end

    local function attributesPointToLocal(object)
        if not object then return false end
        ...
        for key, value in pairs(attributes) do
            if isKillKey(key) and valueMatchesLocal(value) then return true end
        end
        return false
    end

    local function hasLocalKillerTag(character, humanoid)
        if attributesPointToLocal(humanoid) or attributesPointToLocal(character) then
            return true
        end
        for _, root in ipairs({humanoid, character}) do
            if root then
                for _, object in ipairs(root:GetDescendants()) do
                    if isKillKey(object.Name) and objectValueMatchesLocal(object) then
                        return true
                    end
                end
            end
    ...
    local function removePending(entry)
        ...
        entry.Removed = true
        if entry.Humanoid then effectState.PendingByHumanoid[entry.Humanoid] = nil end
    end

    local function prunePending()
        local now = os.clock()
        local writeIndex = 1
        for readIndex = 1, #effectState.PendingDeaths do
            local entry = effectState.PendingDeaths[readIndex]
            if entry
                and not entry.Removed
                and now - entry.At <= PENDING_LIFETIME
                and entry.Character
                and entry.Character.Parent then
                effectState.PendingDeaths[writeIndex] = entry
                writeIndex += 1
            else
                if entry then removePending(entry) end
            end
        end
        for index = writeIndex, #effectState.PendingDeaths do
            effectState.PendingDeaths[index] = nil
        end
    end
    ...
    local function pruneCredits()
        local now = os.clock()
        local writeIndex = 1
        for readIndex = 1, #effectState.RecentCredits do
            local credit = effectState.RecentCredits[readIndex]
            if credit and now - credit.At <= CREDIT_LIFETIME then
                effectState.RecentCredits[writeIndex] = credit
                writeIndex += 1
            end
        end
        for index = writeIndex, #effectState.RecentCredits do
            effectState.RecentCredits[index] = nil
        end
    end

    local function applyConfirmedVictim(targetPlayer, character, humanoid, proof)
        local info = effectState.TrackedHumanoids[humanoid]
        if not info or info.Triggered then return false end
        if not effectState.Enabled or not isEnemyPlayer(targetPlayer, character) then return false end
        if not character or not character.Parent then return false end

        local effectName = selectedEffect()
        if not effectName or not BY_NAME[effectName] then return false end

        info.Triggered = true
        local pending = effectState.PendingByHumanoid[humanoid]
        if pending then removePending(pending) end
        effectState.LastAppliedAt = os.clock()

        task.defer(function()
            if not runtime.Alive or not character or not character.Parent then return end
            local ok, result = pcall(function()
                return runNativeOnVictim(effectName, character, nil)
            end)
            if not ok or result ~= true then
                showBottomMessage(
                    "No se pudo aplicar " .. tostring(effectName) .. " a " .. tostring(targetPlayer.Name) .. ".",
                    {title = "XeroHub · Death Effects", key = "deathfx:error", priority = true}
                )
            end
        end)
    ...
    local function newestPending()
        prunePending()
        for index = #effectState.PendingDeaths, 1, -1 do
            local entry = effectState.PendingDeaths[index]
            if entry
                and not entry.Removed
                and effectState.Enabled
                and isEnemyPlayer(entry.Player, entry.Character)
                and entry.Character
                and entry.Character.Parent then
                return entry
            end
    ...
    local function consumePending(proof)
        local entry = newestPending()
        if not entry then return false end
        removePending(entry)
        return applyConfirmedVictim(entry.Player, entry.Character, entry.Humanoid, proof)
    end

    local function consumeFreshCredit(targetPlayer, character, humanoid)
        pruneCredits()
        if #effectState.RecentCredits == 0 then return false end
        local credit = table.remove(effectState.RecentCredits, 1)
        if not credit then return false end
        return applyConfirmedVictim(targetPlayer, character, humanoid, credit.Source)
    end

    local function recordLocalKillSignal(source, count)
        if not effectState.Enabled then return end
        count = math.max(1, math.floor(tonumber(count) or 1))
        local now = os.clock()

        -- GunKill and the kill counter commonly report the same elimination.
        if effectState.LastSignalSource ~= source
            and now - effectState.LastSignalAt <= SIGNAL_DEDUPE_WINDOW then
            return
        end

        if now - effectState.LastAppliedAt <= SIGNAL_DEDUPE_WINDOW and not newestPending() then
            effectState.LastSignalAt = now
            effectState.LastSignalSource = source
            return
        end
        ...
        for _ = 1, count do
            if not consumePending(source) then
                pruneCredits()
                effectState.RecentCredits[#effectState.RecentCredits + 1] = {
                    At = now,
                    Source = source,
                }
            end
    ...
    local function queueVictimDeath(targetPlayer, character, humanoid)
        local info = effectState.TrackedHumanoids[humanoid]
        if not info or info.Triggered then return end
        if not effectState.Enabled or not isEnemyPlayer(targetPlayer, character) then return end
        if not character or not character.Parent or effectState.PendingByHumanoid[humanoid] then return end

        if hasLocalKillerTag(character, humanoid) then
            applyConfirmedVictim(targetPlayer, character, humanoid, "killer/creator nativo")
            return
        end

        if consumeFreshCredit(targetPlayer, character, humanoid) then return end
        ...
        effectState.PendingByHumanoid[humanoid] = entry
        effectState.PendingDeaths[#effectState.PendingDeaths + 1] = entry

        -- DUELS can stamp creator/killer a few frames after Died.
        task.defer(function()
            for _, delayTime in ipairs({0.03, 0.08, 0.16, 0.30, 0.48}) do
                task.wait(delayTime)
                if not runtime.Alive or info.Triggered or entry.Removed then return end
                if hasLocalKillerTag(character, humanoid) then
                    removePending(entry)
                    applyConfirmedVictim(targetPlayer, character, humanoid, "killer/creator nativo")
                    return
        ...
        task.delay(PENDING_LIFETIME + 0.08, function()
            if entry.Removed or info.Triggered then return end
            removePending(entry)
        end)
    end

    local function hookCharacter(targetPlayer, character)
        if targetPlayer == LP or not character then return end

        task.spawn(function()
            local humanoid = character:FindFirstChildOfClass("Humanoid")
                or character:WaitForChild("Humanoid", 8)
            if not humanoid or effectState.TrackedHumanoids[humanoid] then return end

            local info = {
                OriginalBreakJoints = humanoid.BreakJointsOnDeath,
                Triggered = false,
                Connections = {},
            }
            effectState.TrackedHumanoids[humanoid] = info
            refreshHumanoidMode(targetPlayer, humanoid)

            local function bind(connection)
                info.Connections[#info.Connections + 1] = connection
                trackEffectConnection(connection)
            end

            bind(humanoid.Died:Connect(function()
                queueVictimDeath(targetPlayer, character, humanoid)
            end))
            bind(humanoid.HealthChanged:Connect(function(health)
                if health <= 0 then queueVictimDeath(targetPlayer, character, humanoid) end
            end))
            bind(humanoid:GetPropertyChangedSignal("BreakJointsOnDeath"):Connect(function()
                if effectState.Enabled and isEnemyPlayer(targetPlayer, character)
                    and humanoid.BreakJointsOnDeath ~= false then
            ...
            bind(character.AncestryChanged:Connect(function(_, parent)
                ...
                local pending = effectState.PendingByHumanoid[humanoid]
                if pending then removePending(pending) end
                effectState.TrackedHumanoids[humanoid] = nil
    ...
    local function hookPlayer(targetPlayer)
        if targetPlayer == LP or effectState.PlayerConnections[targetPlayer] then return end
        local connections = {}
        effectState.PlayerConnections[targetPlayer] = connections
        ...
        add(targetPlayer.CharacterAdded:Connect(function(character)
        ...
        local function refreshPlayer()
            local character = targetPlayer.Character
            local humanoid = character and character:FindFirstChildOfClass("Humanoid")
            if humanoid then refreshHumanoidMode(targetPlayer, humanoid) end
        end
        add(targetPlayer:GetPropertyChangedSignal("Team"):Connect(refreshPlayer))
        add(targetPlayer:GetPropertyChangedSignal("TeamColor"):Connect(refreshPlayer))
        add(targetPlayer:GetPropertyChangedSignal("Neutral"):Connect(refreshPlayer))

        if targetPlayer.Character then hookCharacter(targetPlayer, targetPlayer.Character) end
    end

    local function refreshTrackedPlayers()
        for _, targetPlayer in ipairs(Players:GetPlayers()) do
            if targetPlayer ~= LP then
                hookPlayer(targetPlayer)
                local character = targetPlayer.Character
                local humanoid = character and character:FindFirstChildOfClass("Humanoid")
                if humanoid then refreshHumanoidMode(targetPlayer, humanoid) end
            end
    ...
    local function looksLikeKillCounter(object)
        if not (object:IsA("IntValue") or object:IsA("NumberValue")) then return false end
        local name = normalized(object.Name)
        return name == "kills"
            or name == "kill"
            or name == "eliminations"
            or name == "elimination"
            or name == "elims"
            or name == "kos"
    end

    local function hookKillCounter(object)
        if not looksLikeKillCounter(object) or effectState.KillValues[object] then return end
        effectState.KillValues[object] = true
        local previous = tonumber(object.Value) or 0
        trackEffectConnection(object:GetPropertyChangedSignal("Value"):Connect(function()
            local current = tonumber(object.Value) or previous
            local delta = current - previous
            previous = current
            if delta > 0 then
                recordLocalKillSignal("contador " .. tostring(object.Name), delta)
            end
        end))
    end

    local function localWeaponOwnsSound(sound)
        ...
        local function under(root)
            if not root then return false end
            local ok, result = pcall(function() return sound:IsDescendantOf(root) end)
            return ok and result == true
        end

        if under(character) or under(backpack) then return true end

        local tool = sound:FindFirstAncestorWhichIsA("Tool")
        if tool then
            if character and (tool.Parent == character or tool:IsDescendantOf(character)) then return true end
            if backpack and (tool.Parent == backpack or tool:IsDescendantOf(backpack)) then return true end
        end
    ...
    local function isKillConfirmSound(sound)
        if not sound or not sound:IsA("Sound") then return false end
        local name = normalized(sound.Name)
        local digits = extractSoundDigits(sound.SoundId)
        return name == "gunkill"
            or digits == KILL_GUN_SOUND_ID
            or name == "killconfirm"
            or name == "killsound"
            or name == "knifekill"
    end
    ...
    local function hookKillSound(sound)
        if not isKillConfirmSound(sound) or effectState.KillSounds[sound] then return end
        effectState.KillSounds[sound] = {WasLocal = localWeaponOwnsSound(sound)}

        local function refreshOwnership()
            local state = effectState.KillSounds[sound]
            if state and not state.WasLocal and localWeaponOwnsSound(sound) then
                state.WasLocal = true
            end
        end
        ...
        local function confirm()
            local state = effectState.KillSounds[sound]
            refreshOwnership()
            if effectState.Enabled and state and (state.WasLocal or localWeaponOwnsSound(sound)) then
                recordLocalKillSignal("sonido " .. tostring(sound.Name), 1)
            end
        end

        trackEffectConnection(sound.AncestryChanged:Connect(refreshOwnership))
        trackEffectConnection(sound.Played:Connect(confirm))
        trackEffectConnection(sound:GetPropertyChangedSignal("Playing"):Connect(function()
            if sound.Playing then confirm() end
        end))

        -- DescendantAdded can arrive a fraction after :Play() on cloned tools.
        -- If it is already playing when we hook it, consume the confirmation now.
        if sound.Playing or sound.IsPlaying then confirm() end
    end

    local function scanKillSignals(root)
        if not root then return end
        hookKillCounter(root)
        hookKillSound(root)
        for _, object in ipairs(root:GetDescendants()) do
            hookKillCounter(object)
            hookKillSound(object)
        end
    end

    local function watchSignalRoot(root)
        if not root then return end
        scanKillSignals(root)
        trackEffectConnection(root.DescendantAdded:Connect(function(object)
            hookKillCounter(object)
            hookKillSound(object)
        end))
    end

    local function warmSelectedEffect(announce)
        local effectName = selectedEffect()
        if not effectName then return end
        effectState.WarmToken += 1
        local token = effectState.WarmToken
        task.spawn(function()
            local ok, asset, injected, statusText = pcall(ensureNativeAsset, effectName)
            if not runtime.Alive or token ~= effectState.WarmToken then return end
            if not ok or not asset then
                if announce then
                    showBottomMessage(
                        "No se pudo preparar " .. tostring(effectName) .. ".",
                        {title = "XeroHub · Death Effects", key = "deathfx:warm", priority = true}
                    )
                end
                return
            end
            if announce then
                showBottomMessage(
                    tostring(effectName) .. " preparado.",
                    {title = "XeroHub · Death Effects", key = "deathfx:warm"}
                )
            end
        end)
    end

    -- UI: keep the feature in Sonidos for now so the main tab layout/numbering does
    -- not need another remote-UI patch.
    Tabs.Sonidos:Section({Title = "Efectos de muerte"})
    Tabs.Sonidos:Paragraph({
        Title = "Death Effects",
        Desc = "Aplica el efecto al enemigo sólo cuando DUELS confirma que la eliminación fue tuya.",
    })

    local defaultEffect = BY_NAME.Frostbite and "Frostbite" or EFFECTS[1]
    effectState.Selected = defaultEffect

    effectState.Dropdown = Tabs.Sonidos:Dropdown({
        Title = "Efecto de muerte",
        Desc = "Elige el efecto que verás en tus eliminaciones.",
        Values = #EFFECTS > 0 and EFFECTS or {"Sin efectos disponibles"},
        Value = defaultEffect or "Sin efectos disponibles",
        Callback = function(value)
            local chosen = type(value) == "table" and value[1] or value
            if not BY_NAME[chosen] then return end
            effectState.Selected = chosen
            warmSelectedEffect(false)
        end,
    })
    UIElements.DropDeathEffect = effectState.Dropdown

    effectState.Toggle = Tabs.Sonidos:Toggle({
        Title = "Efectos de muerte",
        Desc = "Sólo tus kills confirmados; ignora lobby, aliados y otras rondas.",
        Value = false,
        Callback = function(enabled)
            effectState.Enabled = enabled == true
            if effectState.Enabled then
                warmSelectedEffect(false)
            else
                for _, entry in ipairs(effectState.PendingDeaths) do removePending(entry) end
                table.clear(effectState.PendingDeaths)
                table.clear(effectState.RecentCredits)
            end
            refreshTrackedPlayers()
        end,
    })
    UIElements.TogDeathEffects = effectState.Toggle

    -- Existing/future kill confirmations. Unlike R18, Character.DescendantAdded is
    -- permanently watched, so a GunKill cloned after spawn is still captured.
    watchSignalRoot(LP)
    watchSignalRoot(LP.Character)

    trackEffectConnection(LP.CharacterAdded:Connect(function(character)
        watchSignalRoot(character)
    end))

    local currentBackpack = LP:FindFirstChildOfClass("Backpack")
    if currentBackpack then watchSignalRoot(currentBackpack) end

    -- Catch a GunKill clone even if DUELS briefly places it elsewhere in Workspace.
    runtime.PrimeSoundRegistry()
    for sound in pairs(runtime.SoundRegistry) do
        if sound and sound.Parent then hookKillSound(sound) end
    end
    trackEffectConnection(workspace.DescendantAdded:Connect(function(object)
        if object:IsA("Sound") then hookKillSound(object) end
    end))
    trackEffectConnection(SoundService.DescendantAdded:Connect(function(object)
        if object:IsA("Sound") then hookKillSound(object) end
    end))

    for _, targetPlayer in ipairs(Players:GetPlayers()) do hookPlayer(targetPlayer) end
    trackEffectConnection(Players.PlayerAdded:Connect(hookPlayer))
    trackEffectConnection(Players.PlayerRemoving:Connect(function(targetPlayer)
        effectState.PlayerConnections[targetPlayer] = nil
    end))

    for _, propertyName in ipairs({"Team", "TeamColor", "Neutral"}) do
        trackEffectConnection(LP:GetPropertyChangedSignal(propertyName):Connect(refreshTrackedPlayers))
    end

    refreshTrackedPlayers()

    runtime.SerializeDeathEffectConfig = function()
        return {
            Seleccionado = effectState.Selected,
            Activado = effectState.Enabled == true,
        }
    end

    runtime.LoadDeathEffectConfig = function(data)
        if type(data) ~= "table" then return false end
        local selected = type(data.Seleccionado) == "string" and data.Seleccionado or nil
        if selected and BY_NAME[selected] then
            effectState.Selected = selected
            if effectState.Dropdown then
                pcall(function() effectState.Dropdown:Select(selected) end)
            end
        end
        if effectState.Toggle then
            pcall(function() effectState.Toggle:Set(data.Activado == true) end)
        else
            effectState.Enabled = data.Activado == true
            refreshTrackedPlayers()
        end
        return true
    end

    local function restorePreviewAssets()
        for victim in pairs(victimCleaners) do cleanVictim(victim) end

        local skins, _, preview = ensureSkinsFolders()
        for _, root in pairs(previewInjected) do
            if root and root.Parent and root:GetAttribute("XeroFullPreviewInjected") then
                pcall(function() root:Destroy() end)
            end
        end

        local backupFolder = skins:FindFirstChild("XeroNativePreviewBackups")
        if backupFolder then
            for _, backup in ipairs(backupFolder:GetChildren()) do
                local current = preview:FindFirstChild(backup.Name)
                if current and current:GetAttribute("XeroFullPreviewInjected") then
                    pcall(function() current:Destroy() end)
                    current = nil
                end
                if not preview:FindFirstChild(backup.Name) then
                    pcall(function() backup.Parent = preview end)
                end
            end
            pcall(function() backupFolder:Destroy() end)
        end
    end

    runtime.DeathEffectCleanup = function()
        effectState.Enabled = false
        effectState.WarmToken += 1
        for humanoid, info in pairs(effectState.TrackedHumanoids) do
            if humanoid and humanoid.Parent and info then
                pcall(function() humanoid.BreakJointsOnDeath = info.OriginalBreakJoints end)
            end
        end
        for _, entry in ipairs(effectState.PendingDeaths) do removePending(entry) end
        table.clear(effectState.PendingDeaths)
        table.clear(effectState.RecentCredits)
        restorePreviewAssets()
        runtime.DeathEffects = nil
    end

    -- Warm the default once after the UI exists; no all-effects startup stall.
    warmSelectedEffect(false)
end

runtime.DeathEffectInitOK, runtime.DeathEffectInitError = pcall(runtime.InitDeathEffects)
if not runtime.DeathEffectInitOK then
    warn("[XeroHub · Death Effects] " .. tostring(runtime.DeathEffectInitError))
    showBottomMessage("Death Effects no pudo iniciar.", {title = "XeroHub · Death Effects", key = "deathfx:init", priority = true})
end
runtime.InitDeathEffects = nil
...
-- ==========================================
-- PESTAÑA TEXTURAS: skins para armas default (cuchillo y pistola)
-- Catálogo automático desde carpetas de GitHub + Asset ID personalizado.
...
local TextureCatalogCore = {
    CACHE_FOLDER = "XeroHub/Textures",
    URLS = {
...
local textureState = {
    ...
    Catalog = {knife = {}, gun = {}},
    ByLabel = {knife = {}, gun = {}},
    SelectedLabel = {knife = nil, gun = nil},
    CustomIds = {knife = "", gun = ""},
    Active = {
        knife = {Enabled = false, Source = nil, Asset = nil, Label = nil, Entry = nil},
        gun = {Enabled = false, Source = nil, Asset = nil, Label = nil, Entry = nil},
    },
    Saved = setmetatable({}, {__mode = "k"}),
    Connections = {},
    SyncingWeaponDropdown = false,
    SyncingSkinDropdown = false,
    PendingConfig = nil,
}
runtime.TextureState = textureState

local textureWeaponDropdown
local textureSkinDropdown
local textureCustomInput

local function textureAssetDigits(value)
    return tostring(value or ""):match("(%d+)") or ""
end

local function textureTrim(value)
    return tostring(value or ""):match("^%s*(.-)%s*$") or ""
end

local function texturePrettyName(fileName)
    local name = tostring(fileName or "")
    name = name:gsub("%.[%w]+$", "")
    name = name:gsub("%s*%(%d+%)%s*$", "")
    name = name:gsub("[_%-]+", " "):gsub("%s+", " ")
    name = textureTrim(name)
    name = name:gsub("%S+", function(word)
        return word:sub(1, 1):upper() .. word:sub(2):lower()
    end)
    return name ~= "" and name or "Skin sin nombre"
end

local function textureEntryKey(entry)
    entry = type(entry) == "table" and entry or {}
    return tostring(entry.name or "") .. "@" .. tostring(entry.sha or "")
end

local function textureCachePath(weaponKey, entry)
    entry = type(entry) == "table" and entry or {}
    local extension = tostring(entry.extension or "png")
    local token = tostring(entry.sha or ""):gsub("[^%w]", ""):sub(1, 12)
    if token == "" then
        token = tostring(entry.name or "skin"):gsub("[^%w]", "_"):sub(1, 40)
    end
    return TextureCatalogCore.CACHE_FOLDER .. "/" .. tostring(weaponKey) .. "_" .. token .. "." .. extension
end

local function textureRequest(url)
    local req = (syn and syn.request) or (http and http.request) or http_request or request
    if req then
        local ok, response = pcall(function()
            return req({
                ...
                Method = "GET",
                Headers = {
                    ["User-Agent"] = "XeroHub-Textures/1.0",
                    ["Accept"] = "application/vnd.github+json",
                },
            })
        end)
...
local function ensureTextureFolder()
    if type(makefolder) ~= "function" then return false end
    if not isfolder or not isfolder("XeroHub") then pcall(makefolder, "XeroHub") end
    if not isfolder or not isfolder(TextureCatalogCore.CACHE_FOLDER) then
        pcall(makefolder, TextureCatalogCore.CACHE_FOLDER)
    end
    if type(isfolder) == "function" then
        local ok, exists = pcall(isfolder, TextureCatalogCore.CACHE_FOLDER)
        return ok and exists == true
    end
    return true
end

local function parseTextureCatalog(body)
    if type(body) ~= "string" or body == "" then return {} end
    local ok, decoded = pcall(function() return HttpService:JSONDecode(body) end)
    if not ok or type(decoded) ~= "table" then return {} end

    local result, seenLabels = {}, {}
    for _, item in ipairs(decoded) do
        if type(item) == "table" and item.type == "file"
            and type(item.download_url) == "string" and item.download_url ~= "" then
            local extension = string.lower(tostring(item.name or ""):match("%.([%w]+)$") or "")
            if extension == "png" or extension == "jpg" or extension == "jpeg" or extension == "webp" then
                local label = texturePrettyName(item.name)
                seenLabels[label] = (seenLabels[label] or 0) + 1
                table.insert(result, {
                    name = tostring(item.name),
                    label = label,
                    sha = tostring(item.sha or ""),
    ...
    table.sort(result, function(a, b)
        ...
        if al == bl then return a.name:lower() < b.name:lower() end
        return al < bl
    end)
    ...
    for _, entry in ipairs(result) do
        local key = entry.label:lower()
        if (seenLabels[entry.label] or 0) > 1 then
            seen[key] = (seen[key] or 0) + 1
            entry.label = entry.label .. " [" .. tostring(seen[key]) .. "]"
        end
    end
...
local function downloadTextureAsset(weaponKey, entry)
    if type(textureAssetLoader) ~= "function" or type(writefile) ~= "function" then
        return nil, "unsupported"
    end

    local hasFolders = ensureTextureFolder()
    local localPath = textureCachePath(weaponKey, entry)
    if not hasFolders then
        localPath = "XeroHub_Texture_" .. tostring(weaponKey) .. "_" .. tostring(entry.name or "skin"):gsub("[^%w]", "_")
    end

    local validCache = false
    if type(isfile) == "function" and type(readfile) == "function" and isfile(localPath) then
        local ok, cached = pcall(readfile, localPath)
        validCache = ok and type(cached) == "string" and #cached > 64
    end

    if not validCache then
        local body, status = textureRequest(entry.url)
        if not body or status < 200 or status >= 300 or #body < 64 then
            return nil, "download_failed"
        end
        local ok = pcall(writefile, localPath, body)
        if not ok then return nil, "write_failed" end
    end

    local ok, assetId = pcall(textureAssetLoader, localPath)
    if not ok or type(assetId) ~= "string" or assetId == "" then
        return nil, "asset_failed"
    end
    return assetId, localPath
end

local function fetchTextureCatalog(weaponKey)
    local url = TextureCatalogCore.URLS[weaponKey]
    local body, status = textureRequest(url)
    if status >= 200 and status < 300 then
        return parseTextureCatalog(body)
    end
    return {}
end

local function setTextureWeaponDropdownSilently(label)
    if not textureWeaponDropdown then return end
    textureState.SyncingWeaponDropdown = true
    pcall(function() textureWeaponDropdown:Select(label) end)
    textureState.SyncingWeaponDropdown = false
end

local function setTextureSkinDropdownSilently(label)
    if not textureSkinDropdown then return end
    textureState.SyncingSkinDropdown = true
    pcall(function() textureSkinDropdown:Select(label) end)
    textureState.SyncingSkinDropdown = false
end

local function updateSkinDropdownForWeapon(weaponKey)
    local values, byLabel = {}, textureState.ByLabel[weaponKey]
    for _, entry in ipairs(textureState.Catalog[weaponKey]) do
        values[#values + 1] = entry.label
    end
    if #values == 0 then values = {"Sin skins disponibles"} end

    local selected = textureState.SelectedLabel[weaponKey]
    if not byLabel[selected] then selected = values[1] end
    textureState.SelectedLabel[weaponKey] = byLabel[selected] and selected or nil

    if textureSkinDropdown then
        pcall(function()
            textureSkinDropdown:Refresh(values)
            if textureState.SelectedLabel[weaponKey] then
                setTextureSkinDropdownSilently(textureState.SelectedLabel[weaponKey])
            else
                setTextureSkinDropdownSilently(values[1])
            end
        end)
    end
end

local function saveTextureOriginal(obj, weaponKey)
    if not obj then return end
    local saved = textureState.Saved[obj]
    if saved and saved.Key == weaponKey then return end

    if obj:IsA("SpecialMesh") then
        textureState.Saved[obj] = {Class = "SpecialMesh", Key = weaponKey, TextureId = obj.TextureId}
    elseif obj:IsA("MeshPart") then
        textureState.Saved[obj] = {Class = "MeshPart", Key = weaponKey, TextureID = obj.TextureID}
    elseif obj:IsA("SurfaceAppearance") then
        textureState.Saved[obj] = {Class = "SurfaceAppearance", Key = weaponKey, ColorMap = obj.ColorMap}
    end
end

local function restoreTextureWeapon(weaponKey, activeAsset)
    local originalTexture = TextureCatalogCore.OriginalTexture[weaponKey]
    local canonicalOriginal = originalTexture and ("rbxassetid://" .. tostring(originalTexture)) or nil

    for obj, data in pairs(textureState.Saved) do
        if data and data.Key == weaponKey then
            if obj and obj.Parent then
                pcall(function()
                    if data.Class == "SpecialMesh" then
                        local value = data.TextureId or ""
                        -- Si el objeto nació/clonó cuando la skin custom ya estaba puesta,
                        -- Saved puede contener la propia skin custom. En ese caso usamos
                        -- el ID original conocido del arma default.
                        if canonicalOriginal and activeAsset and value == activeAsset then
            ...
            textureState.Saved[obj] = nil
        end
    end
end

local function textureShouldTreatAsKnife(obj)
    ...
    if anc then
        ...
        if tname:find("knife", 1, true) or tname:find("blade", 1, true) then return true end
    end
    return obj:FindFirstAncestor("KnifePartsFolder") ~= nil
end

local function textureShouldTreatAsGun(obj)
    local name = string.lower(tostring(obj.Name or ""))
    if name:find("gun", 1, true) or name:find("pistol", 1, true) or name:find("revolver", 1, true) then return true end
    local anc = obj:FindFirstAncestorOfClass("Tool")
    if anc then
        local tname = string.lower(anc.Name)
        if tname:find("gun", 1, true) or tname:find("pistol", 1, true) or tname:find("revolver", 1, true) then return true end
    end
    return obj:FindFirstAncestor("GunPartsFolder") ~= nil
end

local function isDefaultWeaponObject(weaponKey, obj)
    if not obj then return false end
    if weaponKey == "knife" and textureShouldTreatAsKnife(obj) then return true end
    if weaponKey == "gun" and textureShouldTreatAsGun(obj) then return true end

    if obj:IsA("SpecialMesh") then
        local tex = textureAssetDigits(obj.TextureId)
        local mesh = textureAssetDigits(obj.MeshId)
        return tex == TextureCatalogCore.OriginalTexture[weaponKey]
            or mesh == TextureCatalogCore.OriginalMesh[weaponKey]
    elseif obj:IsA("MeshPart") then
        local tex = textureAssetDigits(obj.TextureID)
        local mesh = textureAssetDigits(obj.MeshId)
        return tex == TextureCatalogCore.OriginalTexture[weaponKey]
            or mesh == TextureCatalogCore.OriginalMesh[weaponKey]
    elseif obj:IsA("SurfaceAppearance") then
        if weaponKey == "knife" then return textureShouldTreatAsKnife(obj) end
        if weaponKey == "gun" then return textureShouldTreatAsGun(obj) end
    end
    return false
end

local function restoreTextureCopiesWithActiveAsset(weaponKey, activeAsset)
    if type(activeAsset) ~= "string" or activeAsset == "" then return 0 end

    local originalTexture = TextureCatalogCore.OriginalTexture[weaponKey]
    if not originalTexture then return 0 end
    local canonicalOriginal = "rbxassetid://" .. tostring(originalTexture)
    local restored = 0

    local function restoreOne(obj)
        if not obj or not obj.Parent or not isDefaultWeaponObject(weaponKey, obj) then return end

        pcall(function()
            if obj:IsA("SpecialMesh") and obj.TextureId == activeAsset then
                obj.TextureId = canonicalOriginal
                restored = restored + 1
            elseif obj:IsA("MeshPart") and obj.TextureID == activeAsset then
                obj.TextureID = canonicalOriginal
                restored = restored + 1
            elseif obj:IsA("SurfaceAppearance") and obj.ColorMap == activeAsset then
                obj.ColorMap = canonicalOriginal
                restored = restored + 1
            end
        end)
    end
    ...
    -- Respaldo para copias creadas/reparentadas después de aplicar la skin.
    -- Aunque no hayan alcanzado a entrar a textureState.Saved, si todavía
    -- llevan exactamente el asset custom activo las devolvemos al original.
    restoreRoot(player.Character)
    restoreRoot(player:FindFirstChild("Backpack"))

    return restored
end

local function applyTextureToObject(weaponKey, obj)
    local active = textureState.Active[weaponKey]
    if not active or not active.Enabled or not active.Asset or not obj then return false end
    if not isDefaultWeaponObject(weaponKey, obj) then return false end

    local changed = false
    pcall(function()
        if obj:IsA("SpecialMesh") then
            saveTextureOriginal(obj, weaponKey)
            if obj.TextureId ~= active.Asset then
                obj.TextureId = active.Asset
                changed = true
            end
        elseif obj:IsA("MeshPart") then
            saveTextureOriginal(obj, weaponKey)
        ...
        elseif obj:IsA("SurfaceAppearance") then
            saveTextureOriginal(obj, weaponKey)
            if obj.ColorMap ~= active.Asset then
                obj.ColorMap = active.Asset
                changed = true
            end
        end
    end)
...
local function processTextureRoot(root)
    if not root then return end
    for weaponKey, active in pairs(textureState.Active) do
        if active.Enabled then
            applyTextureToObject(weaponKey, root)
        end
    end
    for _, d in ipairs(root:GetDescendants()) do
        for weaponKey, active in pairs(textureState.Active) do
            if active.Enabled then
                applyTextureToObject(weaponKey, d)
            end
        end
    end
end

local function scanTextureAll()
    local character = player.Character
    if character then processTextureRoot(character) end
    local backpack = player:FindFirstChild("Backpack")
    if backpack then processTextureRoot(backpack) end
end

local function disconnectTextureConnections()
    for i = #textureState.Connections, 1, -1 do
        pcall(function() textureState.Connections[i]:Disconnect() end)
        textureState.Connections[i] = nil
    end
end

local function trackTextureConnection(conn)
    if conn then textureState.Connections[#textureState.Connections + 1] = conn end
    return conn
end

local function anyTextureActive()
    return textureState.Active.knife.Enabled or textureState.Active.gun.Enabled
end

local function rebuildTextureHooks()
    disconnectTextureConnections()
    if not anyTextureActive() then return end

    local function onAdded(obj)
        task.defer(function()
            if not runtime.Alive or not anyTextureActive() then return end
            for weaponKey, active in pairs(textureState.Active) do
                if active.Enabled then applyTextureToObject(weaponKey, obj) end
            end
        end)
    end

    trackTextureConnection(player.CharacterAdded:Connect(function(char)
        task.wait(0.15)
        processTextureRoot(char)
        trackTextureConnection(char.DescendantAdded:Connect(onAdded))
    end))

    local char = player.Character
    if char then trackTextureConnection(char.DescendantAdded:Connect(onAdded)) end

    local backpack = player:FindFirstChild("Backpack")
    if backpack then trackTextureConnection(backpack.DescendantAdded:Connect(onAdded)) end
    trackTextureConnection(player.ChildAdded:Connect(function(obj)
        if obj.Name == "Backpack" then
            task.wait(0.05)
            trackTextureConnection(obj.DescendantAdded:Connect(onAdded))
            processTextureRoot(obj)
        end
    end))
end

local function applyTextureNotification(label)
    showBottomMessage("Skin aplicada: " .. tostring(label or "Personalizada"), {
        title = "XeroHub · Texturas",
        key = "textures:apply:" .. tostring(label or "custom"),
    })
end

local function activateTextureWeapon(weaponKey, source, payload, announce)
    local asset, label, entry
    if source == "github" then
        entry = payload
        if type(entry) ~= "table" then return false end
        asset = textureEnv["XERO_TEXTURE_ASSET_" .. weaponKey .. "_" .. textureEntryKey(entry)]
        if type(asset) ~= "string" or asset == "" then
            local loadedAsset = select(1, downloadTextureAsset(weaponKey, entry))
            if type(loadedAsset) ~= "string" or loadedAsset == "" then
                return false
            end
            asset = loadedAsset
            textureEnv["XERO_TEXTURE_ASSET_" .. weaponKey .. "_" .. textureEntryKey(entry)] = asset
        end
        label = entry.label
        textureState.SelectedLabel[weaponKey] = entry.label
    elseif source == "custom" then
        local digits = textureAssetDigits(payload)
        if digits == "" then return false end
        asset = "rbxassetid://" .. digits
        label = "Personalizada"
        textureState.CustomIds[weaponKey] = digits
    else
        return false
    end

    local previousAsset = textureState.Active[weaponKey] and textureState.Active[weaponKey].Asset or nil
    restoreTextureWeapon(weaponKey, previousAsset)
    restoreTextureCopiesWithActiveAsset(weaponKey, previousAsset)
    textureState.Active[weaponKey] = {
        Enabled = true,
        Source = source,
        Asset = asset,
        Label = label,
    ...
    scanTextureAll()
    rebuildTextureHooks()
    if announce then applyTextureNotification(label) end
    return true
end

local function restoreTextureSelected(weaponKey, announce)
    local previousAsset = textureState.Active[weaponKey] and textureState.Active[weaponKey].Asset or nil
    textureState.Active[weaponKey] = {Enabled = false, Source = nil, Asset = nil, Label = nil, Entry = nil}
    restoreTextureWeapon(weaponKey, previousAsset)
    restoreTextureCopiesWithActiveAsset(weaponKey, previousAsset)
    rebuildTextureHooks()
    if announce then
        showBottomMessage("Textura original restaurada.", {title = "XeroHub · Texturas", key = "textures:restore:" .. weaponKey})
    end
end

local function refreshTextureCatalog(weaponKey, updateUiAfter)
    task.spawn(function()
        local catalog = fetchTextureCatalog(weaponKey)
        textureState.Catalog[weaponKey] = catalog
        textureState.ByLabel[weaponKey] = {}
        for _, entry in ipairs(catalog) do
            textureState.ByLabel[weaponKey][entry.label] = entry
        end
        if updateUiAfter and textureState.CurrentWeaponKey == weaponKey then
            updateSkinDropdownForWeapon(weaponKey)
        end

        local pending = textureState.PendingConfig
        if pending and textureState.Catalog.knife and textureState.Catalog.gun then
            local knifeDone = pending._knifeDone
            local gunDone = pending._gunDone
            if weaponKey == "knife" then pending._knifeDone = true else pending._gunDone = true end
            if (weaponKey == "knife" and gunDone) or (weaponKey == "gun" and knifeDone) or (pending._knifeDone and pending._gunDone) then
                textureState.PendingConfig = nil
                runtime.LoadTextureConfig(pending)
            end
        end
    end)
end

function runtime.SerializeTextureConfig()
    return {
        CurrentWeapon = textureState.CurrentWeaponKey,
        Knife = {
            Seleccionado = textureState.SelectedLabel.knife,
            Activo = textureState.Active.knife.Enabled == true,
            Fuente = textureState.Active.knife.Source,
            Archivo = textureState.Active.knife.Entry and textureState.Active.knife.Entry.name or nil,
            CustomId = textureState.CustomIds.knife,
        },
        Gun = {
            Seleccionado = textureState.SelectedLabel.gun,
            Activo = textureState.Active.gun.Enabled == true,
            Fuente = textureState.Active.gun.Source,
            Archivo = textureState.Active.gun.Entry and textureState.Active.gun.Entry.name or nil,
            CustomId = textureState.CustomIds.gun,
        },
    }
end

local function findTextureEntryByConfig(weaponKey, section)
    if type(section) ~= "table" then return nil end
    local fileName = type(section.Archivo) == "string" and section.Archivo or nil
    if fileName and fileName ~= "" then
        for _, entry in ipairs(textureState.Catalog[weaponKey]) do
    ...
    local label = type(section.Seleccionado) == "string" and section.Seleccionado or nil
    if label and label ~= "" then return textureState.ByLabel[weaponKey][label] end
    return nil
end

function runtime.LoadTextureConfig(data)
    if type(data) ~= "table" then return false end

    if #textureState.Catalog.knife == 0 or #textureState.Catalog.gun == 0 then
        textureState.PendingConfig = data
        return true
    end

    local currentWeapon = tostring(data.CurrentWeapon or "knife")
    if currentWeapon ~= "knife" and currentWeapon ~= "gun" then currentWeapon = "knife" end
    textureState.CurrentWeaponKey = currentWeapon
    setTextureWeaponDropdownSilently(TextureCatalogCore.WeaponTitles[currentWeapon])

    local knife = type(data.Knife) == "table" and data.Knife or {}
    local gun = type(data.Gun) == "table" and data.Gun or {}

    local knifeEntry = findTextureEntryByConfig("knife", knife)
    if knifeEntry then
        textureState.SelectedLabel.knife = knifeEntry.label
    end
    local gunEntry = findTextureEntryByConfig("gun", gun)
    if gunEntry then
        textureState.SelectedLabel.gun = gunEntry.label
    end

    textureState.CustomIds.knife = textureAssetDigits(knife.CustomId)
    textureState.CustomIds.gun = textureAssetDigits(gun.CustomId)

    updateSkinDropdownForWeapon(currentWeapon)

    restoreTextureSelected("knife", false)
    restoreTextureSelected("gun", false)

    if knife.Activo == true then
        if knife.Fuente == "custom" and textureState.CustomIds.knife ~= "" then
            activateTextureWeapon("knife", "custom", textureState.CustomIds.knife, false)
        elseif knifeEntry then
            activateTextureWeapon("knife", "github", knifeEntry, false)
        end
    end

    if gun.Activo == true then
        if gun.Fuente == "custom" and textureState.CustomIds.gun ~= "" then
            activateTextureWeapon("gun", "custom", textureState.CustomIds.gun, false)
        elseif gunEntry then
            activateTextureWeapon("gun", "github", gunEntry, false)
        end
    end
...
Tabs.Texturas:Paragraph({
    Title = "Texturas default",
    Desc = "Solo compatible con cuchillo default y pistola default por ahora.",
})
Tabs.Texturas:Section({Title = "Skins"})

textureWeaponDropdown = Tabs.Texturas:Dropdown({
    Title = "Arma default",
    Desc = "Elige qué arma default vas a personalizar.",
    Values = {TextureCatalogCore.WeaponTitles.knife, TextureCatalogCore.WeaponTitles.gun},
    Value = TextureCatalogCore.WeaponTitles.knife,
    Callback = function(value)
        if textureState.SyncingWeaponDropdown then return end
        local label = type(value) == "table" and value[1] or value
        local weaponKey = label == TextureCatalogCore.WeaponTitles.gun and "gun" or "knife"
        textureState.CurrentWeaponKey = weaponKey
        updateSkinDropdownForWeapon(weaponKey)
    end,
})

textureSkinDropdown = Tabs.Texturas:Dropdown({
    Title = "Skin disponible",
    Desc = "Skins subidas a la carpeta del arma default.",
    Values = {"Cargando skins…"},
    Value = "Cargando skins…",
    Callback = function(value)
        if textureState.SyncingSkinDropdown then return end
        local label = type(value) == "table" and value[1] or value
        if textureState.ByLabel[textureState.CurrentWeaponKey][label] then
            textureState.SelectedLabel[textureState.CurrentWeaponKey] = label
        end
    end,
})

Tabs.Texturas:Button({
    Title = "Aplicar skin seleccionada",
    Desc = "Aplica la skin elegida al arma default seleccionada.",
    Callback = function()
        local weaponKey = textureState.CurrentWeaponKey
        local label = textureState.SelectedLabel[weaponKey]
        local entry = label and textureState.ByLabel[weaponKey][label] or nil
        if not entry then return end
        activateTextureWeapon(weaponKey, "github", entry, true)
    end,
})

Tabs.Texturas:Button({
    Title = "Actualizar skins",
    Desc = "Recarga la lista de skins disponibles.",
    Callback = function()
        refreshTextureCatalog("knife", textureState.CurrentWeaponKey == "knife")
        refreshTextureCatalog("gun", textureState.CurrentWeaponKey == "gun")
    end,
})

Tabs.Texturas:Section({Title = "Personalizada"})
textureCustomInput = Tabs.Texturas:Input({
    Title = "Asset ID personalizado",
    Placeholder = "Ej: 122114929807745",
    Callback = function(text)
        textureState.CustomIds[textureState.CurrentWeaponKey] = textureAssetDigits(text)
    end,
})

Tabs.Texturas:Button({
    Title = "Aplicar ID personalizada",
    Desc = "Usa una textura propia en el arma default seleccionada.",
    Callback = function()
        local weaponKey = textureState.CurrentWeaponKey
        local customId = textureState.CustomIds[weaponKey]
        if customId == "" then return end
        activateTextureWeapon(weaponKey, "custom", customId, true)
    end,
})

Tabs.Texturas:Button({
    Title = "Restaurar original",
    Desc = "Devuelve la textura original del arma seleccionada.",
    Callback = function()
        restoreTextureSelected(textureState.CurrentWeaponKey, true)
    end,
})

runtime.TextureCleanup = function()
    restoreTextureSelected("knife", false)
    restoreTextureSelected("gun", false)
    disconnectTextureConnections()
end

refreshTextureCatalog("knife", true)
refreshTextureCatalog("gun", false)
updateSkinDropdownForWeapon("knife")
end
end
...
local XERO_CREDITS_PROFILE = "rbxassetid://74846094133538" -- Foto de Kev para Créditos.
local XERO_CREDITS_ALEX_PROFILE = "rbxassetid://77826666877085" -- Foto de Alex para Créditos.
...
Tabs.Inicio:Button({
    Title = "Murder Mystery V (Actualizado)",
    Callback = function() TPSeguro_Duels(116924926476457, "MMV") end
})
...
local function stopStatsRenderConnection()
    if not statsRenderConnection then return end
    pcall(function() statsRenderConnection:Disconnect() end)
    statsRenderConnection = nil
    runtime.PruneConnections()
end

local function startStatsRenderConnection()
    if statsRenderConnection then
        local connected = false
        pcall(function() connected = statsRenderConnection.Connected == true end)
        if connected then return end
        statsRenderConnection = nil
    end

    statsRenderConnection = runtime.Track(RunService.RenderStepped:Connect(function(deltaTime)
        fpsFrames = fpsFrames + 1
        statsElapsed = statsElapsed + deltaTime
        if statsElapsed < 1 then return end
...
Tabs.Inicio:Toggle({
    ...
    Callback = function(Value)
        showStatsEnabled = Value == true
        statsContainer.Visible = showStatsEnabled
        fpsFrames, statsElapsed = 0, 0
        fpsLabel.Text, pingLabel.Text = "--", "--"
        if showStatsEnabled then
            startStatsRenderConnection()
        else
            stopStatsRenderConnection()
        end
    end,
})
end
...
local function updateTeamCache()
    ...
    if isTeamLobby and runtime.RefreshAimHooks then
        runtime.RefreshAimHooks(false)
    end
end
...
runtime.BindForceFieldCache = function(char)
    ...
    runtime.ForceFieldAddedConnection = runtime.Track(char.ChildAdded:Connect(function(child)
        if child:IsA("ForceField") then
            hasForceField = true
            if runtime.RefreshAimHooks then runtime.RefreshAimHooks(false) end
        end
    end))
    runtime.ForceFieldRemovedConnection = runtime.Track(char.ChildRemoved:Connect(function(child)
        if child:IsA("ForceField") then
            hasForceField = false
            if runtime.RefreshAimHooks then task.defer(runtime.RefreshAimHooks) end
        end
    end))
    runtime.PruneConnections()
end
...
-- Gate EXCLUSIVO para los hooks de aim.
-- No añadimos Team/Neutral como requisito porque Duels puede actualizar esos valores
-- después de que el arma ya esté funcional. El filtro que ya demostramos estable
-- es el del propio juego: Character vivo + sin ForceField + fuera de teams/zona lobby.
function runtime.IsCombatRound()
    local char = player.Character
    ...
    -- estaEnLobby() ya consulta ForceField, team de lobby/espectador y ZONAS_SEGURAS.
    -- Fuera de ese estado permitimos montar los hooks; al volver, se desmontan.
    local okLobby, lobbyResult = pcall(estaEnLobby)
    if not okLobby or lobbyResult == true then return false end
...
function runtime.SetClonedAvatarTemplate(template, userId, targetName, silent, outfitId)
    ...
    local previousOutfitId = state.TargetOutfitId
    ...
    if not ok then
        ...
        state.TargetOutfitId = previousOutfitId
    ...
    state.TargetOutfitId = tonumber(outfitId)
    state.Active = true
...
-- ==============================================================
-- NAVEGADOR DE OUTFITS · SKINS GUARDADAS DE CUALQUIER USUARIO
-- ==============================================================
runtime.OutfitBrowser = {
    Cache = {},
    Gui = nil,
    Overlay = nil,
    Panel = nil,
...
function runtime.HttpGetJson(url)
    local body
    local req = (syn and syn.request) or (http and http.request) or http_request or request
    if req then
        local ok, response = pcall(function()
            return req({
                ...
                Method = "GET",
                Headers = {
                    ["User-Agent"] = "Roblox/XeroHub-OutfitBrowser",
                    ["Accept"] = "application/json",
                },
            })
        end)
    ...
    if type(body) ~= "string" or body == "" then
        return nil, "No se pudo consultar Roblox"
    end

    local ok, decoded = pcall(function() return HttpService:JSONDecode(body) end)
    if not ok or type(decoded) ~= "table" then
        return nil, "Roblox devolvió una respuesta inválida"
    end
    return decoded
end

function runtime.ResolveOutfitBrowserUser()
    local input = tostring(runtime.Appearance.AvatarClone.UsernameInput or "")
        :gsub("^%s+", ""):gsub("%s+$", "")
    ...
    if input ~= "" then
        ...
        if not okId or not userId then
            return nil, nil, "No se encontró ese username"
        end

        local resolved = input
        pcall(function() resolved = Players:GetNameFromUserIdAsync(userId) end)
    ...
    if target and target ~= player then
        return target.UserId, target.Name
    end

    return nil, nil, "Escribe un username o selecciona un jugador del servidor"
end

function runtime.FetchUserOutfits(userId, forceRefresh)
    userId = tonumber(userId)
    if not userId then return nil, "UserId inválido" end

    if not forceRefresh and runtime.OutfitBrowser.Cache[userId] then
        return runtime.OutfitBrowser.Cache[userId]
    end
    ...
    -- El endpoint v1 sigue permitiendo paginar los outfits guardados. Usamos
    -- isEditable=true para mostrar las skins creadas/guardadas por el usuario y
    -- no llenar la lista con presets de bundles.
    while page <= 20 do
        local url = "https://avatar.roblox.com/v1/users/"
            .. tostring(userId)
            .. "/outfits?page=" .. tostring(page)
            .. "&itemsPerPage=50&isEditable=true"

        local decoded, err = runtime.HttpGetJson(url)
        if not decoded then
            if page == 1 then return nil, err end
            break
        end

        local data = type(decoded.data) == "table" and decoded.data or {}
        for _, item in ipairs(data) do
            local outfitId = tonumber(item.id)
            if outfitId and not seen[outfitId] then
                seen[outfitId] = true
                outfits[#outfits + 1] = {
                    Id = outfitId,
                    Name = tostring(item.name or ("Outfit " .. tostring(outfitId))),
                    Editable = item.isEditable ~= false,
                }
            end
        end

        if #data < 50 then break end
        page += 1
        task.wait(0.04)
    end

    runtime.OutfitBrowser.Cache[userId] = outfits
    return outfits
end

function runtime.GetHumanoidDescriptionFromOutfitIdSafe(outfitId)
    outfitId = tonumber(outfitId)
    if not outfitId then return nil, "Outfit inválido" end
    ...
    return nil, tostring(description or "No se pudo cargar ese outfit")
end

function runtime.CloneAvatarFromOutfitId(outfitId, outfitName, ownerUserId, ownerName, silent)
    local char = player.Character
    local humanoid = char and char:FindFirstChildOfClass("Humanoid")
    if not humanoid then return false, "Tu personaje todavía no cargó" end

    local description, err = runtime.GetHumanoidDescriptionFromOutfitIdSafe(outfitId)
    if not description then return false, err end

    local template, templateErr = runtime.CreateAvatarCloneTemplateFromDescription(description, humanoid.RigType)
    pcall(function() description:Destroy() end)
    if not template then return false, templateErr end

    local displayName = tostring(ownerName or "Usuario") .. " · " .. tostring(outfitName or "Outfit")
    return runtime.SetClonedAvatarTemplate(template, ownerUserId, displayName, silent, outfitId)
end

function runtime.RestoreOutfitBrowserGuiOrder()
    local state = runtime.OutfitBrowser
    local restore = state and state.GuiOrderRestore
    if type(restore) ~= "table" then return end
    for guiObject, displayOrder in pairs(restore) do
        if guiObject and guiObject.Parent and guiObject:IsA("ScreenGui") then
            pcall(function() guiObject.DisplayOrder = displayOrder end)
        end
    end
    state.GuiOrderRestore = nil
end

function runtime.BringOutfitBrowserToFront()
    local state = runtime.OutfitBrowser
    local gui = state and state.Gui
    if not gui or not gui.Parent then return end

    runtime.RestoreOutfitBrowserGuiOrder()

    local parent = gui.Parent
    local restore = setmetatable({}, {__mode = "k"})
    local ownName = string.lower(gui.Name)
    for _, other in ipairs(parent:GetChildren()) do
        if other ~= gui and other:IsA("ScreenGui") then
            local name = string.lower(other.Name or "")
            local likelyHub = string.find(name, "xero", 1, true)
                or string.find(name, "ilun", 1, true)
                or string.find(name, "wind", 1, true)
                or string.find(name, "nox", 1, true)
                or string.find(name, "hub", 1, true)
            if likelyHub and other.DisplayOrder >= 2147483646 then
                restore[other] = other.DisplayOrder
                pcall(function() other.DisplayOrder = 2147483646 end)
            end
        end
    end
    state.GuiOrderRestore = restore

    -- Si el hub usa el mismo DisplayOrder máximo, reparentar al final resuelve
    -- el empate de sibling order sin esconder la ventana principal.
    pcall(function()
        gui.Parent = nil
        gui.Parent = parent
        gui.DisplayOrder = 2147483647
...
function runtime.DestroyOutfitBrowser()
    runtime.RestoreOutfitBrowserGuiOrder()
    if runtime.OutfitBrowser.Gui then
        pcall(function() runtime.OutfitBrowser.Gui:Destroy() end)
    end
    runtime.OutfitBrowser.Gui = nil
    runtime.OutfitBrowser.Overlay = nil
    runtime.OutfitBrowser.Panel = nil
    runtime.OutfitBrowser.Scroll = nil
    runtime.OutfitBrowser.Title = nil
    runtime.OutfitBrowser.Count = nil
    runtime.OutfitBrowser.SearchBox = nil
end

function runtime.EnsureOutfitBrowser()
    local state = runtime.OutfitBrowser
    if state.Gui and state.Gui.Parent then return state end
    ...
    local gui = Instance.new("ScreenGui")
    gui.Name = "XeroHub_OutfitBrowser"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.DisplayOrder = 2147483647
    ...
    panel.Position = UDim2.fromOffset(400, 300)
    panel.Size = UDim2.fromOffset(820, 700)
    ...
    -- Evita subpíxeles: CanvasGroup + porcentajes podían rasterizar el texto borroso
    -- en móvil. El panel se centra y dimensiona siempre en offsets enteros.
    local function fitPanelToViewport()
        local size = overlay.AbsoluteSize
        if size.X < 16 or size.Y < 16 then return end
        local width = math.floor(math.clamp(size.X - 24, 300, 820))
        local height = math.floor(math.clamp(size.Y - 32, 360, 700))
        panel.Size = UDim2.fromOffset(width, height)
        panel.Position = UDim2.fromOffset(math.floor(size.X * 0.5), math.floor(size.Y * 0.5))
    end
    overlay:GetPropertyChangedSignal("AbsoluteSize"):Connect(fitPanelToViewport)
    task.defer(fitPanelToViewport)
    ...
    count.Position = UDim2.fromOffset(20, 41)
    count.Size = UDim2.new(1, -76, 0, 17)
    count.BackgroundTransparency = 1
    count.Text = ""
    count.TextColor3 = Color3.fromRGB(130, 130, 138)
    ...
    searchWrap.Position = UDim2.fromOffset(16, 76)
    searchWrap.Size = UDim2.new(1, -32, 0, 42)
    searchWrap.BackgroundColor3 = Color3.fromRGB(14, 14, 17)
    searchWrap.BorderSizePixel = 0
    searchWrap.ZIndex = 502
    searchWrap.Parent = panel
    Instance.new("UICorner", searchWrap).CornerRadius = UDim.new(0, 12)
    ...
    searchBox.Position = UDim2.fromOffset(13, 0)
    searchBox.Size = UDim2.new(1, -26, 1, 0)
    searchBox.BackgroundTransparency = 1
    searchBox.ClearTextOnFocus = false
    searchBox.PlaceholderText = "Buscar outfit..."
    searchBox.PlaceholderColor3 = Color3.fromRGB(92, 92, 101)
    searchBox.Text = ""
    ...
    scroll.Position = UDim2.fromOffset(12, 128)
    scroll.Size = UDim2.new(1, -24, 1, -140)
    scroll.BackgroundTransparency = 1
    scroll.BorderSizePixel = 0
    scroll.ScrollBarThickness = 3
    scroll.ScrollBarImageColor3 = Color3.fromRGB(95, 95, 104)
    ...
    grid.CellSize = UDim2.new(0.5, -7, 0, 188)
    ...
    local function updateGrid()
        ...
        if width < 460 then
            grid.CellSize = UDim2.new(1, -8, 0, 156)
        elseif width < 700 then
            grid.CellSize = UDim2.new(0.5, -7, 0, 174)
        else
            grid.CellSize = UDim2.new(0.333333, -8, 0, 178)
        end
    end
    ...
    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        if runtime.RenderOutfitBrowser then runtime.RenderOutfitBrowser() end
    end)

    return state
end

function runtime.RenderOutfitBrowser()
    local state = runtime.OutfitBrowser
    local scroll = state.Scroll
    if not scroll then return end

    state.RenderSerial += 1
    local serial = state.RenderSerial
    local query = string.lower(tostring(state.SearchBox and state.SearchBox.Text or ""))
    ...
    for index, outfit in ipairs(state.CurrentOutfits or {}) do
        local nameLower = string.lower(outfit.Name)
        if query == "" or string.find(nameLower, query, 1, true) then
            visibleCount += 1

            local card = Instance.new("TextButton")
            card.Name = "Outfit_" .. tostring(outfit.Id)
            ...
            cardStroke.Color = Color3.fromRGB(45, 45, 51)
            cardStroke.Transparency = 0.28

            local image = Instance.new("ImageLabel")
            image.Position = UDim2.fromOffset(8, 8)
            image.Size = UDim2.new(1, -16, 1, -58)
            image.BackgroundColor3 = Color3.fromRGB(7, 7, 9)
            image.BorderSizePixel = 0
            image.Image = "rbxthumb://type=Outfit&id=" .. tostring(outfit.Id) .. "&w=150&h=150"
            ...
            name.Position = UDim2.new(0, 10, 1, -43)
            ...
            hint.Position = UDim2.new(0, 10, 1, -24)
            hint.Size = UDim2.new(1, -20, 0, 14)
            hint.BackgroundTransparency = 1
            hint.Text = "TOCAR PARA CLONAR"
            ...
            card.MouseEnter:Connect(function()
                cardStroke.Color = Color3.fromRGB(105, 105, 116)
            end)
            ...
            card.Activated:Connect(function()
                if serial ~= state.RenderSerial then return end
                task.spawn(function()
                    showBottomMessage("Cargando outfit · " .. outfit.Name .. "...")
                    local ok, err = runtime.CloneAvatarFromOutfitId(
                        outfit.Id,
                        outfit.Name,
                        state.CurrentUserId,
                    ...
                    if ok then
                        ...
                        runtime.RestoreOutfitBrowserGuiOrder()
                        showBottomMessage("Skin clonada · " .. outfit.Name)
                    else
                        warn("[XeroHub] Outfit:", err)
                        showBottomMessage("No se pudo clonar ese outfit: " .. tostring(err or "error"))
                    end
                end)
            end)
        end
    ...
    if state.Count then
        local total = #(state.CurrentOutfits or {})
        if query ~= "" then
            state.Count.Text = tostring(visibleCount) .. " de " .. tostring(total) .. " outfits"
        else
            state.Count.Text = tostring(total) .. (total == 1 and " outfit" or " outfits")
        end
    end
end

function runtime.OpenUserOutfits(forceRefresh)
    local userId, username, resolveError = runtime.ResolveOutfitBrowserUser()
    if not userId then
        showBottomMessage(resolveError)
        return false
    end

    local state = runtime.EnsureOutfitBrowser()
    state.CurrentUserId = userId
    state.CurrentUserName = username
    state.Title.Text = "Skins de @" .. tostring(username)
    state.Count.Text = "Cargando outfits..."
    state.SearchBox.Text = ""
    runtime.BringOutfitBrowserToFront()
    state.Overlay.Visible = true

    task.spawn(function()
        local outfits, err = runtime.FetchUserOutfits(userId, forceRefresh == true)
        if not runtime.Alive or not state.Gui or not state.Gui.Parent then return end
        if state.CurrentUserId ~= userId then return end

        if not outfits then
            state.Count.Text = "No se pudieron cargar"
            showBottomMessage(tostring(err or "No se pudieron cargar los outfits"))
            return
        end

        state.CurrentOutfits = outfits
        runtime.RenderOutfitBrowser()
        if #outfits == 0 then
            showBottomMessage("Ese usuario no tiene outfits guardados públicos.")
        end
    end)
...
local introAppearance = Tabs.Apariencia:Paragraph({
    ...
    Desc = "Activa tus accesorios visuales y, si quieres ajustarlos fino, abre el editor visual."
})
...
runtime.SetupAvatarCloneUI = function()
    ...
    local browseOutfitsButton = Tabs.Apariencia:Button({
        Title = "Ver skins del usuario",
        Desc = "Muestra sus outfits guardados con miniatura y te deja clonar cualquiera.",
        Callback = function()
            runtime.OpenUserOutfits(false)
        end,
    })
    placeAppearanceElement(browseOutfitsButton, nextAppearanceOrder())

    local refreshOutfitsButton = Tabs.Apariencia:Button({
        Title = "Actualizar skins guardadas",
        Desc = "Vuelve a consultar Roblox por si el usuario creó o cambió outfits.",
        Callback = function()
            runtime.OpenUserOutfits(true)
        end,
    })
    placeAppearanceElement(refreshOutfitsButton, nextAppearanceOrder())
...
-- XERO AIM A/B TEST: misma lógica que AimFix, pero SIN spoof de __index / Mouse.Hit / Mouse.Target.
local AIM_HOOK_LIFECYCLE_VERSION = 3 -- EXPERIMENTO: __namecall únicamente, sin __index
local aimHookState = runtimeEnv.__ILUNX_AIM_HOOK_STATE

-- Un hook de una versión vieja no guardaba los metamethods originales, así que no
-- podemos desmontarlo de forma fiable sin una sesión limpia. No apilamos otro hook
-- encima: lo dejamos pasivo (Target=nil) hasta el próximo rejoin.
local legacyAimHook = type(aimHookState) == "table"
    and (aimHookState.LegacyPermanent == true
        or aimHookState.Version ~= AIM_HOOK_LIFECYCLE_VERSION)

if type(aimHookState) ~= "table" then
    aimHookState = {}
    runtimeEnv.__ILUNX_AIM_HOOK_STATE = aimHookState
end

aimHookState.Version = AIM_HOOK_LIFECYCLE_VERSION
aimHookState.Target = nil
aimHookState.Mouse = mouse
aimHookState.Owner = runtime
aimHookState.Wanted = false
aimHookState.RoundActive = false
aimHookState.LegacyPermanent = legacyAimHook
aimHookState.Installed = legacyAimHook and true or false

aimHookState.OldNamecall = legacyAimHook and nil or aimHookState.OldNamecall
aimHookState.OldIndex = nil -- EXPERIMENTO A/B: no instalamos spoof de Mouse.Hit/Mouse.Target

local function wrapAimHook(fn)
...
function runtime.InstallAimHooks()
    local state = aimHookState
    if not runtime.Alive or state.Owner ~= runtime then return false end
    if state.LegacyPermanent then
        -- Sesión heredada: ya existe el hook viejo. Sólo lo usamos con Target
        -- mientras haya ronda para no crear una segunda cadena de metamethods.
    ...
    local namecallHook = wrapAimHook(function(self, ...)
        local target = state.RoundActive and state.Wanted and state.Target or nil
                ...
                if self == workspace then
                    if method == "Raycast" then
                        ...
                        if typeof(direction) == "Vector3"
                            ...
                            return state.OldNamecall(self, origin, newDir, p3)
                        end
                    elseif method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" then
                        ...
                        if typeof(ray) == "Ray"
                            ...
                            return state.OldNamecall(self, newRay, p2, p3, p4)
                        end
                    end
                end
            else
    ...
    local okNamecall, oldNamecall = pcall(hookmetamethod, game, "__namecall", namecallHook)
    ...
    -- ==========================================================
    -- EXPERIMENTO NO-INDEX
    -- Deliberadamente NO hookeamos game.__index.
    -- Mouse.Hit y Mouse.Target quedan 100% originales.
    -- Sólo se conserva la redirección de Raycast / FindPartOnRay
    -- que ya vive en __namecall arriba.
    -- ==========================================================
    state.OldIndex = nil
    state.Installed = true
    return true
...
function runtime.UninstallAimHooks(force)
    local state = aimHookState
    state.Target = nil

    if state.LegacyPermanent then
        -- El hook de una ejecución antigua no expuso sus originales. Lo mantenemos
        -- completamente pasivo; una sesión nueva sí podrá usar desmontaje real.
        state.Wanted = false
    ...
    -- EXPERIMENTO NO-INDEX: sólo existe un hook de __namecall que restaurar.
    if oldNamecall then pcall(hookmetamethod, game, "__namecall", oldNamecall) end
...
function runtime.RefreshAimHooks(inMatchOverride)
    local state = aimHookState
    if state.Owner ~= runtime or not runtime.Alive then return end

    local wanted = autoShootEnabled or autoShootCuchilloEnabled
        or silentAimPistolaEnabled or silentAimCuchilloEnabled

    local inMatch = inMatchOverride
    if inMatch == nil then
        local ok, result = pcall(runtime.IsCombatRound)
        inMatch = ok and result == true or false
    else
        inMatch = inMatch == true
    end

    state.Wanted = wanted == true
    state.RoundActive = inMatch

    if state.Wanted and state.RoundActive then
        runtime.InstallAimHooks()
    else
        runtime.UninstallAimHooks(false)
        -- Uninstall no debe borrar el estado real de ronda; sólo el hook.
        state.RoundActive = inMatch
        state.Wanted = wanted == true
    end
end

-- AutoShoot puede encontrar objetivo entre dos refrescos de lobby (0.25 s).
-- Antes de disparar garantizamos que el hook ya esté montado; esta función vuelve
-- a consultar IsCombatRound(), por lo que jamás lo fuerza dentro de zona segura.
function runtime.EnsureAimHooksReady()
    local state = aimHookState
    if state.Owner ~= runtime or not runtime.Alive then return false end
    if state.Installed and state.RoundActive and state.Wanted then return true end
    runtime.RefreshAimHooks()
    return state.Installed and state.RoundActive and state.Wanted
end

-- Cambios nativos del juego que permiten cortar el hook antes del próximo tick.
runtime.Track(player:GetPropertyChangedSignal("Team"):Connect(function()
    task.defer(runtime.RefreshAimHooks)
end))
...
local ESP_GLOW_FILL_TRANSPARENCY = 0.45
...
function runtime.SyncESPGlow(targetPlayer, char, visible)
    ...
    if espObj and (espObj.Char ~= char
        or not espObj.Highlight
        or not espObj.Highlight.Parent) then
        cleanESP(targetPlayer)
        espObj = nil
    end

    if not visible or not char then
    ...
    if not espObj then
        ...
        activeESPs[targetPlayer] = espObj
    end
...
-- Este estado pertenece a ESP/Hitbox/AutoShoot y conserva el filtro visual clásico.
-- Los hooks tienen su propio gate estricto en runtime.IsCombatRound().
local enLobby = true
local timerLobby = 1
...
runtime.Track(RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if timerLobby >= 0.25 then
        timerLobby = 0

        -- IMPORTANTE: no compartir el gate estricto de hooks con ESP/Hitbox/AutoShoot.
        -- El filtro visual conserva el método que ya funcionaba: zonas/ForceField/equipo lobby.
        local okLobby, lobbyResult = pcall(estaEnLobby)
        enLobby = (not okLobby) or lobbyResult == true

        -- Los metamethod hooks se montan sólo fuera del filtro real de lobby/zona segura.
        local okRound, inMatch = pcall(runtime.IsCombatRound)
        inMatch = okRound and inMatch == true or false
        runtime.RefreshAimHooks(inMatch)
    end
    ...
    -- 2. ESP
    -- ==========================================
    -- Glow/Nombre/Distancia/Vida/Box comparten ahora la MISMA validación
    -- en RenderESP2D. El Heartbeat maestro sólo conserva el estado global.
    if espEnabled then
        mState.espAct = true
                    ...
                    if arma and arma:FindFirstChild("Handle") then
                        ...
                        if (esGun and autoShootEnabled) or (not esGun and autoShootCuchilloEnabled) then
                            ...
                            if closestTargetPart then
                                -- Montaje just-in-time: evita que el primer disparo de una ronda
                                -- ocurra antes del refresco periódico de hooks.
                                runtime.EnsureAimHooksReady()
...
runtime.VisualConnections = runtime.VisualConnections or {}
runtime.VisualTextConnections = runtime.VisualTextConnections or setmetatable({}, {__mode = "k"})
runtime.VisualInitialScanDone = false

local function isSpoofTextObject(v)
    return v and (v:IsA("TextLabel") or v:IsA("TextBox") or v:IsA("TextButton"))
end

local function isHubTextObject(v)
    local parentGui = v and v:FindFirstAncestorWhichIsA("ScreenGui")
    if not parentGui then return false end
    local guiName = tostring(parentGui.Name or "")
    return string.find(guiName, "WindUI", 1, true) ~= nil
        or string.find(guiName, "iLunX", 1, true) ~= nil
        or string.find(guiName, "Onyx", 1, true) ~= nil
        or string.find(guiName, "XeroHub", 1, true) ~= nil
end

local function safeReplace(str, find, replace)
    str, find, replace = tostring(str or ""), tostring(find or ""), tostring(replace or "")
    if find == "" then return str end
    local safeFind = find:gsub("[%-%^%$%(%)%%%.%[%]%*%+%?]", "%%%1")
    return (str:gsub(safeFind, replace))
end

local function restoreStrokeState(data)
    for stroke, strokeData in pairs((data and data.Strokes) or {}) do
...
local function restoreTextState(v, data)
    if not v or not v.Parent or not data then return end
    v.Text = data.Text
    v.TextColor3 = data.Color
    v.TextTransparency = data.TextTransp
    v.TextStrokeTransparency = data.StrokeTransp
    if data.RichText ~= nil then v.RichText = data.RichText end
    restoreStrokeState(data)
end

local function processText(v, myName, myDisp)
    if not isSpoofTextObject(v) or isHubTextObject(v) then return end

    local txt = tostring(v.Text or "")
    local hasName = txt ~= "" and (
        string.find(txt, myName, 1, true) ~= nil
        or string.find(txt, myDisp, 1, true) ~= nil
    )
    ...
    if fakeNameEnabled or creatorTagEnabled then
        ...
        if player.Character then
            if v:IsDescendantOf(player.Character) then
                ...
                local parentGui = v:FindFirstAncestorWhichIsA("BillboardGui")
                if parentGui and parentGui.Adornee and parentGui.Adornee:IsDescendantOf(player.Character) then
                    isOverheadTag = true
                end
            end
        end
        ...
        v.TextStrokeTransparency = data.StrokeTransp
        restoreStrokeState(data)
        v.TextColor3 = rainbowEnabled and Color3.fromHSV(tick() % 4 / 4, 1, 1) or data.Color
        return
    end

    restoreTextState(v, data)
    if rainbowEnabled then
        v.TextColor3 = Color3.fromHSV(tick() % 4 / 4, 1, 1)
    end
end

local function disconnectVisualTextConnections()
    for v, connection in pairs(runtime.VisualTextConnections) do
        if connection then pcall(function() connection:Disconnect() end) end
        if v then
            pcall(function() v:SetAttribute("AstraInfectado", nil) end)
            runtime.InfectedTexts[v] = nil
        end
        runtime.VisualTextConnections[v] = nil
    end
end

local function infectarTextoSeguro(v)
    if not isWorkspaceLooping or not isSpoofTextObject(v) or isHubTextObject(v) then return end

    local myName = player.Name
    local myDisp = player.DisplayName
    processText(v, myName, myDisp)

    if runtime.VisualTextConnections[v] then return end
    pcall(function() v:SetAttribute("AstraInfectado", true) end)
    runtime.TrackInfectedText(v)

    local connection
    connection = runtime.Track(v:GetPropertyChangedSignal("Text"):Connect(function()
        if not isWorkspaceLooping or not v.Parent then return end
        local currentText = tostring(v.Text or "")
        if currentText == " " or currentText == spoofNameText
            or string.find(currentText, spoofNameText, 1, true)
            or string.find(currentText, "[Content Creator]", 1, true) then
            return
        end

        local currentName = player.Name
        local currentDisplay = player.DisplayName
        if string.find(currentText, currentName, 1, true)
            or string.find(currentText, currentDisplay, 1, true) then
            -- El juego actualizó el texto real: refrescamos el snapshot, no uno
            -- que ya estaba oculto por XeroHub.
            originalData[v] = nil
            processText(v, currentName, currentDisplay)
        end
    end))
    runtime.VisualTextConnections[v] = connection
end

local function scanExistingSpoofTexts()
    task.spawn(function()
        for _, p in ipairs(listaJugadores) do
            if not isWorkspaceLooping then return end
            local character = p.Character
            if character then
                for _, v in ipairs(character:GetDescendants()) do
                    if isSpoofTextObject(v) then infectarTextoSeguro(v) end
                end
            end
        end
...
local function startVisualWatchers()
    if isWorkspaceLooping then return end
    ...
    if pGui then
        table.insert(runtime.VisualConnections, runtime.Track(pGui.DescendantAdded:Connect(function(nuevoObjeto)
            if isWorkspaceLooping and isSpoofTextObject(nuevoObjeto) then
    ...
    scanExistingSpoofTexts()
end

local function stopVisualWatchersAndRestore()
    -- Primero apagamos la bandera para que restaurar Text no vuelva a disparar
    -- el propio spoofer mediante TextChanged.
    isWorkspaceLooping = false
    runtime.VisualInitialScanDone = false

    for _, conn in ipairs(runtime.VisualConnections) do
        pcall(function() conn:Disconnect() end)
    end
    table.clear(runtime.VisualConnections)
    disconnectVisualTextConnections()
    ...
    -- El snapshot se borra al apagar: la siguiente activación captura el estado
    -- REAL actual del juego, evitando restaurar una transparencia vieja/invisible.
...
runtime.VisualSpoofCleanup = function()
    hideNameEnabled = false
    fakeNameEnabled = false
    rainbowEnabled = false
    creatorTagEnabled = false
...
UIElements.TogEspGl = Tabs.Vis:Toggle({
    ...
    Callback = function(s)
        espSettings.Glow = s
        if not s and runtime.HideAllESPGlow then runtime.HideAllESPGlow() end
        if runtime.UpdateESP2DRenderConnection then runtime.UpdateESP2DRenderConnection() end
    end
})
...
-- ==========================================
-- Scope propio: evita mantener los locals del renderer vivos hasta el final del chunk.
do
...
-- Scope propio: libera Networking/RemoteFarm/cajas al terminar esta pestaña.
do
...
-- ==========================================
-- Scope propio: evita que ~20 locals del inventario permanezcan activos.
do
...
end -- Inventario / armas scope

Tabs.Config:Section({ Title = "Personalización de Interfaz" })

Tabs.Config:Paragraph({
    Title = "Xero",
    Desc = "Negro, blanco y una interfaz hecha para XeroHub.",
})
...
-- Auto Load usa un archivo de estado aparte para no contaminar la lista de configs.
-- La extensión .xero evita que refreshConfigs() lo trate como una configuración normal.
...
local function readAutoLoadState()
    if type(isfile) ~= "function" or type(readfile) ~= "function" or not isfile(autoLoadPath) then
        return
    end
    local ok, decoded = pcall(function()
    ...
    if ok and type(decoded) == "table" then
        ...
        if type(decoded.Config) == "string" and decoded.Config ~= "" and decoded.Config ~= "Ninguna" then
...
local configDropdown = Tabs.Config:Dropdown({
    ...
    Value = "Ninguna",
    Callback = function(Value)
        local value = type(Value) == "table" and Value[1] or Value
        selectedConfig = tostring(value or "Ninguna")

        -- Si Auto Load está activo, cambiar el dropdown cambia también
        -- cuál configuración se abrirá en la próxima ejecución.
        if not configDropdownSyncing and autoLoadEnabled and selectedConfig ~= "Ninguna" then
    ...
    Callback = function(Value)
        ...
        if Value then
            if selectedConfig == "" or selectedConfig == "Ninguna" then
                ...
                saveAutoLoadState()
                showBottomMessage("Selecciona una configuración antes de activar Auto Load.", {
            ...
            autoLoadConfigName = selectedConfig
            saveAutoLoadState()
            showBottomMessage("Auto Load: " .. selectedConfig, {
                title = "XeroHub · Config",
                key = "config:autoload",
            })
        else
            autoLoadEnabled = false
            saveAutoLoadState()
            showBottomMessage("Auto Load desactivado.", {
                title = "XeroHub · Config",
                key = "config:autoload",
            })
        end
...
function refreshConfigs()
    ...
    pcall(function()
        ...
        local desiredConfig = selectedConfig
        if autoLoadEnabled and autoLoadConfigName and table.find(list, autoLoadConfigName) then
            desiredConfig = autoLoadConfigName
        end
        if not table.find(list, desiredConfig) then
            desiredConfig = list[1]
        end

        configDropdownSyncing = true
        configDropdown:Select(desiredConfig)
        configDropdownSyncing = false
        selectedConfig = desiredConfig
    end)
end
...
Tabs.Config:Button({ Title = "Guardar Configuración", Callback = function()
    ...
    local configData = {
        ...
        DeathEffects = runtime.SerializeDeathEffectConfig and runtime.SerializeDeathEffectConfig() or nil,
        Texturas = runtime.SerializeTextureConfig and runtime.SerializeTextureConfig() or nil,
...
local function loadConfigByName(configName, loadMode)
    selectedConfig = tostring(configName or selectedConfig or "Ninguna")
    ...
    if isfile and isfile(path) then
        ...
        if success and type(decoded) == "table" then
            ...
            if decoded.DeathEffects and runtime.LoadDeathEffectConfig then
                runtime.LoadDeathEffectConfig(decoded.DeathEffects)
            end

            if decoded.Texturas and runtime.LoadTextureConfig then
                runtime.LoadTextureConfig(decoded.Texturas)
            end
            ...
            if loadMode == "auto" then
                showBottomMessage("Auto Load: '" .. selectedConfig .. "' cargada.", {
                    title = "XeroHub · Config",
                    key = "config:autoload:loaded",
                })
            else
...
Tabs.Config:Button({
    ...
    Callback = function()
        loadConfigByName(selectedConfig, "manual")
    end
})

-- Cargar lista y, si el usuario lo dejó activado, ejecutar su config elegida.
task.spawn(function()
    task.wait(1)
    refreshConfigs()
    ...
    if autoLoadEnabled and autoLoadConfigName then
        local path = configPaths[autoLoadConfigName] or (configFolder .. "/" .. autoLoadConfigName .. ".json")
        if type(isfile) == "function" and isfile(path) then
            ...
            task.wait(0.10)
            loadConfigByName(autoLoadConfigName, "auto")
        else
            showBottomMessage("Auto Load: no se encontró '" .. tostring(autoLoadConfigName) .. "'.", {
                title = "XeroHub · Config",
                key = "config:autoload:error",
            })
        end
    end
end)
...
-- ============================================================
-- XERO APPEARANCE SYNC · aislado en chunk propio para no gastar
-- locales del chunk principal de XeroHub.
-- ============================================================
assert(loadstring([====[
return function(runtime, player, Players, HttpService, workspace, RunService, isHairAccessory, manualAttachAppearanceAccessory, KORBLOX_UPPER_MESH, KORBLOX_TEXTURE)
-- ============================================================
-- XERO APPEARANCE SYNC · usuarios del hub en el mismo JobId
-- ============================================================
-- El backend sólo intercambia DESCRIPCIONES visuales. Posición/pose siguen
-- viniendo de la replicación normal de Roblox; nunca enviamos CFrames por HTTP.
do
    local syncState = {
        Endpoint = "https://hub.onyx-scripts.com/appearance_sync",
        Entries = {},
        RenderEntries = {},
        TemplateCache = {},
        TemplateCacheLimit = 8,
        HiddenEnforceInterval = 0.10,
        Revision = 0,
        LastSerialized = nil,
        LastPublishAt = 0,
        Alive = true,
    }
    runtime.AppearanceSync = syncState

    local syncRequest = (syn and syn.request) or (http and http.request) or http_request or request
    ...
    local function vecToArray(v)
        return {
            numberOr(v and v.X, 0),
            numberOr(v and v.Y, 0),
            numberOr(v and v.Z, 0),
        }
    end
    ...
    local function buildLocalAppearanceState()
        local enabled = {}
        local offsets = {}

        for _, key in ipairs(runtime.AppearanceOrder) do
            local asset = runtime.AppearanceCatalog[key]
            if asset and runtime.Appearance.Enabled[key] == true
                and asset.Kind ~= "Headless" and asset.Kind ~= "Korblox"
                and asset.Kind ~= "HairToggle" then

                enabled[#enabled + 1] = key

                if asset.Kind == "Accessory" then
                    local offset = runtime.Appearance.AccessoryOffsets[key]
                    if offset then
                        offsets[key] = {
                            p = vecToArray(offset.Position),
                            r = vecToArray(offset.Rotation),
                            s = math.clamp(numberOr(offset.Scale, 1), 0.25, 3),
                        }
                    end
                end
            end
        ...
        local cloneState = runtime.Appearance.AvatarClone
        local baseUserId = player.UserId
        if cloneState and cloneState.Active and tonumber(cloneState.TargetUserId) then
            baseUserId = tonumber(cloneState.TargetUserId)
        end

        return {
            baseUserId = baseUserId,
            outfitId = cloneState and cloneState.Active and tonumber(cloneState.TargetOutfitId) or nil,
            headless = runtime.Appearance.Enabled.Headless == true,
            korblox = runtime.Appearance.Enabled.Korblox == true,
            hideHair = runtime.Appearance.Enabled.HideHair == true,
            enabled = enabled,
            offsets = offsets,
        }
    end

    local function httpPostState(state, revision)
        if not syncRequest or not runtime.Alive or not syncState.Alive then return false end
        ...
        local ok, response = pcall(function()
            return syncRequest({
                Url = syncState.Endpoint,
                Method = "POST",
                Headers = {
                    ["Astra-Auth"] = "OnyxHub!",
                    ["User-Agent"] = "Roblox/XeroHub-AppearanceSync",
                    ["Content-Type"] = "application/json",
                },
                Body = payload,
            })
        end)
        return ok and response and tonumber(response.StatusCode or response.Status or 0) >= 200
            and tonumber(response.StatusCode or response.Status or 0) < 300
    end

    local function httpGetStates()
        if not syncRequest or not runtime.Alive or not syncState.Alive then return nil end
        local url = syncState.Endpoint .. "?jobid=" .. HttpService:UrlEncode(tostring(game.JobId))
        local ok, response = pcall(function()
            return syncRequest({
                Url = url,
                Method = "GET",
        ...
        local status = tonumber(response.StatusCode or response.Status or 0) or 0
        if status < 200 or status >= 300 or type(response.Body) ~= "string" then return nil end
        local decodedOk, decoded = pcall(function() return HttpService:JSONDecode(response.Body) end)
        if not decodedOk or type(decoded) ~= "table" or decoded.success ~= true then return nil end
        return type(decoded.users) == "table" and decoded.users or {}
    end

    local function getSyncFolder()
        local folder = workspace:FindFirstChild("XeroHub_RemoteAppearances")
        if not folder then
            folder = Instance.new("Folder")
            folder.Name = "XeroHub_RemoteAppearances"
            folder.Parent = workspace
        end
        return folder
    end

    local function addRenderEntry(entry)
        if not entry or entry.RenderIndex then return end
        local list = syncState.RenderEntries
        local index = #list + 1
        list[index] = entry
        entry.RenderIndex = index
    end

    local function removeRenderEntry(entry)
        if not entry then return end
        local index = entry.RenderIndex
        if not index then return end
        local list = syncState.RenderEntries
        local last = list[#list]
        list[index] = last
        list[#list] = nil
        if last and last ~= entry then last.RenderIndex = index end
        entry.RenderIndex = nil
    end

    local function disconnectEntry(entry)
        if not entry then return end
        entry.Generation = (entry.Generation or 0) + 1
        removeRenderEntry(entry)
        entry.SyncPose = nil
        entry.NextHiddenEnforceAt = nil
        ...
        local hiddenList = entry.HiddenList or {}
        for index = 1, #hiddenList do
            local object = hiddenList[index]
            local oldValue = entry.Hidden and entry.Hidden[object]
            if oldValue ~= nil and object and object.Parent and object:IsA("BasePart") then
                pcall(function() object.LocalTransparencyModifier = oldValue end)
            end
        end
        entry.Hidden = setmetatable({}, {__mode = "k"})
        entry.HiddenList = {}
        entry.HiddenIndex = setmetatable({}, {__mode = "k"})
        entry.ToolGuards = setmetatable({}, {__mode = "k"})

        if entry.Overlay then
            pcall(function() entry.Overlay:Destroy() end)
            entry.Overlay = nil
        end
        entry.RemoteFace = nil
        entry.MotorPairs = nil
    end

    local function destroyEntry(userId)
        local entry = syncState.Entries[userId]
        if not entry then return end
        disconnectEntry(entry)
        syncState.Entries[userId] = nil
    end

    -- Mismo filtro de armas que usa el clonador local. El Character remoto puede
    -- recibir/reparentar Handles, MeshParts y efectos DESPUÉS de que ocultamos su
    -- avatar base; cualquier cosa que termine dentro de un Tool se libera al instante.
    local function isToolVisual(object, char)
        if not object then return false end
        local tool = object:IsA("Tool") and object or object:FindFirstAncestorWhichIsA("Tool")
        return tool ~= nil and (not char or tool:IsDescendantOf(char))
    end

    local function untrackRemoteHiddenObject(entry, object)
        if not entry or not object then return end
        local index = entry.HiddenIndex and entry.HiddenIndex[object]
        if index then
            local list = entry.HiddenList
            local lastIndex = #list
            local last = list[lastIndex]
            list[index] = last
            list[lastIndex] = nil
            if last and last ~= object then
                entry.HiddenIndex[last] = index
            end
            entry.HiddenIndex[object] = nil
        end
        if entry.Hidden then entry.Hidden[object] = nil end
    end

    local function restoreRemoteHiddenObject(entry, object)
        if not entry or not object then return end
        local oldValue = entry.Hidden and entry.Hidden[object]
        if oldValue == nil then return end
        if object.Parent and object:IsA("BasePart") then
            pcall(function() object.LocalTransparencyModifier = oldValue end)
        end
        untrackRemoteHiddenObject(entry, object)
    end

    local function enforceRemoteHidden(entry, char, now, force)
        if not entry or not char then return end
        now = now or os.clock()
        if not force and now < (entry.NextHiddenEnforceAt or 0) then return end
        entry.NextHiddenEnforceAt = now + syncState.HiddenEnforceInterval

        local list = entry.HiddenList or {}
        local index = 1
        while index <= #list do
            local object = list[index]
            if not object or not object.Parent then
                untrackRemoteHiddenObject(entry, object)
            elseif isToolVisual(object, char) then
                restoreRemoteHiddenObject(entry, object)
            else
    ...
    local function releaseRemoteToolVisuals(entry, char, tool)
        if not entry or not char or not tool or not tool:IsA("Tool")
            or not tool:IsDescendantOf(char) then return end

        restoreRemoteHiddenObject(entry, tool)
        for _, object in ipairs(tool:GetDescendants()) do
            restoreRemoteHiddenObject(entry, object)
        end
    end

    local function bindRemoteToolGuard(entry, char, tool)
        if not entry or not char or not tool or not tool:IsA("Tool")
            or not tool:IsDescendantOf(char) then return end

        entry.ToolGuards = entry.ToolGuards or setmetatable({}, {__mode = "k"})
        if entry.ToolGuards[tool] then return end

        local bundle = {}
        entry.ToolGuards[tool] = bundle
        releaseRemoteToolVisuals(entry, char, tool)

        bundle.DescendantAdded = tool.DescendantAdded:Connect(function(object)
            if entry.Character ~= char then return end
            restoreRemoteHiddenObject(entry, object)
        end)
        table.insert(entry.Connections, bundle.DescendantAdded)
        ...
        bundle.AncestryChanged = tool.AncestryChanged:Connect(function()
            ...
            if entry.ToolGuards and entry.ToolGuards[tool] == bundle then
                entry.ToolGuards[tool] = nil
            end
        end)
        table.insert(entry.Connections, bundle.AncestryChanged)
    end

    local function hideRemoteBaseObject(entry, object, char)
        if not object or not object.Parent then return end

        -- Si algo que ya estaba cacheado como cuerpo terminó siendo parte de un arma,
        -- restauramos su LTM en vez de dejarlo invisible.
        if isToolVisual(object, char) then
            restoreRemoteHiddenObject(entry, object)
            local tool = object:IsA("Tool") and object or object:FindFirstAncestorWhichIsA("Tool")
            if tool then bindRemoteToolGuard(entry, char, tool) end
            return
        end

        if object:IsA("BasePart") and object.Name ~= "HumanoidRootPart" and object:IsDescendantOf(char) then
            if entry.Hidden[object] == nil then
                entry.Hidden[object] = object.LocalTransparencyModifier
                local list = entry.HiddenList
                local index = #list + 1
                list[index] = object
                entry.HiddenIndex[object] = index
            end
            if object.LocalTransparencyModifier ~= 1 then
                object.LocalTransparencyModifier = 1
            end
    ...
    local function bindRemoteBaseHidden(entry, char)
        entry.ToolGuards = setmetatable({}, {__mode = "k"})
        entry.HiddenList = entry.HiddenList or {}
        entry.HiddenIndex = entry.HiddenIndex or setmetatable({}, {__mode = "k"})

        -- Primero registramos armas ya equipadas para que jamás entren al hide inicial.
        for _, child in ipairs(char:GetChildren()) do
            if child:IsA("Tool") then bindRemoteToolGuard(entry, char, child) end
        end

        for _, object in ipairs(char:GetDescendants()) do
            hideRemoteBaseObject(entry, object, char)
        end

        local descendantConnection = char.DescendantAdded:Connect(function(object)
            if entry.Character ~= char then return end
            task.defer(function()
                if not object or not object.Parent or entry.Character ~= char then return end
                local tool = object:IsA("Tool") and object or object:FindFirstAncestorWhichIsA("Tool")
                if tool and tool:IsDescendantOf(char) then
                    bindRemoteToolGuard(entry, char, tool)
                    releaseRemoteToolVisuals(entry, char, tool)
                else
                    hideRemoteBaseObject(entry, object, char)
                end
        ...
        table.insert(entry.Connections, descendantConnection)

        local childConnection = char.ChildAdded:Connect(function(child)
            if entry.Character ~= char or not child:IsA("Tool") then return end
            bindRemoteToolGuard(entry, char, child)
        end)
        table.insert(entry.Connections, childConnection)
    end
    ...
    local function findAccessoryWeld(accessory)
        ...
        if not handle then return nil end
        local preferred = handle:FindFirstChild("AccessoryWeld") or handle:FindFirstChild("iLunXAppearanceWeld")
        if preferred and preferred:IsA("Weld") then return preferred end
        for _, object in ipairs(handle:GetChildren()) do
            if object:IsA("Weld") then return object end
        end
        return nil
    end

    local function scaleRemoteAccessory(accessory, scale)
        scale = math.clamp(numberOr(scale, 1), 0.25, 3)
        if math.abs(scale - 1) < 0.0001 then return end
        for _, object in ipairs(accessory:GetDescendants()) do
            if object:IsA("SpecialMesh") then
                object.Scale = object.Scale * scale
            elseif object:IsA("BasePart") and not object:FindFirstChildOfClass("SpecialMesh") then
                object.Size = object.Size * scale
            end
        end
    end

    local function attachRemoteAccessory(overlay, key, offsetData)
        local template = runtime.GetAppearanceTemplate(key)
        local humanoid = overlay and overlay:FindFirstChildOfClass("Humanoid")
        if not template or not humanoid then return end

        local accessory = template:Clone()
        accessory:SetAttribute("XeroHubRemoteAppearanceKey", key)
        local ok = pcall(function() humanoid:AddAccessory(accessory) end)
        if not ok or accessory.Parent ~= overlay then
            accessory.Parent = overlay
            pcall(function() manualAttachAppearanceAccessory(overlay, accessory) end)
        end

        if accessory.Parent ~= overlay then
            pcall(function() accessory:Destroy() end)
            return
        end

        local data = type(offsetData) == "table" and offsetData or {}
        scaleRemoteAccessory(accessory, data.s)
        ...
        if weld then
            local p = arrayToVector(data.p)
            local r = arrayToVector(data.r)
            local baseC1 = weld.C1
            weld.C1 = baseC1
                * CFrame.new(p.X, p.Y, p.Z)
    ...
    local function getRemoteHeadScale(overlay)
        local humanoid = overlay and overlay:FindFirstChildOfClass("Humanoid")
        if not humanoid then return 1 end
        local value = humanoid:FindFirstChild("HeadScale")
        if value and value:IsA("NumberValue") then
            return math.clamp(numberOr(value.Value, 1), 0.5, 2)
        end
        local ok, description = pcall(function() return humanoid:GetAppliedDescription() end)
        if ok and description then
            local result = math.clamp(numberOr(description.HeadScale, 1), 0.5, 2)
            pcall(function() description:Destroy() end)
            return result
        end
        return 1
    ...
    local function applyRemoteFace(entry, overlay, key, headless)
        local texture = runtime.GetFaceTexture(key)
        local head = overlay and overlay:FindFirstChild("Head")
        if not texture or not head or not head:IsA("BasePart") then return end

        -- Dynamic/custom heads do not share the UV layout of classic Roblox faces.
        -- Reuse the same idea as the local face layer: hide only the visual head and
        -- render a classic head mesh on top, while keeping the original Head alive so
        -- hats/attachments continue following the cloned avatar correctly.
        local needsClassic = false
        pcall(function() needsClassic = runtime.FaceNeedsClassicVisual(overlay) == true end)

        if needsClassic then
            head.Transparency = 1
            head.LocalTransparencyModifier = 1
            local originalDecal = head:FindFirstChildWhichIsA("Decal")
            if originalDecal then originalDecal.Transparency = 1 end

            local part = Instance.new("Part")
            part.Name = "XeroRemoteClassicFace"
            part.Size = Vector3.new(2, 1, 1)
            part.CFrame = head.CFrame
            part.Color = head.Color
            part.Material = Enum.Material.SmoothPlastic
            part.Transparency = headless and 1 or 0
            part.LocalTransparencyModifier = 0
            part.CanCollide = false
            part.CanTouch = false
            part.CanQuery = false
            ...
            local mesh = Instance.new("SpecialMesh")
            mesh.Name = "XeroRemoteClassicHeadMesh"
            mesh.MeshType = Enum.MeshType.Head
            mesh.Scale = Vector3.new(1.25, 1.25, 1.25) * getRemoteHeadScale(overlay)
            mesh.Parent = part
            ...
            decal.Transparency = headless and 1 or 0
            decal.Parent = part

            part.Parent = overlay
            entry.RemoteFace = {Part = part, Head = head, Decal = decal}
            return
        end
    ...
    local function applyRemoteLayers(entry, overlay, state)
        if not overlay or type(state) ~= "table" then return end

        local enabledSet = {}
        for _, key in ipairs(type(state.enabled) == "table" and state.enabled or {}) do
            if type(key) == "string" and runtime.AppearanceCatalog[key] then
                enabledSet[key] = true
            end
        end

        -- Una sola cara puede estar activa visualmente.
        for _, key in ipairs(runtime.AppearanceOrder) do
            local asset = runtime.AppearanceCatalog[key]
            if enabledSet[key] and asset and asset.Kind == "Face" then
                applyRemoteFace(entry, overlay, key, state.headless == true)
                break
            end
        end

        -- Limiteds/accesorios propios de XeroHub.
        for _, key in ipairs(runtime.AppearanceOrder) do
            local asset = runtime.AppearanceCatalog[key]
            if enabledSet[key] and asset and asset.Kind == "Accessory" then
                attachRemoteAccessory(overlay, key, state.offsets and state.offsets[key])
            end
        end

        if state.hideHair == true then
            for _, object in ipairs(overlay:GetChildren()) do
                if object:IsA("Accoutrement") and isHairAccessory(object) then
                    for _, visual in ipairs(object:GetDescendants()) do
                        if visual:IsA("BasePart") then visual.LocalTransparencyModifier = 1 end
                    end
                end
            end
        end

        if state.korblox == true then
            ...
            if humanoid and humanoid.RigType == Enum.HumanoidRigType.R15 then
                ...
                if lower then lower.LocalTransparencyModifier = 1 end
                if foot then foot.LocalTransparencyModifier = 1 end
            end
        end

        if state.headless == true then
            if entry.RemoteFace and entry.RemoteFace.Part then
                entry.RemoteFace.Part.Transparency = 1
                if entry.RemoteFace.Decal then entry.RemoteFace.Decal.Transparency = 1 end
            end
            local head = overlay:FindFirstChild("Head")
            if head and head:IsA("BasePart") then
                head.LocalTransparencyModifier = 1
    ...
    local function prepareRemoteOverlayPhysics(overlay)
        for _, object in ipairs(overlay:GetDescendants()) do
            if object:IsA("BasePart") then
                object.CanCollide = false
                object.CanTouch = false
    ...
    local function findSharedRigAttachment(part0, part1)
        if not part0 or not part1 then return nil, nil end
        for _, object in ipairs(part0:GetChildren()) do
            if object:IsA("Attachment") and string.find(object.Name, "RigAttachment", 1, true) then
                local other = part1:FindFirstChild(object.Name)
                if other and other:IsA("Attachment") then
                    return object, other
                end
    ...
    local function bindRemotePose(entry, targetPlayer, char, overlay)
        local humanoid = char:FindFirstChildOfClass("Humanoid")
        local realRoot = char:FindFirstChild("HumanoidRootPart")
        local overlayRoot = overlay:FindFirstChild("HumanoidRootPart")
        if not humanoid or not realRoot or not overlayRoot then return false end

        local realParts = {}
        local cloneParts = {}
        for _, object in ipairs(char:GetChildren()) do
            if object:IsA("BasePart") then realParts[object.Name] = object end
        end
        for _, object in ipairs(overlay:GetChildren()) do
            if object:IsA("BasePart") and object.Name ~= "XeroRemoteClassicFace" then
                cloneParts[object.Name] = object
            end
        end

        -- IMPORTANTE PARA JUGADORES REMOTOS:
        -- Motor6D.Transform es excelente para el jugador LOCAL porque Animator lo escribe
        -- directamente en nuestro cliente. En otros jugadores ese Transform puede llegar
        -- tarde, incompleto o con un estado distinto al que finalmente Roblox dibuja.
        -- Sus BasePart.CFrame, en cambio, ya contienen la pose replicada FINAL.
        --
        -- Por eso aquí NO copiamos Transform. Recuperamos la pose relativa a partir de
        -- CFrame + pivotes neutrales del rig real y luego la aplicamos sobre LOS pivotes
        -- del avatar clonado. Así animación y proporciones quedan completamente separadas.
        local liveMotorMap = {}
        for _, object in ipairs(char:GetDescendants()) do
            if object:IsA("Motor6D") and object.Part0 and object.Part1 then
                liveMotorMap[object.Part0.Name .. ">" .. object.Part1.Name] = object
        ...
        local specs = humanoid.RigType == Enum.HumanoidRigType.R15 and R15_REMOTE_JOINTS or R6_REMOTE_JOINTS
        local pairsList = {}

        for _, spec in ipairs(specs) do
            local parentName, childName = spec[1], spec[2]
            local realParent, realChild = realParts[parentName], realParts[childName]
            local cloneParent, cloneChild = cloneParts[parentName], cloneParts[childName]

            if realParent and realChild and cloneParent and cloneChild then
                local motor = liveMotorMap[parentName .. ">" .. childName]
                if motor and (motor.Part0 ~= realParent or motor.Part1 ~= realChild) then
                    motor = nil
                end
                ...
                -- Los RigAttachment son la mejor representación del pivote neutral.
                -- Si Duels los reemplazó, C0/C1 del Motor6D actual sirven de respaldo.
                local realC0 = realA0 and realA0.CFrame or (motor and motor.C0 or nil)
                local realC1 = realA1 and realA1.CFrame or (motor and motor.C1 or nil)

                if cloneA0 and cloneA1 and realC0 and realC1 then
                    pairsList[#pairsList + 1] = {
                        ...
                        CloneC0 = cloneA0.CFrame,
                        CloneC1Inverse = cloneA1.CFrame:Inverse(),
                        PoseFromReplicatedParts = true,
                    }
                else
                    -- R6/custom fallback: conserva la geometría neutral del clon y sólo
                    -- aplica el delta observado en el rig remoto.
                    local realRelative = realParent.CFrame:ToObjectSpace(realChild.CFrame)
                    pairsList[#pairsList + 1] = {
                        RealParent = realParent,
                        RealChild = realChild,
                        CloneParent = cloneParent,
                        CloneChild = cloneChild,
                        InitialRealRelativeInverse = realRelative:Inverse(),
                        InitialCloneRelative = cloneParent.CFrame:ToObjectSpace(cloneChild.CFrame),
        ...
        if #pairsList == 0 then return false end
        entry.MotorPairs = pairsList

        local function syncPose(now, forceHidden)
            if not runtime.Alive or not syncState.Alive or entry.Overlay ~= overlay
                or entry.Character ~= char or targetPlayer.Character ~= char
                or not overlay.Parent or not char.Parent then
                return false
            end

            -- Movimiento global del jugador sí se copia directamente.
            overlayRoot.CFrame = realRoot.CFrame

            pcall(function()
                -- Orden raíz -> extremidades: cada cloneParent ya quedó colocado antes
                -- de calcular su hijo. Eso evita que Head/brazos usen el frame anterior.
                for i = 1, #pairsList do
                    ...
                    if realParent and realParent.Parent
                        ...
                        if pair.PoseFromReplicatedParts then
                            -- Recupera ÚNICAMENTE la pose animada del rig remoto:
                            --      pose = (Parent * neutralC0)^-1 * (Child * neutralC1)
                            -- Al aplicarla con los pivotes DEL CLON, sus escalas/bundles
                            -- nunca contaminan cuello, brazos, piernas ni torso.
                            local parentJointWorld = realParent.CFrame * pair.RealC0
                            local childJointWorld = realChild.CFrame * pair.RealC1
            ...
            -- La cara clásica se pega DESPUÉS de resolver Neck/Head este mismo frame.
            -- Así nunca puede quedarse una actualización atrás respecto a la cabeza.
            local remoteFace = entry.RemoteFace
            if remoteFace and remoteFace.Part and remoteFace.Part.Parent
                and remoteFace.Head and remoteFace.Head.Parent then
                remoteFace.Part.CFrame = remoteFace.Head.CFrame
                remoteFace.Part.Color = remoteFace.Head.Color
                remoteFace.Part.CastShadow = remoteFace.Head.CastShadow
            end

            -- La invisibilidad del avatar base ya no se recorre 60 veces/segundo.
            -- Los cambios de Tool siguen siendo event-driven y este guard sólo
            -- reafirma LTM unas pocas veces por segundo por si otro sistema lo toca.
            enforceRemoteHidden(entry, char, now, forceHidden)
            return true
        end

        entry.SyncPose = syncPose
        addRenderEntry(entry)
        syncPose(os.clock(), true)
        return true
    end

    local function cloneRemoteTemplate(baseUserId, rigType, outfitId)
        outfitId = tonumber(outfitId)
        local sourceKey = outfitId and ("outfit:" .. tostring(outfitId)) or ("user:" .. tostring(baseUserId))
        local key = sourceKey .. "|" .. tostring(rigType)
        local cached = syncState.TemplateCache[key]
        if cached and cached.Model then
            ...
            pcall(function() cached.Model:Destroy() end)
            syncState.TemplateCache[key] = nil
        end
        ...
        if outfitId then
            ...
            if not okDescription or not description then
                okDescription, description = pcall(function()
                    return Players:GetHumanoidDescriptionFromOutfitId(outfitId)
                end)
            end
            if okDescription and description then
                template, err = runtime.CreateAvatarCloneTemplateFromDescription(description, rigType)
                pcall(function() description:Destroy() end)
            else
                err = "No se pudo cargar el outfit remoto"
            end
        else
            template, err = runtime.CreateAvatarCloneTemplateFromUserId(baseUserId, rigType)
        end
        if not template then return nil, err end
        template.Parent = nil
        syncState.TemplateCache[key] = {Model = template, LastUsed = os.clock()}

        local count = 0
        local oldestKey, oldestAt = nil, math.huge
        for cacheKey, item in pairs(syncState.TemplateCache) do
            count = count + 1
            local used = numberOr(item and item.LastUsed, 0)
            if cacheKey ~= key and used < oldestAt then
                oldestAt = used
                oldestKey = cacheKey
            end
        end
        if count > syncState.TemplateCacheLimit and oldestKey then
            local oldest = syncState.TemplateCache[oldestKey]
            syncState.TemplateCache[oldestKey] = nil
        ...
        return nil, "No se pudo clonar el template cacheado"
    end

    local function rebuildRemoteEntry(userData)
        local userId = tonumber(userData and userData.userId)
        ...
        local state = type(userData.state) == "table" and userData.state or {}
        local revision = userData.revision
        local entry = syncState.Entries[userId]
        if not entry then
            entry = {
                UserId = userId,
                ...
                HiddenList = {},
                HiddenIndex = setmetatable({}, {__mode = "k"}),
                ToolGuards = setmetatable({}, {__mode = "k"}),
            }
            syncState.Entries[userId] = entry
        end

        if entry.Revision == revision and entry.Character == targetPlayer.Character
            and entry.Overlay and entry.Overlay.Parent then
            return
        end
        ...
        disconnectEntry(entry)
        -- disconnectEntry incrementa otra vez para invalidar trabajos anteriores.
        ...
        task.spawn(function()
            ...
            if not humanoid then
                ...
                humanoid = char and char:WaitForChild("Humanoid", 3)
            end
            if not runtime.Alive or not syncState.Alive or not char or not humanoid
                or syncState.Entries[userId] ~= entry or entry.Generation ~= generation then
                return
            end

            local baseUserId = tonumber(state.baseUserId) or userId
            local overlay, err = cloneRemoteTemplate(baseUserId, humanoid.RigType, state.outfitId)
            if not overlay then
                warn("[XeroHub AppearanceSync] No se pudo crear overlay de " .. tostring(userId) .. ": " .. tostring(err))
                return
            end

            if syncState.Entries[userId] ~= entry or entry.Generation ~= generation then
                overlay:Destroy()
                return
            end

            overlay.Name = "XeroRemoteAppearance_" .. tostring(userId)
            prepareRemoteOverlayPhysics(overlay)
            overlay.Parent = getSyncFolder()
            entry.Overlay = overlay
            entry.Character = char

            local layersOk, layersErr = pcall(function()
                applyRemoteLayers(entry, overlay, state)
                prepareRemoteOverlayPhysics(overlay)
            end)
            if not layersOk then
                warn("[XeroHub AppearanceSync] Capa remota parcial para " .. tostring(userId) .. ": " .. tostring(layersErr))
            end

            -- IMPORTANTE: primero demostramos que el overlay puede seguir la pose.
            -- El Character real se oculta SOLAMENTE después. Si cualquier dependencia
            -- del renderer falla, el jugador continúa visible con su avatar normal.
            if not bindRemotePose(entry, targetPlayer, char, overlay) then
                warn("[XeroHub AppearanceSync] No se pudo enlazar la pose de " .. tostring(userId) .. "; se conserva su avatar normal.")
                entry.Overlay = nil
                pcall(function() overlay:Destroy() end)
                return
            end
            bindRemoteBaseHidden(entry, char)

            local charConnection = targetPlayer.CharacterAdded:Connect(function()
                if syncState.Entries[userId] ~= entry then return end
                -- El siguiente fetch reconstruirá con el Character nuevo.
                entry.Character = nil
            end)
            table.insert(entry.Connections, charConnection)
        end)
    end

    local function reconcileRemoteStates(users)
        local seen = {}
        for _, userData in ipairs(users or {}) do
            local userId = tonumber(userData and userData.userId)
            if userId and userId ~= player.UserId then
                seen[userId] = true
                rebuildRemoteEntry(userData)
            end
        end

        for userId in pairs(syncState.Entries) do
            if not seen[userId] then destroyEntry(userId) end
        end
    end

    -- Un solo RenderStepped para TODOS los usuarios remotos. Antes cada usuario
    -- abría su propia conexión; ahora sólo iteramos las entradas activas.
    syncState.RenderConnection = runtime.Track(RunService.RenderStepped:Connect(function()
        if not runtime.Alive or not syncState.Alive then return end
        local now = os.clock()
        local list = syncState.RenderEntries
        local index = 1
        while index <= #list do
            local entry = list[index]
            local syncPose = entry and entry.SyncPose
            if not entry or not syncPose or not entry.Overlay or not entry.Overlay.Parent then
                if entry then
                    removeRenderEntry(entry)
                else
                    local last = list[#list]
                    list[index] = last
                    list[#list] = nil
                    if last then last.RenderIndex = index end
                end
            elseif syncPose(now, false) == false then
                removeRenderEntry(entry)
            else
                index = index + 1
            end
    ...
    runtime.AppearanceSyncCleanup = function()
        if not syncState.Alive then return end
        syncState.Alive = false
        for userId in pairs(syncState.Entries) do
            destroyEntry(userId)
        end
        table.clear(syncState.RenderEntries)
        for key, cached in pairs(syncState.TemplateCache) do
            if cached and cached.Model then
                pcall(function() cached.Model:Destroy() end)
            end
            syncState.TemplateCache[key] = nil
    ...
    -- Publicador: sólo incrementa Revision cuando el aspecto realmente cambia.
    task.spawn(function()
        while runtime.Alive and syncState.Alive do
            if syncRequest and tostring(game.JobId) ~= "" then
                local state = buildLocalAppearanceState()
                local encoded = HttpService:JSONEncode(state)
                if encoded ~= syncState.LastSerialized then
                    syncState.LastSerialized = encoded
                    syncState.Revision += 1
                end

                local now = os.clock()
                if now - syncState.LastPublishAt >= 6 or syncState.Revision == 1 then
                    if httpPostState(state, syncState.Revision) then
                        syncState.LastPublishedRevision = syncState.Revision
                        syncState.LastPublishAt = now
                    end
                elseif encoded ~= nil and syncState.LastPublishedRevision ~= syncState.Revision then
                    if httpPostState(state, syncState.Revision) then
                        syncState.LastPublishedRevision = syncState.Revision
                        syncState.LastPublishAt = now
                    end
    ...
    -- Receptor: consulta ligera. Las revisiones sin cambios no reconstruyen nada.
    task.spawn(function()
        while runtime.Alive and syncState.Alive do
            local users = httpGetStates()
            if users then reconcileRemoteStates(users) end
            task.wait(1.5)
        end
    end)

    runtime.Track(Players.PlayerRemoving:Connect(function(leaving)
        if leaving and leaving.UserId ~= player.UserId then
            destroyEntry(leaving.UserId)
        end
    end))
end
end
]====]))()(runtime, player, Players, HttpService, workspace, RunService, isHairAccessory, manualAttachAppearanceAccessory, KORBLOX_UPPER_MESH, KORBLOX_TEXTURE)
